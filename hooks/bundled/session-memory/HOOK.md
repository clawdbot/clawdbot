...
1. **Finds the previous session** - Locates the most recent session file before the reset
2. **Extracts conversation** - Reads the last 15 lines of conversation from the session
3. **Generates descriptive slug** - Uses LLM to create a meaningful filename slug based on conversation content
4. **Saves to memory** - Creates a new file at `~/clawd/memory/YYYY-MM-DD-slug.md`
