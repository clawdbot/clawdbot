...
describeControlUiE2e("Control UI Memory defaults mocked Gateway E2E", () => {
  ...
  it("locks curated settings when an admin Gateway omits config.set", async () => {
    const context = await browser.newContext({
      colorScheme: "dark",
      locale: "en-US",
      serviceWorkers: "block",
    ...
    const gateway = await installMockGateway(page, {
      ...
      methodResponses: {
        "config.get": {
          config,
          hash: "memory-read-only-e2e",
          appliedConfigHash: "memory-read-only-e2e",
          issues: [],
          raw: JSON.stringify(config),
          valid: true,
        },
    ...
    try {
      ...
      const engine = settingsRow(page, "Memory engine").locator("wa-radio-group");
      const backend = settingsRow(page, "Retrieval backend").locator("wa-radio-group");
      const frequency = settingsRow(page, "Dreaming frequency").getByRole("textbox");
      await expect.poll(() => engine.getAttribute("disabled")).not.toBeNull();
      await expect.poll(() => backend.getAttribute("disabled")).not.toBeNull();
      await expect.poll(() => frequency.isDisabled()).toBe(true);
