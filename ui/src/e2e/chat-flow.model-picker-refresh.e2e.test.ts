...
suite.define(() => {
  ...
  it("keeps the warm model list interactive while a picker-open read is in flight", async () => {
    const context = await suite.newBrowserContext({
      locale: "en-US",
      serviceWorkers: "block",
      viewport: { height: 900, width: 1280 },
    ...
    try {
      ...
      expect(requireRecord(request.params)).toMatchObject({ view: "configured" });
      expect(requireRecord(request.params).refresh).toBeUndefined();
