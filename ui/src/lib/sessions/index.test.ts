...
describe("createSessionCapability", () => {
  it("retries a rejected message unsubscribe against the original live Gateway observer", async () => {
  ...
  it("shares canonical message observers across capabilities without an early release", async () => {
  ...
  it("upgrades a plain message observer for approvals without downgrading existing owners", async () => {
  ...
  it("coalesces plain owners onto an existing approval observer without downgrading it", async () => {
    const request = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      if (method === "sessions.messages.subscribe") {
        return {
          key: params?.key,
  ...
  it("does not share targeted global observers across selected agents", async () => {
    const request = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      if (method === "sessions.messages.subscribe") {
        return { key: params?.key };
      }
