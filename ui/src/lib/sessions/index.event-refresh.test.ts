...
describe("event-driven session list refresh", () => {
  ...
  it("retains the owner-first tail when a session event replaces the canonical list", async () => {
    vi.useFakeTimers();
    ...
    const request = vi.fn(async (method: string, params?: { ownerId?: string }) => {
      ...
      return params?.ownerId === ownerId
        ? sessionsResult(1, [ownerHead, ownerTail])
        : sessionsResult(2, [ownerHead, shared]);
    });
    const { sessions, emitEvent } = createHarness(
      request as unknown as GatewayBrowserClient["request"],
      ownerId,
    ...
    try {
      ...
      emitEvent(sessionChangedEvent(shared.key));
      await vi.advanceTimersByTimeAsync(SESSION_EVENT_REFRESH_DEBOUNCE_MS);
