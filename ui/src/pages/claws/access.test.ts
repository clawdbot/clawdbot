...
describe("clawMutationAvailable", () => {
  it("requires operator.admin and every advertised mutation", () => {
    const methods = ["claws.add.apply", "claws.update.apply"];
    expect(clawMutationAvailable(snapshot(["operator.read"], methods), methods)).toBe(false);
    expect(clawMutationAvailable(snapshot(["operator.admin"], [methods[0]]), methods)).toBe(false);
