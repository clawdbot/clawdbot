...
import { renderBookmarkHistory } from "./chat-bookmark-history.ts";
import type { ImageRenderOptions } from "./chat-message-media.ts";
...
export function renderChatBookmarksDialog(
  ...
  if (!state.open || !scope || !scope.isCurrent()) {
    return nothing;
  }
  ...
  return html`
    <openclaw-modal-dialog label=${title} @modal-cancel=${close}>
      <section
        class="exec-approval-card chat-bookmarks-dialog ${state.history ? "chat-bookmarks-dialog--history" : ""}"
        @click=${guardOwner}
        @input=${guardOwner}
        @submit=${guardOwner}
      >
        ${
          state.history
            ? renderBookmarkHistory(
                ...
                {
                  close,
                  back: () => state.backToList(),
                  update: actions.update,
                  retry: () => {
                    if (state.history) {
                      void state.showHistory(state.history.bookmark);
                    }
                  },
                },
                actions.media,
                  ...
                  <ul class="chat-bookmarks-dialog__list" aria-busy=${state.loading}>
                    ${repeat(
                      ...
                      (item) => html` <li class="chat-bookmarks-dialog__item">
                        ...
                        >
                          ${item.name}
                          ${state.canOpen(item) && item.sessionId !== scope.sessionId ? html`<br /><small class="muted">${t("chat.bookmarks.earlier")}</small>` : nothing}
