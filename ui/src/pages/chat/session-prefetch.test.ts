...
describe("recent session prefetch", () => {
  ...
  it("keeps an unchanged session warm when another session refreshes the roster during prefetch", async () => {
    ...
    updatePrefetch({
      ...snapshot,
      listRevision: 2,
      rows: [{ ...unchanged }, { ...other, updatedAt: NOW + 1 }],
    });
    response.resolve(historyResult(key));
    await settlePromises();
