...
describe("ConfigPage custom theme import ownership", () => {
  it("keeps Replace then Clear authoritative when the replacement resolves late", async () => {
    ...
    const replacementTheme = customThemeFixture("Replacement", "replacement");
    ...
    replacement.resolve(replacementTheme);
    await pendingImport;
  ...
  it("lets a newer import own state while the stale import settles", async () => {
