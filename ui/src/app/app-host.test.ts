  ...
  it("replaces an unresolvable session with the owning agent's main chat", () => {
    ...
    shell.runtime = {
      context: {
        ...
        gateway: { setSessionKey, snapshot: { phase: "connected", hello: null } },
        sessions: {
          state: {
            result: {
    ...
    shell.activeSessionKey = "agent:main:deleted-thread";
    shell.routeState = { routeId: "chat" };

    shell.replaceChatWithCurrentSession();

    expect(setSessionKey).toHaveBeenCalledExactlyOnceWith("agent:main:main");
  ...
  it("keeps a resolvable active session when a different chat route is not found", () => {
    ...
    shell.runtime = {
      context: {
        ...
        agents: { state: { agentsList: { defaultId: "main", mainKey: "main" } } },
        agentSelection: { state: { selectedId: "main" } },
        gateway: { snapshot: { phase: "connected", hello: null } },
        sessions: {
          state: {
            result: {
              sessions: [{ key: "agent:main:existing-thread" }],
            },
          },
        },
        replace,
    ...
    shell.activeSessionKey = "agent:main:existing-thread";
    shell.routeState = { routeId: "chat" };
  ...
  it("honors a deleted session event before its stale cached row is refreshed", () => {
    ...
    shell.runtime = {
      context: {
        ...
        sessions: {
          state: {
            deletedSessions: [{ agentId: "main", key: deletedKey }],
            result: {
              sessions: [{ key: deletedKey }, { key: "agent:main:main" }],
            },
          },
        },
        replace,
    ...
    shell.activeSessionKey = deletedKey;
    shell.routeState = { routeId: "chat" };
  ...
  it("rejects a late route commit for a session already marked deleted", () => {
    ...
    shell.updateRouteState(
      selectShellRouteState(
        committedRouterState("chat", "/chat/main/deleted-thread", {
          kind: "session",
