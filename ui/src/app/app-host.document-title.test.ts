...
describe("OpenCrustacean shell document title", () => {
  ...
  it("keeps the boot title before a route commits", () => {
    ...
    document.title = "OpenCrustacean Control";

    shell.routeState = {};
    shell.syncDocumentTitle();
    expect(document.title).toBe("OpenCrustacean Control");
  });
  ...
  it("uses the route title for a connected route", () => {
    ...
    shell.syncDocumentTitle();
    expect(document.title).toBe("Usage — OpenCrustacean");
  });
  ...
  it("uses the active session's derived title for a non-main chat", () => {
    ...
    expect(document.title).toBe("Quarterly launch plan — OpenCrustacean");
  });
  ...
  it("uses the agent name for an agent main chat", () => {
    ...
    expect(document.title).toBe("Molty — OpenCrustacean");
  });
  ...
  it("falls back to the session display name when the main agent is missing", () => {
    ...
    expect(document.title).toBe("Fallback thread — OpenCrustacean");
  });
  ...
  it("prefixes the pending approval count", () => {
    ...
    expect(document.title).toBe("(2) Usage — OpenCrustacean");
  });
  ...
  it("shows offline instead of a stale approval count", () => {
    ...
    expect(document.title).toBe("(Offline) Usage — OpenCrustacean");
  });
  ...
  it("includes stored chat outbox messages in the offline marker", () => {
    ...
    expect(document.title).toBe("(Offline · 3 queued) Usage — OpenCrustacean");
  });
  ...
  it("uses the meaningful custodian label without a brand suffix", () => {
    ...
    expect(document.title).toBe("Ask OpenCrustacean");
  });
});
