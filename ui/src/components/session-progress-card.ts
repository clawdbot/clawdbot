...
function progressStepMarker(status: PresentedProgressStepStatus) {
  switch (status) {
    case "completed":
      return icons.check;
    case "in_progress":
...
function renderSteps(card: ProgressCard, hasActiveRun: boolean) {
  const steps = card.steps;
  if (!steps?.length) {
    return nothing;
  }
  return html`<ol class="session-progress-card__steps">
    ${steps.map((step) => {
      const presentedStatus =
        step.status === "in_progress" && !hasActiveRun ? "paused" : step.status;
      const statusLabel =
        presentedStatus === "paused"
          ? t("sessionProgressCard.status.paused")
          : t(STATUS_LABEL_KEYS[presentedStatus]);
      return html`<li
      ...
      >
        <span
          ...
          aria-hidden="true"
          >${progressStepMarker(presentedStatus)}</span
        >
...
function renderBody(card: ProgressCard, hasActiveRun: boolean) {
  return html`<div class="session-progress-card__body">
    ${renderMarkdown(card.markdown)} ${renderSteps(card, hasActiveRun)}
  </div>`;
}
...
export function renderSessionProgressCard(
  ...
  if (placement === "composer") {
    ...
    const presentedCurrentStatus =
      currentStep?.status === "in_progress" && !hasActiveRun ? "paused" : currentStep?.status;
