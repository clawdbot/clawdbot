...
use openclaw_node_host::{
    read_sidecar_frame, write_sidecar_frame, AuthenticatedSidecarChannel, SidecarHandshake,
    SidecarHandshakeState, SidecarLimits, SidecarPeerIdentity, SidecarPeerRole,
...
async fn authenticated_sidecar_crosses_a_real_process_boundary() {
    ...
    let ready = channel.seal(&"supervisor-ready").unwrap();
    write_sidecar_frame(&mut output, &ready, channel.max_frame_bytes(), IO_TIMEOUT)
        .await
        .unwrap();
    let reply = read_sidecar_frame(&mut input, channel.max_frame_bytes(), IO_TIMEOUT)
        .await
        .unwrap();
    assert_eq!(channel.open::<String>(&reply).unwrap(), "runtime-ready");
...
async fn sidecar_process_child() {
    ...
    let ready = read_sidecar_frame(&mut input, channel.max_frame_bytes(), IO_TIMEOUT)
        .await
        .unwrap();
    assert_eq!(channel.open::<String>(&ready).unwrap(), "supervisor-ready");
    let reply = channel.seal(&"runtime-ready").unwrap();
    write_sidecar_frame(&mut output, &reply, channel.max_frame_bytes(), IO_TIMEOUT)
        .await
        .unwrap();
}
