...
const WINDOWS_PROCESS_IDENTITY_SCOPE_RE =
  /^(?:src\/(?:shared\/pid-alive(?:\.[a-z-]+)?(?:\.test)?\.ts|infra\/(?:windows-port-pids|gateway-lock)(?:\.test)?\.ts|node-host\/node-worker-process-identity(?:\.test)?\.ts|cron\/store\/run-receipt-store(?:\.test)?\.ts)|test\/e2e\/windows-cron-process-identity\.e2e\.test\.ts)$/;
