...
const GIT_OWNER_SCOPE_RE =
  /^(?:\.github\/(?:actions\/(?:git-owner|ensure-base-commit)\/|workflows\/(?:workflow-sanity|qa-profile-evidence)\.yml$)|scripts\/generate-ci-git-owner\.mts$|test\/scripts\/(?:ci-(?:checkout|git-owner|linux-git|platform-checkout)\.test(?:-support)?\.ts|fixtures\/ci-platform-checkout\.mjs)$)/;
