...
type RunGh = (args: string[], options?: { input?: string }) => string;
type ExecFile = (
  command: string,
  args: string[],
  options: ExecFileSyncOptionsWithStringEncoding,
) => string;
type PullRequest = ReturnType<typeof loadPr>;
type CloseEvidence = {
  sharedIssues: number[];
  overlappingHunks: boolean;
  sharedFiles: string[];
};
 ...
 */
export function parsePrNumberList(value: unknown) {
...
export function parseArgs(argv: string[], env: NodeJS.ProcessEnv = process.env) {
  ...
  return help ? Object.assign(args, { help: true } satisfies { help: true }) : args;
}

type CloseArgs = ReturnType<typeof parseArgs>;

export function defaultRunGh(
  args: string[],
  options: { input?: string } = {},
  params: { execFileSyncImpl?: ExecFile } = {},
) {
...
function issueRefsFromPr(pr: PullRequest) {
 ...
 */
export function parseUnifiedDiffRanges(diffText: unknown) {
...
type DiffRanges = ReturnType<typeof parseUnifiedDiffRanges>;

/**
 * Reports whether two PR diffs touch overlapping hunks.
 */
function hasOverlappingHunks(leftRanges: DiffRanges, rightRanges: DiffRanges) {
...
function filePaths(pr: PullRequest) {
  const files: unknown[] = Array.isArray(pr.files) ? pr.files : [];
  return new Set(
    files.flatMap((file) => (isRecord(file) && typeof file.path === "string" ? [file.path] : [])),
  );
}

function formatEvidence(evidence: CloseEvidence) {
  const parts: string[] = [];
...
function loadPr(repo: string, number: number, runGh: RunGh) {
  const pullRequest: unknown = JSON.parse(
    runGh([
      "pr",
      "view",
      String(number),
...
function loadDiff(repo: string, number: number, runGh: RunGh) {
...
export function applyClosePlan({
  ...
  labels?: string[];
  plan: ReturnType<typeof buildDuplicateClosePlan>;
  repo: string;
  runGh: RunGh;
}) {
 ...
 */
export function runDuplicateCloseWorkflow(args: CloseArgs, runGh: RunGh = defaultRunGh) {
  if (!args.landedPr) {
    throw new Error("--landed-pr is required");
  }
