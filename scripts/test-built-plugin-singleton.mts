...
const registryHandles = new Set<{ release(): void }>();
function loadBuiltRegistry(...args: Parameters<typeof loadOpenClawPlugins>) {
  const handle = loadOpenClawPlugins(...args);
...
try {
  ...
  const smsRegistry = loadBuiltRegistry(
    buildPluginRuntimeLoadOptions(
      resolvePluginRuntimeLoadContext({
        config: {
          plugins: {
  ...
  const { acquirePluginDiscoveryProvidersRuntime } = await import(
  ...
  for (const toolDiscovery of [false, true]) {
    for (const preferBuiltPluginArtifacts of [undefined, false]) {
      ...
      const options = toolDiscovery
        ...
        : values;
      const selected = loadBuiltRegistry({
        ...options,
        cache: false,
        activate: false,
        toolDiscovery,
      ...
      if (!toolDiscovery) {
        ...
        const providerHandle = withPluginRuntimeGenerationScope(
        ...
        try {
          assert.equal(
            providerHandle.providers[0]?.label,
            label,
  ...
  for (const [orderIndex, preferences] of [
    ...
    const registries = preferences.map((preferBuiltPluginArtifacts) => {
      const selected = loadBuiltRegistry({ ...options, preferBuiltPluginArtifacts });
    ...
    for (const [index, preferBuiltPluginArtifacts] of preferences.entries()) {
      assert.equal(
        loadBuiltRegistry({ ...options, preferBuiltPluginArtifacts }),
        registries[index],
  ...
  const registry = loadBuiltRegistry({
    cache: false,
    workspaceDir: tempRoot,
    env: {
      ...process.env,
  ...
  try {
    for (const handle of registryHandles) {
      handle.release();
    }
    registryHandles.clear();
    // Import only after the cold built-loader checks; the shared owner joins disposal.
    const { drainPluginRegistryResourceDisposals } =
      await import("../src/plugins/registry-resources.js");
