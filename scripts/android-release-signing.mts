...
import { runAndroidSigningCommandSync } from "./lib/android-release-signing-process.mts";
...
type CommandOptions = NonNullable<Parameters<typeof runAndroidSigningCommandSync>[2]>;
type CryptOptions = { decrypt: boolean; inputPath: string; outputPath: string };
type SigningProperties = Map<string, string>;
...
type SigningOptions = ReturnType<typeof parseArgs>;
...
function requireGradlePropertyNames(value: unknown) {
  if (
    !Array.isArray(value) ||
...
function resolveMaterializedDir(manifest: SigningManifest, options: SigningOptions) {
  return options.materializedDir || path.resolve(rootDir, manifest.materializedRoot);
}

function resolveWorkspace(manifest: SigningManifest, options: SigningOptions) {
  return options.workspace || path.join(resolveMaterializedDir(manifest, options), "apps-signing");
}

function assertWorkspaceInsideMaterialized(workspace: string, materializedDir: string) {
...
function assetDir(workspace: string, manifest: SigningManifest) {
  return path.join(workspace, manifest.assetPath);
}

function encryptedKeystorePath(workspace: string, manifest: SigningManifest) {
  return path.join(assetDir(workspace, manifest), manifest.uploadKeystoreEncryptedFile);
}

function encryptedPropertiesPath(workspace: string, manifest: SigningManifest) {
  return path.join(assetDir(workspace, manifest), manifest.gradlePropertiesEncryptedFile);
}

function materializedKeystorePath(materializedDir: string) {
  return path.join(materializedDir, "upload-keystore.jks");
}

function materializedPropertiesPath(materializedDir: string) {
...
function runText(command: string, args: string[], options: CommandOptions = {}) {
  return String(
    runAndroidSigningCommandSync(command, args, {
      cwd: options.cwd,
      env: options.env || process.env,
...
function cloneSigningRepo(manifest: SigningManifest, workspace: string, materializedDir: string) {
...
function opensslCrypt({ decrypt, inputPath, outputPath }: CryptOptions) {
...
function readProperties(filePath: string) {
  const properties: SigningProperties = new Map();
...
function requireProperties(properties: SigningProperties, names: string[], filePath: string) {
...
function writeMaterializedProperties(materializedDir: string, sourceProperties: SigningProperties) {
...
function validateMaterializedSigning(materializedDir: string) {
  const keystorePath = materializedKeystorePath(materializedDir);
  const propertiesPath = materializedPropertiesPath(materializedDir);
...
function writePlan(manifest: SigningManifest, options: SigningOptions) {
...
function writeSigningRepoManifest(workspace: string, manifest: SigningManifest) {
...
function materialize(manifest: SigningManifest, options: SigningOptions) {
...
function syncPull(manifest: SigningManifest, options: SigningOptions) {
  const workspace = resolveWorkspace(manifest, options);
  const materializedDir = resolveMaterializedDir(manifest, options);
...
function requirePushSources(options: SigningOptions) {
...
function syncPush(manifest: SigningManifest, options: SigningOptions) {
  requireMatchPassword();
