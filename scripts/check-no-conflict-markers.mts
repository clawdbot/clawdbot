 ...
 */
export function findConflictMarkerLines(content: string) {
...
export function findConflictMarkersInFiles(
  filePaths: string[],
  readFile: (path: string) => string | Uint8Array = fs.readFileSync,
) {
  const violations = [];
  for (const filePath of filePaths) {
    ...
    const lines = findConflictMarkerLines(buffer.toString("utf8"));
    if (lines.length > 0) {
      violations.push({
        filePath,
        lines,
 ...
 */
function parseGitGrepConflictMarkerOutput(stdout: Buffer) {
