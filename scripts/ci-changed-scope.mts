...
const NODE_FAST_CI_ROUTING_SCOPE_RE =
  /^(scripts\/(?:ci-changed-scope|check-changed|run-vitest|test-projects(?:\.test-support)?)\.(?:mjs|mts)$|scripts\/lib\/(?:changed-path-facts|ci-changed-node-test-plan)\.(?:mjs|mts)$|src\/commands\/status\.scan-result\.test\.ts$|src\/scripts\/ci-changed-scope(?:\.[^/]+)?\.test\.ts$|test\/scripts\/(?:changed-lanes|changed-path-facts|ci-changed-node-test-plan|run-vitest|test-projects)\.test\.ts$)/;
 ...
 */
export function detectChangedScope(changedPaths: string[]) {
...
function isControlUiCanonicalMemoryMigration(
  changedPaths: string[],
...
function resolveChangedBranchName(): string {
...
export function shouldRunNativeI18n(changedPaths: string[] | null | undefined): boolean {
  return (
 ...
 */
export function detectNodeFastScope(changedPaths: string[]) {
  if (!Array.isArray(changedPaths) || changedPaths.length === 0) {
...
function detectInstallSmokeScopeForPath(path: string) {
 ...
 */
export function detectInstallSmokeScope(changedPaths: string[]) {
  if (!Array.isArray(changedPaths) || changedPaths.length === 0) {
...
export function writeGitHubOutput(
  scope: ReturnType<typeof detectChangedScope>,
  outputPath: string | undefined = process.env.GITHUB_OUTPUT,
  installSmokeScope: ReturnType<typeof detectInstallSmokeScope> = {
    runFastInstallSmoke: scope.runChangedSmoke,
    runFullInstallSmoke: scope.runChangedSmoke,
  },
  nodeFastScope: ReturnType<typeof detectNodeFastScope> = {
    runFastOnly: false,
    runPluginContracts: false,
    runCiRouting: false,
  },
  runNativeI18n = true,
  changedPaths: string[] | null = null,
): void {
  if (!outputPath) {
...
function readRefValue(argv: string[], index: number, optionName: string): string {
