...
class RoomChatTranscriptCacheTest {
  ...
  @Test
  fun cachedInstanceCannotAuthorizeDeletingUnresolvedQueuedInput() =
    runTest {
      store.saveTranscript("gateway-a", "owner-a", "custom", listOf(message("old cached transcript")), sessionId = "retired-id")
      ...
      val controller =
        createChatController(
          ...
          onSessionDeleted = { deletions += it },
        ) { method, _ ->
          if (method == "sessions.list") """{"sessions":[]}""" else error("offline")
        }
      controller.load("custom", ownerAgentId = "owner-a")
      advanceUntilIdle()
      assertNull(controller.sessionId.value)
      assertTrue(controller.sendMessageAwaitAcceptance("unresolved replacement input", "off", emptyList()))
      advanceUntilIdle()
      controller.load("agent:owner-b:main")
      advanceUntilIdle()
      assertTrue(controller.sessions.value.none { it.key == "custom" })
      assertEquals(listOf("unresolved replacement input"), controller.outboxItems.value.map { it.text })

      controller.handleGatewayEvent(
        "sessions.changed",
        """{"reason":"delete","sessionKey":"custom","agentId":"owner-a","sessionId":"retired-id"}""",
      )
      advanceUntilIdle()

      assertEquals(listOf("unresolved replacement input"), controller.outboxItems.value.map { it.text })
      assertEquals("agent:owner-b:main", controller.sessionKey.value)
      assertTrue(store.loadTranscript("gateway-a", "owner-a", "custom").isEmpty())
      assertEquals("retired-id", deletions.single().sessionId)
      assertTrue(!deletions.single().clearLocalInput)
    }

  @Test
  fun confirmedDeleteRetiresAnIdentifiedCacheBeforeHistoryResolves() =
    runTest {
      val key = "agent:main:archived"
      store.saveTranscript("gateway-a", "main", key, listOf(message("old cached transcript")), sessionId = "retired-id")
      val controller =
        createChatController(
          transcriptCache = store,
          cacheScope = { ChatCacheScope("gateway-a", 1) },
          ...
          when (method) {
            ...
            else -> error("history not available")
          }
        }
      controller.load(key)
      advanceUntilIdle()
      assertNull(controller.sessionId.value)
      assertEquals("retired-id", store.loadSessionId("gateway-a", "main", key))
      assertTrue(controller.deleteSession(key) != null)
      advanceUntilIdle()

      assertTrue(controller.sessionKey.value != key)
      assertTrue(store.loadTranscript("gateway-a", "main", key).isEmpty())
      assertTrue(store.loadSessions("gateway-a", "main").none { it.key == key })
    }

  @Test
  fun liveDeletedInstanceRetiresDespiteAnUnidentifiedCachedDrawerRow() =
    runTest {
      val key = "agent:main:older-chat"
      store.saveTranscript("gateway-a", "main", key, listOf(message("cached transcript")), sessionId = "live-id")
      val controller =
        createChatController(
          transcriptCache = store,
          cacheScope = { ChatCacheScope("gateway-a", 1) },
          ...
          when (method) {
            "chat.history" -> historyResponse(if (params.orEmpty().contains(key)) "live-id" else "main-id", emptyList())
            "sessions.list" -> """{"sessions":[]}"""
      ...
      controller.load(key)
      advanceUntilIdle()
      assertEquals("live-id", controller.sessionId.value)
      val retained = controller.sessions.value.single()
      assertEquals(key, retained.key)
      assertNull(retained.sessionId)

      controller.handleGatewayEvent(
        "sessions.changed",
        """{"reason":"delete","sessionKey":"$key","agentId":"main","sessionId":"live-id"}""",
      )
      advanceUntilIdle()

      assertTrue(controller.sessionKey.value != key)
      assertTrue(controller.sessions.value.none { it.key == key })
      assertTrue(store.loadTranscript("gateway-a", "main", key).isEmpty())
      assertTrue(store.loadSessions("gateway-a", "main").none { it.key == key })
    }

  @Test
  fun listedReplacementCannotInheritKnownOrUnidentifiedCachedMessages() =
    runTest {
      for (previousId in listOf("old-id", null)) {
        store.saveTranscript("gateway-a", "main", "main", listOf(message("old transcript")), sessionId = previousId)
        saveSessions(listOf(ChatSessionEntry(key = "main", updatedAtMs = null)))
        assertEquals(previousId, store.loadSessionId("gateway-a", "main", "main"))
        assertEquals(listOf("old transcript"), loadTranscript().map { it.content.single().text })

        saveSessions(listOf(ChatSessionEntry(key = "main", updatedAtMs = null, sessionId = "replacement-id")))
        assertTrue(loadTranscript().isEmpty())
        assertEquals("replacement-id", store.loadSessionId("gateway-a", "main", "main"))
        assertNull(loadSessions().single().sessionId)
      }
    }

  @Test
  fun instanceConditionalDeletionRechecksThePersistedRowAndKeepsOtherScopes() =
    runTest {
      for (previousId in listOf("old-id", null)) {
        store.saveTranscript("gateway-a", "main", "main", listOf(message("old transcript")), sessionId = previousId)
        val observed = store.loadSessionId("gateway-a", "main", "main")
        store.saveTranscript("gateway-a", "main", "main", listOf(message("replacement")), sessionId = "replacement-id")
        store.saveTranscript("gateway-b", "main", "main", listOf(message("other gateway")), sessionId = "replacement-id")
        store.saveTranscript("gateway-a", "other", "main", listOf(message("other owner")), sessionId = "replacement-id")

        store.deleteSession("gateway-a", "main", "main", expectedSessionId = observed)
        assertEquals(listOf("replacement"), loadTranscript().map { it.content.single().text })
        assertEquals("replacement-id", store.loadSessionId("gateway-a", "main", "main"))
        store.deleteSession("gateway-a", "main", "main", expectedSessionId = "replacement-id")
        assertTrue(loadTranscript().isEmpty())
        assertTrue(loadSessions().isEmpty())
        assertEquals(listOf("other gateway"), loadTranscript(gatewayId = "gateway-b").map { it.content.single().text })
        assertEquals(listOf("other owner"), loadTranscript(agentId = "other").map { it.content.single().text })
      }
    }

  @Test
  fun unlistedCachedTranscriptIsDeletedOnlyForItsRecordedInstance() =
    runTest {
      val controller =
        createChatController(
          transcriptCache = store,
          ...
          when (method) {
            "chat.history" -> {
              if (params.orEmpty().contains("custom")) {
                """{"sessionId":"replacement-id","sessionInfo":{"key":"custom","agentId":"owner-a","sessionId":"replacement-id"},"messages":[{"role":"assistant","content":"replacement transcript","timestamp":1}]}"""
              } else {
                historyResponse("other-id", emptyList())
              }
            }
      ...
      assertTrue(controller.sessions.value.none { it.key == "custom" })
      assertEquals(
        listOf("replacement transcript"),
        store.loadTranscript("gateway-a", "owner-a", "custom").map { it.content.single().text },
      )
      ...
      controller.handleGatewayEvent(
        "sessions.changed",
        """{"reason":"delete","sessionKey":"custom","agentId":"owner-a","sessionId":"replacement-id"}""",
      )
      advanceUntilIdle()
      assertTrue(store.loadTranscript("gateway-a", "owner-a", "custom").isEmpty())
      assertTrue(store.loadSessions("gateway-a", "owner-a").none { it.key == "custom" })
      assertEquals("agent:owner-b:main", controller.sessionKey.value)
    }
  ...
  fun fullHistoryUsageClearSurvivesOfflineReopenWithoutAListResponse() =
    runTest {
      ...
      controller.load("main")
      advanceUntilIdle()
      assertEquals("session-1", store.loadSessionId("gateway-a", "main", "main"))
      assertEquals(
        null,
