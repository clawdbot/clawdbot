...
extension ChatViewModelTests {
    ...
    @Test @MainActor func modelCatalogFailureRetainsChoicesAndClearsOnlyItsNotice() async throws {
        ...
        #expect(vm.input == "Unsent draft")
        #expect(vm.errorText == Self.modelCatalogReadFailureNotice)
        await vm.fetchModels()
        #expect(vm.errorText == nil)
    ...
    @Test @MainActor func modelCatalogCancellationAndUnrelatedErrorKeepTheirOwners() async {
    ...
    @Test @MainActor func modelCatalogStaleFailureCannotReplaceNewerReadOutcome() async throws {
        let gate = AsyncGate()
        let choice = modelChoice(id: "alpha", name: "alpha", provider: "f26", available: true)
    ...
    @Test @MainActor func modelCatalogFailureNoticeSurvivesBootstrapCompletion() async throws {
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload()],
    ...
    @Test @MainActor func modelCatalogReadCannotMaskAnInFlightSettingsFailure() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            setSessionModelHook: { _ in
                await patchGate.wait()
                throw NSError(domain: "ModelPatch", code: 1, userInfo: [NSLocalizedDescriptionKey: "Model patch failed"])
            })
        await vm.fetchModels()
        ...
        do {
            ...
            #expect(vm.errorText == "Model patch failed")
        } catch {
            await readGate.open()
            await patchGate.open()
            await oldRead.value
