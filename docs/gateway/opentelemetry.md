...
| `OTEL_SDK_DISABLED`                                                                                               | A case-insensitive `true` disables all plugin-owned trace, metric, log, and stdout routes before endpoint, protocol, or TLS setup. Any other value leaves the SDK enabled; unrecognized values also emit an OpenTelemetry warning. Async context and `OTEL_PROPAGATORS` remain active.                                                      |
