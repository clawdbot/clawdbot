...
| `OTEL_PROPAGATORS`                                                                                                | Propagators kept active when `OTEL_SDK_DISABLED=true`. Defaults to `tracecontext,baggage`; supported values are `tracecontext`, `baggage`, `b3`, `b3multi`, `jaeger`, and `none`. Values are case-insensitive.                                                                                                                                             |
| `OTEL_SDK_DISABLED`                                                                                               | A case-insensitive `true` disables all plugin-owned trace, metric, log, and stdout routes before endpoint, protocol, or TLS setup. Any other value leaves the SDK enabled; unrecognized values also emit a plugin warning. Async context and `OTEL_PROPAGATORS` remain active.                                                                             |
...
| `OPENCLAW_OTEL_PRELOADED`                                                                                         | Set to `1` when another preload or host process already registered the global OpenTelemetry SDK. The plugin then skips its own provider lifecycle but still wires diagnostic listeners and honors `traces`/`metrics`/`logs`. With `OTEL_SDK_DISABLED=true`, external globals remain installed while the plugin registers no telemetry listeners or routes. |
...
When the plugin would otherwise own the telemetry providers, keep propagation available while
