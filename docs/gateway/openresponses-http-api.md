...
- Use `x-openclaw-message-to` (optionally with `x-openclaw-account-id` / `x-openclaw-thread-id`) to give the run a delivery target, so asynchronous out-of-turn output — such as sub-agent completion announces — can be delivered through the bound channel. Like `x-openclaw-model`, this is an owner-level control (requires `operator.admin` on identity-bearing auth paths, otherwise `403 missing scope: operator.admin`). The turn's own reply is still returned only in the HTTP response and is never pushed to a channel. Once accepted, the bound target follows the normal session delivery-route lifecycle (stored on the session, used as is by later out-of-turn deliveries, not re-validated at delivery time — same as routes recorded from inbound channel messages or the WS `agent` method).
