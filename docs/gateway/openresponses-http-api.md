...
- Use `x-openclaw-message-to` (optionally with `x-openclaw-account-id` / `x-openclaw-thread-id`) to give the run a delivery target, so asynchronous out-of-turn output — such as sub-agent completion announces — can be delivered through the bound channel. Like `x-openclaw-model`, this is an owner-level control (requires `operator.admin` on identity-bearing auth paths, otherwise `403 missing scope: operator.admin`). The turn's own reply is still returned only in the HTTP response and is never pushed to a channel.
