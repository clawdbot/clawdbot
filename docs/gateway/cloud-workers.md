...
Use trusted recipes that finish all build and installation writes before returning. Setup runs with the machine account's permissions; deliberately detached daemons are outside its process-group cleanup and can keep modifying the image source. If the setup supervisor is forcibly killed, OpenClaw refuses that preparation and retains teardown ownership until deletion is confirmed.
...
For a Gateway worktree project with a Git commit, initial capture happens during provisioning, before node enrollment. After profile setup, OpenClaw prepares a pristine checkout of the admitted commit, installs the verified node runtime, and captures the machine. The first dispatch includes that work; subsequent sessions can reuse the image without waiting for the first session to stop. Session edits, eligible untracked files, and node enrollment credentials arrive only after capture. Repository-only sessions have no Gateway project checkout to prepare; they use machine/runtime image reuse and node Git seeds, with image capture at an eligible enrolled worker's teardown.

When a compatible prepared image exists, a foreground session restores it, updates to the requested commit, and runs its admitted setup without waiting for replacement snapshot capture. A background reserve independently prepares that same admitted commit and captures any replacement before its own enrollment. With reserve capacity set to zero, no hidden refresh worker is created: later sessions can reuse the retained image, but may repeat its Git update and setup until another permitted capture occurs.
...
Image reuse is keyed by the backend, setup command, sorted `setupEnv` variable names (not their values), desktop setting, exact effective machine class, and project identity when present. Project identity comes from the Gateway's namespace and the canonical shared Git directory. Linked session worktrees from the same repository share it; a new session or commit does not create another project identity. Separate repository clones have separate identities. Prepared project images additionally require a cache fingerprint covering the committed setup recipe, provider/profile settings, machine platform and class, execution mode, runtime artifact and plugin content, and worker bundle. The commit is excluded from this cache fingerprint so an older checkout can accelerate a new commit. Ready workers still require the exact preparation fingerprint, including the commit. Matching the displayed OpenClaw version alone does not establish reuse.
...
Scrub failure releases only its own capture reservation. Once creation starts, failure, timeout, or unusable output leaves its outcome uncertain: the profile stays paused until explicit recovery. The exception is a complete, matching Crabbox `crabbox.checkpoint.create.failure.v1` result ([Crabbox #1952](https://github.com/openclaw/crabbox/pull/1952)) that attests no image submission and verified local reservation removal. OpenClaw clears only that capture; project provisioning still fails and cleans up the lease, because source rollback may have failed. Older CLI errors and lost results remain uncertain. An unresolved project capture prevents node enrollment on that source, so fresh node credentials cannot enter a capture that may still be running. Lease cleanup still runs, and a retained usable image can serve new allocations. Capture needs a Crabbox CLI and backend that support fixed-ID checkpoint forks. Continuing a coordinator-retained `checkpoint_pending` response requires the CLI repair in [Crabbox #1698](https://github.com/openclaw/crabbox/pull/1698); older binaries can accept `--wait` and still fail on that response. Correct missing capabilities or permissions before recovering an uncertain capture.
...
Project preparation checks for the exact pristine seed before building or uploading a Git pack. A ready worker binds its registered fixed workspace in place and applies the current session's eligible file manifest. Other seed-backed workspaces copy only the seed's Git objects into a fresh repository, recreate its Git metadata, and apply that manifest. A matching seed skips both an origin fetch and a full Git pack download, including for private or unpublished commits. When a compatible prepared checkout contains a different commit, OpenClaw verifies its retained Git objects and transfers a delta for the requested commit. The checkout and HOME keep their absolute paths so build caches can survive. Without a retained base, preparation uses the complete Gateway pack; an invalid prepared seed fails visibly. Workspaces without a prepared project keep the eligible origin/seed path. The Gateway builds transfer packs only on demand, and each transfer and background refill retains its admitted commit even if local commits change later.
...
Warm profiles use a version-3 envelope in the existing `warm-images` plugin-state namespace. Ready-worker lifecycle facts and node workspace bindings use state SQLite schema version 17; the per-agent schema is unchanged. Before upgrading, stop the owning Gateway and original capture processes and take a verified backup that includes committed WAL data. Then run:

```bash
openclaw doctor --fix
```

Doctor performs this migration under the Gateway's exclusive maintenance lock. It preserves checkpoint identities, fixed allocation choices, capture selectors, source leases, and retirement obligations. Earlier version-3 records without cache identity or allocation purpose receive explicit unknown values; their exact preparation and demand records remain intact, but they cannot serve new prepared allocations. Legacy images without a proven preparation fingerprint remain unverified; historical last-use timestamps do not become proven demand. Such images remain recorded while pinned and retire through ordinary cleanup when unpinned, rather than serving new prepared allocations. Older empty capture markers become explicitly uncertain captures with their original recovery selector. Unsupported records stay unchanged and produce a warning. Runtime provisioning requires the canonical envelope; it does not silently convert old rows.
...
`sessions.dispatch` closes local turn admission, drains active work, and validates the workspace source. For an eligible Gateway-source project, it first tries to claim a matching ready worker. A hit keeps that worker's existing environment and node identity, mints fresh session authority, applies the session workspace, and activates without another provider allocation or enrollment. On a miss, dispatch provisions the lease for the selected execution mode and runs setup. With project warm images enabled, initial preparation captures the committed checkout and node runtime before enrollment; a foreground restore from a compatible image skips replacement capture. It then enrolls the node, installs the required pinned Gateway bundle, and applies the session workspace. Both paths return once the placement reaches `active` ownership.

Gateway-source inventory validation happens before provider allocation. Repository-only inventory is captured on the enrolled node after fetching the pinned source; either path reports actionable size or entry limits. Budget several minutes for the first cloud dispatch, including capture when needed; subsequent dispatches can claim an eligible ready reserve or reuse the image, project seed, and runtime installs during provisioning. Workspace changes and current authorization still govern activation; a ready pool does not guarantee a fixed startup time for arbitrary transfers or concurrent bursts. After dispatch, talk to the session as usual. OpenClaw turns route to the worker process; Codex native operations run on the authorized cloud node, paired device, or supported SSH-backed provider.
