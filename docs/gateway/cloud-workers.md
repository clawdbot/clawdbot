...
- **Placement is `failed` after a dispatch or bootstrap failure** — `failed` keeps the last error but is not a dead end: the next turn reclaims the placement to `local` and runs locally so the session keeps working, and `sessions.dispatch` accepts `local` again once the underlying cause is fixed.
