...
- `controlUi.github.token`: optional SecretRef-backed service credential for Control UI GitHub previews and project discovery. When omitted, the shipped `GH_TOKEN` then `GITHUB_TOKEN` Gateway environment fallback remains active. This credential is isolated from agent child processes; an explicitly configured but unavailable credential fails closed instead of using the environment fallback.
