...
- [Memory configuration reference](/reference/memory-config) for `memory.search.*`, `memory.citations`, and dreaming config under the selected `plugins.entries.<memory-plugin>.config.dreaming` owner.
...
- `plugins.entries.<memory-plugin>.config.dreaming`: memory dreaming settings on the active memory plugin (selected by `plugins.slots.memory`, by default `memory-core`). `memory-core` executes the lifecycle; a non-core selected plugin must explicitly declare `contracts.memoryDreamingEngines: ["memory-core"]` before OpenClaw loads it as a sidecar. The bundled `memory-lancedb` plugin declares this contract. See [Dreaming](/concepts/dreaming) for phases and thresholds.
...
- Full memory config lives in [Memory configuration reference](/reference/memory-config):
  ...
  - `agents.entries.*.memory.dreaming.enabled` for per-agent automatic Dreaming exclusion
  - `memory.citations`
  - `plugins.entries.<memory-plugin>.config.dreaming`
