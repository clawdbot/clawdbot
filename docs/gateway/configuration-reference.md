...
- `agents.entries.*.plugins.slots`: normal authored per-agent overrides for the same memory role slots. Compatibility/internal list-shaped input may also accept `agents.list[].plugins.slots`.
