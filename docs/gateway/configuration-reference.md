...
- `remote.transport`: `direct` (default for new macOS remote configurations) or `ssh` (explicit fallback). Direct remote clients should use a `wss://` endpoint; plaintext `ws://` is accepted only for loopback, LAN, link-local, `.local`, `.ts.net`, and Tailscale CGNAT hosts. Run `openclaw doctor --fix` after upgrading to make existing remote configurations explicit: routes with `sshTarget` or a loopback tunnel URL become `ssh`; all other routes become `direct`.
