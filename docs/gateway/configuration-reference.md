...
- `webhookTokenHosts`: destination hostnames allowed to receive `webhookToken`. Webhook URLs come from job records that an agent turn can create or edit, so the token is withheld from every host that is not listed and the POST goes out unauthenticated. Set the bare hostnames of your own receivers, with no scheme, port, or path (`hooks.example.com`, not `https://hooks.example.com:8443/cron`); config validation rejects other shapes rather than letting an entry silently never match. Use `["*"]` to attach the token to any destination a job names. A `cron.failureAlert` destination with `mode: "webhook"` is declared in this config file rather than on a job, so its host is trusted automatically and does not need a second entry here.
