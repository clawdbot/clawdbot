...
- `remote.transport`: `direct` (the default for new macOS remote configurations) or `ssh` (explicit fallback). Direct remote clients should use a `wss://` endpoint; plaintext `ws://` is accepted only for loopback, LAN, link-local, `.local`, `.ts.net`, and Tailscale CGNAT hosts. Existing configurations remain unchanged.
