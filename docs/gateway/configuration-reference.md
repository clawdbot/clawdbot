...
- `settings.warmImage`: reusable machine images are enabled by default; set this to `false` to disable capture and image reuse. Images incur provider snapshot storage charges. Capture adds at most about three minutes to a stop on providers with slow native snapshots, then degrades to cold-only provisioning.
