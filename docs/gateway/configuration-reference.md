...
- `remote.transport`: `direct` (the default for new macOS remote configurations) or `ssh` (explicit fallback). Automatic macOS discovery selects `direct` only for `wss://` endpoints and otherwise uses SSH; manually configured plaintext `ws://` is accepted only for loopback, LAN, link-local, `.local`, `.ts.net`, and Tailscale CGNAT hosts. Existing configurations remain unchanged.
