...
- `timeoutSeconds`: compaction abort budget in seconds. Default: `180`. For compaction that reports no progress, this is the maximum total duration of the operation. Context engines that report progress through the compaction progress callback re-arm the timer on each report, so the budget instead bounds maximum silence between reports — a slow but advancing summarization may run longer in total. Progress can never extend the operation past an absolute ceiling of 10x the budget (clamped to the Node-safe timer maximum).
