...
- `qualityGuard`: bounded validation for built-in safeguard summaries. Enabled by default in safeguard mode. After final budgeting, required headings must remain in the retained generated body, while pending asks and exact identifiers must remain in the exact artifact to be stored. When no attempt passes, or the required facts cannot fit the finalized budget, OpenClaw stores a bounded degraded summary instead of cancelling, because cancelling left the session permanently uncompactable. The degraded artifact keeps the structured previous summary, the preserved recent turns, and the generated split-turn prefix, and is logged with `reasonCode=quality_guard_degraded_fallback`. A configured provider that throws or returns an empty result falls back to built-in LLM summarization. A failed built-in generation, an unresolvable summarization model, and missing credentials still preserve the original history and return a compaction failure; caller-initiated aborts and transport timeouts are re-thrown. Set `enabled: false` to skip the audit. Configured compaction-provider output keeps its existing provider-owned validation behavior.
