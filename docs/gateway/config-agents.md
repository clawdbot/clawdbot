...
- `verboseDefault`: default verbose level for agents. Values: `"off"`, `"on"`, `"full"`. Default: `"off"`. Plain-language sentences are a session presentation, not a permanent config value: run `/verbose plain` (TUI/chat/Control UI session picker) to switch a session to non-technical tool sentences.
- `toolProgressDetail`: detail mode for `/verbose` tool summaries and progress-draft tool lines. Values: `"explain"` (default, compact human labels) or `"raw"` (append raw command/detail when available). Plain-language sentences are session presentation only: `/verbose plain` forces plain detail for that session. Per-agent `agents.entries.*.toolProgressDetail` overrides this default.
