...
- `verboseDefault`: default verbose level for agents. Values: `"off"`, `"on"`, `"full"`, `"plain"`. Default: `"off"`. `"plain"` presents tool activity as non-technical sentences (no paths, argv, or tool chrome).
- `toolProgressDetail`: detail mode for `/verbose` tool summaries and progress-draft tool lines. Values: `"explain"` (default, compact human labels), `"raw"` (append raw command/detail when available), or `"plain"` (non-technical sentences only). Session `/verbose plain` forces `"plain"`. Per-agent `agents.entries.*.toolProgressDetail` overrides this default.
