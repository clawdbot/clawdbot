...
- **`maintenance`**: session-store cleanup + retention controls.
  ...
  - The cap only removes unprotected entries: protected ones (active work admissions, thread- and channel-scoped conversation pointers, archived sessions) are never deletion targets, they only reduce the budget left for the rest. When protected entries alone reach `maxEntries` that budget would be zero, which would delete **every** unprotected entry on every write and still leave the store over the cap. In that state the cap instead keeps up to `min(maxEntries, 25)` of the most recently updated unprotected entries and trims the rest, so count-based retention stays active and bounded rather than becoming all-or-nothing. A store whose protected population exceeds `maxEntries` therefore stays over the configured cap by design; lower the protected population or raise `maxEntries` if that matters for your deployment.
