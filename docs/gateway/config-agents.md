...
- **`maintenance`**: session-store cleanup + retention controls.
  ...
  - `highWaterBytes`: optional target after budget cleanup. Defaults to `80%` of `maxDiskBytes`. Must be positive; a value that resolves to zero or less falls back to the default. Disable the budget with `maxDiskBytes`, not with a zero high-water mark.
