...
- `compat.supportsEncryptedReasoningReplay`: set to `false` for OpenAI-compatible Responses endpoints that reject encrypted reasoning replay. OpenClaw then omits replayed reasoning items, reasoning summaries, and `reasoning.encrypted_content` from requests. Azure Responses always keeps encrypted replay enabled.
