...
- `params.chat_template_kwargs`: vLLM/OpenAI-compatible chat-template arguments merged into top-level `api: "openai-completions"` request bodies. For `vllm/nemotron-3-*` with thinking off, the bundled vLLM plugin automatically sends `enable_thinking: false` and `force_nonempty_content: true`; for self-hosted DeepSeek V4 (`vllm/deepseek-v4-pro`, `vllm/deepseek-v4-flash`) with thinking on, it sends `thinking: true` and `reasoning_effort: "high"` (or `"max"`); explicit `chat_template_kwargs` override generated defaults, and `extra_body.chat_template_kwargs` still has final precedence. Configured vLLM Qwen and Nemotron thinking models expose binary `/think` choices (`off`, `on`); vLLM DeepSeek V4 exposes a three-level ladder (`off`, `high`, `max`) instead of the multi-level effort ladder. See [vLLM advanced configuration](/providers/vllm#advanced-configuration).
