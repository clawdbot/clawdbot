...
You do not have to pick a device. Choosing **Any available node** in the Place picker — or dispatching with `autoDevice: true` — selects the paired session host with the most free worker slots, breaking ties by device ID, and retries up to three ranked hosts if provisioning fails before a machine is allocated. When no host qualifies, the error says exactly why: no session hosts paired, all disconnected, or all at capacity.
