...
<AccordionGroup>
  ...
  <Accordion title="Session lifecycle and audit">
    ...
    - For a direct heartbeat alert, OpenClaw captures the authoritative target owner, route, and current session generation immediately before platform I/O. After a confirmed send to a different target session, it binds a first-contact route when needed and records only the payload that the channel actually delivered, after outbound hooks and payload shaping.
    - A heartbeat's in-process canonical `message(action="send")` path uses the same target-session projection. It captures the target owner and generation after route resolution but before platform I/O, then starts projection at the confirmed-delivery boundary. Exact post-hook delivered payloads are mirrored into target history; caller input or an unconfirmed plugin payload is never treated as delivered transcript content. Partial delivery queues failure wording that says some content may have arrived.
    - Route refresh is compare-and-swap guarded. If an inbound message updates the conversation route while the heartbeat send is in flight, that newer inbound route wins while safe transcript and awareness updates can still land. A target reset, ownership change, or relevant hot-reloaded session configuration change during delivery fails closed: OpenClaw does not write a late route, transcript entry, or notice into the stale target generation.
    - Target-session awareness is bounded and one-time. Heartbeat runs cannot consume these notices; the next ordinary turn in the target session receives them. The queue is in memory and keeps at most 20 events per session, so a pending notice can be lost on process restart or eviction; a successfully mirrored alert remains in target history for audit.
    - Provider-native tools and explicit remote Gateway message actions send outside the local heartbeat authority, so they are not host-captured and cannot be projected into target-session history or awareness. A local plugin send that reports only an opaque accepted result retains route-only compatibility, without transcript or awareness; an explicitly suppressed send projects nothing.
    - Direct failures and uncertain outcomes queue failure or uncertainty notices instead of recording an alert as confirmed history. Queue-owned retry failures do not enqueue a notice because recovery still owns delivery.
