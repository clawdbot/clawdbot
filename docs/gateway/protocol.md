...
<AccordionGroup>
  ...
  <Accordion title="Talk and TTS">
    ...
    - `talk.client.toolCall` lets client-owned realtime transports forward provider tool calls to gateway policy. The first supported tool is `openclaw_agent_consult`; current Gateways return the run id, resolved `agentId`, and canonical `agentSessionKey`, and clients use that identity for normal chat lifecycle events and cancellation before submitting the provider-specific tool result. The two agent-identity fields remain optional in protocol v4 so native clients can decode acknowledgements from older Gateways that omit both; a current Gateway always emits both. Voice-bound high-impact actions return `VOICE_CONFIRMATION_REQUIRED:<id>` until a later finalized user utterance explicitly confirms that exact final execution action and the next consult supplies the `confirmationId`; policy or hook rewrites require confirmation again.
