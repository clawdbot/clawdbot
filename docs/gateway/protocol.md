...
<AccordionGroup>
  ...
  <Accordion title="Agent and workspace helpers">
    ...
    - `agents.create`, `agents.update`, and `agents.delete` manage agent records and workspace wiring. Legacy `agents.create` requests remain hashless. A caller may pass the opaque `config.get.hash` as `baseHash`; the Gateway rejects a stale revision before workspace setup and rechecks the raw revision under the config lock. `initialConfig` requires `baseHash` and is a closed Safe Start subset containing only `{ memory: { dreaming: { enabled: boolean } } }`. The complete entry, workspace/bootstrap state, transcript directory, identity state, and initial config are prepared before the new agent becomes visible. On conflict, call `config.get` again and retry the complete create request; do not fall back to create-then-patch.
