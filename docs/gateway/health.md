...
| `/startup`, `/startupz` | Gateway startup is complete and the Gateway is not draining. Channel and plugin sidecars are ready; downstream channel health is not consulted. | Orchestrator startup and traffic admission.                    |
...
`/startupz` returns `503` with `status: "starting"` while startup sidecars are
pending, `503` with `status: "draining"` during drain, and `200` with
`status: "started"` once the unified startup boundary is complete. In normal
Gateway mode, sidecars may begin after the core Gateway binds, but probes and
startup-gated methods remain pending until they finish. Use `/readyz` when
downstream channel health must also be healthy. A broken Telegram or other
channel account can make `/readyz` return `503` without taking a healthy
Control UI out of service through `/startupz`.
