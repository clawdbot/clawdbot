...
- Slack detects Enterprise Grid org-wide installations automatically from the
  ...
  `pairing`. Channel and user policies must use stable Slack IDs and can use
  `team:<team-id>:channel:<channel-id>` or `team:<team-id>:user:<user-id>` for
  workspace scope; mutable names and unsupported channel prefixes fail startup.
