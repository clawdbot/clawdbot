...
| `group:openclaw`   | All built-in tools above except `read`/`grep`/`find`/`ls`/`write`/`edit`/`apply_patch`/`exec`/`process`/`canvas` (excludes plugin tools)                                                                                                                 |
| `group:plugins`    | Tools owned by loaded plugins, including configured MCP servers exposed through `bundle-mcp`                                                                                                                                                             |

`grep`, `find`, and `ls` provide file discovery without granting `exec` or `process`; `tools.fs.workspaceOnly` applies the same workspace boundary used by the other filesystem tools.

On macOS and Windows host sessions, unrestricted filesystem-only discovery uses bounded in-process operations. Workspace-confined discovery is registered only when helper processes are policy-authorized; sandboxed sessions can also expose it through a backend `listDirectory` bridge. Filesystem-only local macOS and Windows sessions with `tools.fs.workspaceOnly: true` omit discovery because Node does not provide a safe descriptor-relative directory iterator on those platforms.
