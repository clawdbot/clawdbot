...
The ownership check reads `launchctl print system/<label>` and also checks installed plists under `/Library/LaunchDaemons`. It fails closed when the system-domain query or the directory scan itself cannot be completed, or when a plist whose filename contains the gateway label is unreadable or unparseable, and `--force` does not bypass it. Unreadable or unparseable third-party plists whose filenames do not contain the gateway label are skipped, and the check re-queries `launchctl print system/<label>` afterward, so a loaded same-label daemon is always refused regardless of its plist filename. The one case this cannot detect is a system daemon that is not currently loaded and whose plist is unreadable and named without the label. `openclaw gateway status` reports a loaded same-label system job; add `--deep` to scan installed system service files.
