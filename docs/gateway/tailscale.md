...
- Tailscale routes remain persistent on shutdown. `gateway.tailscale.resetOnExit: true` is no longer accepted because the Tailscale CLI cannot atomically prove that a route still belongs to the exiting Gateway; deleting it could remove a newer operator or controller route. Run `openclaw doctor --fix` to migrate the setting to `false`. Startup reclaims the managed endpoint before ordinary IPv4 loopback accepts requests.
...
- OpenClaw-managed Serve/Funnel proxy to a dedicated `[::1]:<gateway-port>` listener while ordinary local clients use `127.0.0.1:<gateway-port>`. IPv6 loopback must be available; startup fails closed rather than sharing listener provenance. The ordinary Gateway listener does not trust Tailscale-shaped headers by themselves.
