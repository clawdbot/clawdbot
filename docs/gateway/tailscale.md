...
- OpenClaw holds Serve/Funnel as a foreground Tailscale claim. Ordinary exposure failures degrade to a loopback-only Gateway (remote Serve/Funnel stays unavailable until you fix the tailnet config and restart); only Tailscale HTTPS route-ownership conflicts still fail startup, and stopping or losing the Gateway releases the claim automatically.
...
- OpenClaw-managed Serve/Funnel proxy to a dedicated `127.0.0.1:<ephemeral-port>` listener while ordinary local clients keep the configured Gateway port. Generic Serve/Funnel setup failures close the private listener (bounded teardown, never blocking local startup) and continue loopback-only; route-ownership conflicts still fail closed rather than sharing listener provenance, and the foreground claim releases the route when its Gateway owner disappears.
