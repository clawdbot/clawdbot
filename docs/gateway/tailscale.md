...
- When upgrading from an older OpenClaw release, startup automatically replaces the exact persistent HTTPS root route previously created for the configured Gateway port. OpenClaw does not change custom targets, extra paths, Tailscale Services, foreground routes, or a Serve/Funnel mode mismatch. If one of those configurations owns HTTPS port 443, startup explains the conflict and exits without entering a systemd restart loop; inspect `tailscale serve status`, resolve the route ownership manually, and restart the Gateway.
