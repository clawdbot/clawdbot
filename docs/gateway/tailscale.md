...
- OpenClaw holds Serve/Funnel as a foreground Tailscale claim. Gateway startup succeeds only after the claim is active, and stopping or losing the Gateway releases it automatically. If the claim is lost while the Gateway runs (for example, the Tailscale daemon restarts or upgrades), the Gateway re-claims the route with backoff and republishes it without a restart; only a route that another owner took in the meantime stays unclaimed until you restart the Gateway.
