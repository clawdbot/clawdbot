...
- Older releases could advertise an externally configured default HTTPS Serve route that targeted a `gateway.bind: "lan"` listener. Doctor reports matching external routes but leaves the bind address and omitted or disabled Tailscale management unchanged, including with `--fix`. To retain external ownership, use the explicit `trustedProxies` path above. To choose managed ingress, configure token, password, or trusted-proxy authentication, inspect and remove only the external route you intend to replace, then explicitly set `gateway.bind: "loopback"` and `gateway.tailscale.mode: "serve"` before restarting. Doctor does not reset Tailscale state or adopt a route based on its URL.
