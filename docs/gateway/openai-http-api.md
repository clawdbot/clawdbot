...
| `x-openclaw-message-to: <target>`               | Delivery target for later out-of-turn output such as sub-agent completion announces (with `x-openclaw-account-id` / `x-openclaw-thread-id`). Requires `x-openclaw-message-channel` naming a deliverable channel — the default synthetic `webchat` context cannot deliver. Same optional header family as [/tools/invoke](/gateway/tools-invoke-http-api); the turn's own reply is still returned only in the HTTP response. |
