...
| `x-openclaw-message-to: <target>`               | Delivery target for later out-of-turn output such as sub-agent completion announces (with `x-openclaw-account-id` / `x-openclaw-thread-id`). Same optional header family as [/tools/invoke](/gateway/tools-invoke-http-api); the turn's own reply is still returned only in the HTTP response. |
