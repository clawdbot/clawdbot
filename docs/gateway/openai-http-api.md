...
| `x-openclaw-message-to: <target>`               | Gives the run a delivery target (optionally with `x-openclaw-account-id` / `x-openclaw-thread-id`), so asynchronous out-of-turn output — such as sub-agent completion announces — can be delivered through the bound channel. Owner-level control like `x-openclaw-model`: shared-secret bearer callers can use it directly; identity-bearing callers need `operator.admin`, otherwise `403 missing scope: operator.admin`. The turn's own reply is still returned only in the HTTP response and is never pushed to a channel. |

Once accepted at ingress, the bound delivery target behaves like every other session delivery route: it is stored on the session and later out-of-turn deliveries use it as is, without re-checking the original caller's authority at delivery time — the same lifecycle as a route recorded from an inbound channel message, the WS `agent` method's `to` parameter, or the `/tools/invoke` target headers. Authority for binding a target is enforced once, at the owner-gated ingress.
