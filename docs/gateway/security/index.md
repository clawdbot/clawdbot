...
- Failed authentication from loopback is never locked out, so a local CLI cannot be denied before its credentials are checked. Wrong credentials are still tracked and progressively delayed, with a bounded delay and bounded concurrent timers; successful authentication resets the failure history.
