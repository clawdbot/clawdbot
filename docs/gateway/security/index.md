...
rejected because OpenClaw cannot attribute its forwarded client headers. HTTP
requests receive `403` with `proxy_attribution_required`; WebSocket auth fails
with guidance to configure `gateway.trustedProxies`. Configure that list
narrowly and make the proxy overwrite or safely rebuild forwarded headers;
