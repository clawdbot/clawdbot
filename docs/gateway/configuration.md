...
SecretRef details are in [Secrets Management](/gateway/secrets). `env` and `store` refs on the [effective default alias](/gateway/secrets#effective-default-alias) need no `secrets.providers` entry; custom aliases must be declared there.
