...
<AccordionGroup>
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - Required migrations refuse unavailable service inspection and unmatched services that may still run. After their owner stops them and the native manager confirms they are offline, Doctor repairs its selected state without changing or starting those services. A disabled systemd unit can still be restarting; Doctor checks runtime state as well as installation state.
