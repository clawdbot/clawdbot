...
<AccordionGroup>
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - On Linux, doctor does not rewrite command/entrypoint metadata while the matching systemd gateway unit is active. If a stopped unit's command or working directory is overridden by an operator-owned systemd drop-in, inspect it with `systemctl --user cat <unit>.service`, then update or remove the drop-in; rewriting the managed base cannot change the effective launcher. `Environment=` drop-ins remain supported, but Doctor also refuses repairs when an overridden variable owns the reported problem. For example, a drop-in that replaces `PATH` with an incomplete value must be corrected there; changing the base unit's `PATH` would have no effect. Unrelated environment overrides do not block repair. Doctor also ignores inactive non-legacy extra gateway-like units during the duplicate-service scan so companion service files do not create cleanup noise.
