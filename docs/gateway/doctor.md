...
<AccordionGroup>
  ...
  <Accordion title="Config and migrations">
    ...
    - Reports external HTTPS Tailscale Serve routes without enabling managed ingress or changing the Gateway bind address. Route ownership remains an explicit operator choice. Retired named-Service config is removed with managed ingress disabled until the operator chooses a device route.
