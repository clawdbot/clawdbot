...
<AccordionGroup>
  ...
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    - Signed device identity: from `~/.openclaw/identity/device.json` into the `primary` `device_identities` row in `state/openclaw.sqlite`; Gateway startup also performs this verified import for valid legacy identities, while Doctor retains repair authority for invalid canonical rows; the separate device-auth file is left untouched. A pending native `.native-importing` claim is owned by the Mac app: Doctor does not consume it. If the app reports conflicting preserved identities, finish import or reconcile from the Mac menu instead of restarting into the same conflict.
