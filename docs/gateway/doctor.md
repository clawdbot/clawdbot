...
<AccordionGroup>
  ...
  <Accordion title="2. Legacy config key migrations">
    ...
    For legacy agent rosters without `agents.defaults.systemAgent.agentId`, Doctor selects the agent marked `default: true`, or `main` when present. This restores ownership for ambient operations and agent-less cron jobs after an update. Doctor also pins `agents.defaults.heartbeat.agentId` when heartbeat enrollment would otherwise be unresolved; existing heartbeat owners, shared defaults, and per-agent enrollment are preserved. These changes are reported and saved by `doctor --fix`, including the update-time doctor pass. If no default can be identified, configure the system-agent owner explicitly.
