...
Note that CLI dotenv resolution operates in two distinct stages when `OPENCLAW_STATE_DIR` is unset and `OPENCLAW_CONFIG_PATH` points outside `~/.openclaw`. First, normal CLI startup (`loadCliDotEnv` in `src/cli/dotenv.ts`) explicitly loads the state-directory dotenv file (`~/.openclaw/.env`) into `process.env`. Second, config snapshot initialization (`maybeLoadDotEnvForConfig`) invokes `loadDotEnv`, resolving `resolveConfigDir` to the external config directory and loading the config-adjacent `.env` without overriding existing environment keys. Consequently, if a credential variable is defined in both files, the state-directory assignment from the first stage takes precedence over the config-adjacent file. Gateway pre-bootstrap similarly loads both files in sequence without overwriting existing keys. To ensure predictable credential resolution across CLI commands (like `openclaw secrets audit`) and Gateway runtime, operators should explicitly configure `OPENCLAW_STATE_DIR` or maintain the backing assignment in the state directory's `.env`.
