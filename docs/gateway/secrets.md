...
Note that `resolveConfigDir` prioritizes `OPENCLAW_STATE_DIR` over `OPENCLAW_CONFIG_PATH`. When `OPENCLAW_STATE_DIR` is set, both CLI and gateway pre-bootstrap deduplicate paths to the state directory and only load the state-directory `.env`. When `OPENCLAW_STATE_DIR` is unset while `OPENCLAW_CONFIG_PATH` points outside `~/.openclaw`, CLI commands resolve `stateEnvPath` to the config directory's `.env` via `resolveConfigDir`, while gateway pre-bootstrap loads both the default state `.env` (`~/.openclaw/.env`) and the config-adjacent `.env`. To ensure consistent credential resolution across both CLI commands (like `openclaw secrets audit`) and Gateway runtime, ensure `OPENCLAW_STATE_DIR` is explicitly configured or maintain the backing assignment in the dotenv file selected by `resolveConfigDir`.
