...
- Unsupported files saved on local disk get a self-serve marker naming the on-disk path and instructing the agent to extract the content with its own tools (modern Office files additionally get an unzip hint) instead of asking the sender to paste it — including files whose format could not be identified, since the agent can inspect the bytes itself. URL-only attachments and sandboxed sessions keep the plain `[Unsupported document format: <mime>. PDF and plain-text attachments can be read.]` marker.
