...
This fulfiller currently controls only the primary display. `hold_key`, `left_mouse_down`, and `left_mouse_up` are unavailable because the CUA Driver SDK has no desktop-scope held-input contract. Modifier-held clicks are supported on X11 (native Wayland returns a structured refusal). A modifier-held drag is accepted and the modifier is forwarded to the driver's typed `drag` contract, but with the pinned `@trycua/cua-driver` 0.22.2 the held modifier was not observed at the X11 target during the drag, so treat a modified drag as driver-dependent and currently not effective on X11. Modifier-held scrolling is rejected because the CUA Driver `scroll` tool has no modifier argument. The `key` action accepts named keys, letters, and modifier combos (for example `cmd+c` or `Return`); digit and punctuation keys are rejected because the driver drops their layout-dependent shift state, so send that text through the `type` action instead. Cancellation is passed to the SDK for each node invocation.

Modifier-held drag is validated on the **node**, not just the agent-facing schema: `computer.act` params are parsed against the node's own installed contract. A Gateway/agent build new enough to send a drag `modifiers` field against an older, not-yet-upgraded node host rejects the request with a structured `COMPUTER_INVALID_REQUEST` refusal instead of the previous behavior of silently dropping the modifier and performing an unmodified drag. Upgrade the node host so the request is accepted again; whether the modifier takes effect on the target then depends on the driver (see above). An unmodified drag is unaffected on either version.
...
| `COMPUTER_UNSUPPORTED_ACTION`                        | An action this fulfiller cannot faithfully deliver: `hold_key`, `left_mouse_down`, `left_mouse_up`, or modifier-held scroll.                                  | Use a supported action. The CUA Driver `scroll` tool has no modifier argument, and neither `mouse_button_down/up` nor keyboard hold has a desktop-scope contract.                                        |
