...
| `COMPUTER_UNSUPPORTED_ACTION`                        | An action this fulfiller cannot faithfully deliver: `hold_key`, `left_mouse_down`, `left_mouse_up`, or modifier-held drag/scroll.                             | Use a supported action. The CUA Driver does not hold a drag modifier on the target, its `scroll` tool has no modifier argument, and neither `mouse_button_down/up` nor keyboard hold has a desktop-scope contract. |
