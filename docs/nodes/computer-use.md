...
The built-in `computer` tool takes one action per call. Coordinates are non-negative integer pixels in the most recent screenshot image (the `WxH` printed with the screenshot result), not the native desktop resolution; the node maps them to display points. When the node reports logical display size, the screenshot caption also includes `display WxH pts` so models can tell the downscaled image apart from the live desktop. On multi-monitor macOS nodes, the same caption includes a `screens:` inventory (`index=WxH pts`, with `(main)` on the primary display) in the same order as `screenIndex`; pass `screenIndex` on the next screenshot to switch displays. Default `screenIndex` is `0` when omitted and no prior frame is bound. Coordinate actions must echo the screenshot result's `frameId`, and an explicit `screenIndex` must match that frame. OpenClaw also carries a node-issued display identity from the screenshot into the action, so a display reconnect or geometry change fails closed instead of silently retargeting the same index. These checks reject guessed tokens and tokens from another delivered frame or display. A token is not a freshness guarantee: apps can change pixels on the same display after capture, so take a new screenshot whenever the scene may have changed.
