...
The compact Node planner keeps separate Blacksmith and standard 4-core hosted timing ownership. The `github` and `hybrid` profiles admit 150 predicted seconds per job, including shared runtime preparation, without changing file partitions, process envelopes, workers or runner requirements. GitHub applies a 1.6x median scaling fallback only to unmeasured groups. Hybrid splits groups using the slower of the first-attempt Blacksmith estimate and the hosted retry estimate against the unchanged 150-second ceiling. This prevents a faster retry estimate from leaving a slow first attempt indivisible. Its attempt-1 packing applies the existing 0.87 scale to Blacksmith estimates, using committed measurements before the cold-start hints. These calibrated predictions remain separate from the recorded wrapper wall times. Refits can change the number and composition of compact jobs without changing runner policy. Direct sampled hints cover the doctor and cron-service outliers. In hybrid, the unmeasured `agentic-gateway-core-3` tail retains its 140-second fallback and can share only the remaining ten predicted seconds.

Ordinary non-Windows CI targets seven minutes; Windows must still pass but sits outside this latency objective. The 150-second packing budget is an estimate, not a job timeout or a measured workflow result. Preflight, checkout/setup, queueing and actual test walls all count.
...
Expanded large/small jobs admit 150 predicted seconds and retain the 96-row hybrid and 112-row GitHub compact caps. Plugin fallback contributes at most 78 rows; eligible pairs reduce the emitted count. Exclusive-group admission remains 150 seconds. The PR-only performance lifecycle file uses a 136-second fallback, rounded up from the larger native shard span of 127.288/135.808 seconds in runs 33532741896/33545657559. Canonical main pushes omit that tooling family. Trusted contributor forks select the GitHub planner profile but can still run those rows on Blacksmith, so their unchanged 112-row cap owns the conservative PR bound. The current workflow has 86 other potential job rows across backends: 13 nonmatrix rows and 73 matrix rows, including both Swift and both iOS build phases. Historical targets without the UI named-project contract can still use one extra UI row, giving a maximum of 87. Excluding the four unconditionally hosted iOS build/capture rows leaves 82 potentially eligible rows, or 83 at the historical width. Retain the conservative allowance of 84 Blacksmith admissions, including other hosted and skipped rows; the saved capacity remains unspent. This gives a broad-PR bound of `112 + 78 + 84 = 274`; canonical main uses the 96-row hybrid cap without PR fallback descriptors, giving a bound of 180. The command splits fit inside those caps, so their rows are not added again. Two active main slots plus both pending successors and the observed peak of nineteen PR arrivals, including superseded heads, give `4 × 180 + 19 × 274 = 5,926` registrations in five minutes. The live bucket rechecked on 2026-09-02 was 10,000; this leaves 74 below the 6,000 operating target before additional release and adjacent-repository arrivals. Npm qualification adds three registrations per arrival compared with its former serial layout; budget all six current npm jobs per qualification inside five minutes. Further fanout needs reductions elsewhere or a narrower, evidenced arrival bound. This is an arrival-envelope estimate, not an organization-wide admission cap: re-evaluate emitted rows, adjacent repositories and queued-run overlap before increasing fanout. An unused registration bucket does not prove physical runner capacity.
