...
- **zsh shell execution** changes to shell argument selection or shell snapshots select the existing `macos-node` lane. It runs both shell suites with real zsh, including literal exec arguments and native explicit-session semantics, without selecting Swift or iOS builds.
