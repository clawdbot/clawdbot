...
`checks-ui-e2e` emits 14 rows on every backend: 11 bundled-project shards with up to two workers, two single-worker serial-project shards, and one browser-extension row. The measured 2026-09-01 Control UI inventory contained 319 bundled files and 20 serial files. All five real-Gateway files join the serial project when selected by a full or local run. The serial owner protects private source servers that share a Vite optimizer cache and the runtime-budget measurement. Named projects share one root-owned production-bundle setup and retain the existing weighted sequencer, test cases, and deadlines. Default Blacksmith runs now use all fourteen rows already charged by the conservative registration bound; GitHub and hybrid row counts stay unchanged.
