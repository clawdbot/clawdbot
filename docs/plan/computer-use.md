...
| W5-REF     | Converge both providers' opaque-ref lifecycles on the contract, with a shared case fixture so drift fails CI                                                                        | openclaw              | pr      | #124374 round 2: Peekaboo keyed refs on `mutationIdentity` (which includes bounds + minimized), so a moved window silently got a new ref while CUA keeps one keyed on `(pid, windowId)`; refs also leaked per generation                                                                                                                                                                                                                |
