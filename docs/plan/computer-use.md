...
- CUA 0.19.3: 49 MCP tools; background delivery per-call with typed refusals
  ...
  landed upstream (#2410 closed); pin the first release containing it.
...
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ------ | -------------------------------- |
| W0-FIX     | Parity matrix + pinned fixtures: 49 CUA tools + 25 Peekaboo tools classified against the v2 union; recorded tools/list + result fixtures           | openclaw              | todo   | RFC CU-1 + Peekaboo column (new) |
| W0-PIN     | Pick + pin CUA release with embedded/inherited-IPC support; dependency bump (needs Dependency Guard approval)                                      | openclaw              | todo   | verify which release has #2410   |
| W1-PROTO   | v2 types: action union (+`invoke_menu`), target union, capability descriptor, result envelope, closed error codes; **replaces** v1 params in place | openclaw              | todo   | RFC OC-1 minus compat lane       |
| W1-TOOL    | Built-in computer tool v2: capability-filtered schema, observation refs, result projection, no auto-retry                                          | openclaw              | todo   | RFC OC-2; token budget measured  |
| W1-SEAM    | Node-host provider seam: one registration, provider selection, generation, lifecycle close path; absorbs cua-computer's direct registration        | openclaw              | todo   | RFC OC-3; LOC-negative target    |
| W2-CUA     | CUA plugin refactor onto seam: full v2 mapping (window/element/background), session-per-execution, deletes desktop-scope-only adapter              | openclaw              | todo   | RFC OC-4                         |
| W2-MAC     | macOS app: bundle + re-sign driver, direct `serve --embedded` spawn, private socket handoff to worker, TCC restart handling                        | openclaw              | todo   | RFC OC-5                         |
...
| W2-UX      | Settings -> Computer Use provider picker + readiness checklist (both apps: macOS now, Tauri Linux later)                                           | openclaw              | todo   | RFC OC-10B slice; video proof    |
