...
- Worker exec is full-permission within the box when the Gateway-issued turn authority includes it. The worker box replaces an explicitly unconditional sandbox route, but the Gateway omits worker exec when the resolved source policy is not unconditional or targets a paired node. The worker does not reproduce source allowlists, approvals, or node authority. The box is disposable and credential-free, so no second per-command approval runs inside it; the guarded boundaries are Gateway admission, inbound reconciliation, and audit. Exec never traverses the gateway node-approval path.
