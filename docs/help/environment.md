...
Docker Compose is a separate layer with its own rules. It interpolates `${VAR}` inside `docker-compose.yml`, including the `volumes:` entries, and it also expands values in an `.env` file loaded through `env_file:`. Compose draws those substitutions from the shell environment, the project `.env`, and earlier lines in the same `env_file`, which it reads top to bottom. A line cannot reference one declared below it. Only OpenClaw's own dotenv loader leaves them literal. The bundled `docker-compose.yml` then pins the container-side paths in its `environment:` block, so `OPENCLAW_WORKSPACE_DIR` from the project `.env` picks which host directory gets bind-mounted rather than the path OpenClaw resolves inside the container. That one stays `/home/node/.openclaw/workspace`.
