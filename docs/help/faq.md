...
The wizard generates a gateway token by default (even on loopback) so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway. Paste the token into **Overview → Gateway Access** (or your client config) to connect.
...
- In **Overview → Gateway Access**, paste the same token (or refresh with a one-time `?token=...` link).
