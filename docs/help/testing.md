...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - `pnpm check:changed` is the normal smart local check gate for narrow work. It classifies the diff into core, core tests, extensions, extension tests, apps, docs, release metadata, live Docker tooling, and tooling, then runs the matching typecheck, lint, and guard commands. Native `tsgo` and `tsgolint` targets can reuse exact changed-check evidence receipts when the current worktree is clean and the repo, changed paths, lane plan, effective argv, env, wrapper/helper inputs, toolchain, and config closure match. If the exact receipt is absent, they can also reuse ancestor proof when the producer commit is an ancestor, the worktree is clean, and the descendant delta classifies to unaffected lanes such as docs-only or release metadata only; stale earlier receipts do not stop the bounded scan from accepting a valid later receipt. Affected, global, planner, wrapper, config, package/lock, malformed, skipped, dirty, non-ancestor, or unresolved cases rerun. `--dry-run` prints the reuse or no-reuse reason. It does not run Vitest tests; call `pnpm test:changed` or explicit `pnpm test <target>` for test proof. Release metadata-only version bumps run targeted version/config/root-dependency checks, with a guard that rejects package changes outside the top-level version field.
