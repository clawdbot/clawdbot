...
**Claim:** `dmScope` precedence and identity links behave deterministically: the default `main` scope shares one rolling session across a single owner's DMs (the personal-agent default), while any configured isolating scope (`per-peer`, `per-channel-peer`, `per-account-channel-peer`) keeps DM sessions separated by the identity that scope keys on. `per-peer` is channel-agnostic and therefore matches peer ids case-insensitively, so peers whose ids differ only by case share one session under it; `per-channel-peer` and `per-account-channel-peer` keep case-sensitive ids distinct (see [Session](/concepts/session)). Channel-specific `dmScope` overrides win over global defaults; `identityLinks` collapse sessions only within explicit linked groups, not across unrelated peers. Multi-user inboxes are expected to opt into an isolating scope (the runtime security audit recommends this when it detects multi-user DM traffic).
