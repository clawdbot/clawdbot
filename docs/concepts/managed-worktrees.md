...
Archiving a session snapshots and removes its managed checkout while preserving the conversation and worktree binding. Repeating archive also cleans an already-archived session's remaining checkout. If manual cleanup cannot finish safely, the error stays visible and the archive flag is not changed. Automatic session archival commits its metadata first, then attempts safe cleanup; startup and hourly garbage collection retry any checkout that remains.
