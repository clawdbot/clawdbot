...
Sub-agents receive `sessions_spawn`, `subagents`, `sessions_list`, and `sessions_history` so they can manage their own children. Set a finite `maxSpawnDepth` to turn sessions at that depth into leaves without recursive orchestration tools.
