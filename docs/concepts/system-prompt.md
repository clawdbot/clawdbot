...
On the native Codex harness, OpenClaw avoids repeating stable workspace files in every user turn. Codex loads the execution folder's `AGENTS.md`, including its `## Tools` section, through native project-doc discovery, so OpenClaw does not inject that file again. When execution uses another folder, OpenClaw adds the configured agent workspace's bounded `AGENTS.md` snapshot to the thread-level developer instructions so native Codex sub-agents inherit it. The eligible skills list, `SOUL.md`, `IDENTITY.md`, `USER.md`, and workspace-memory guidance follow the generic policy on the same thread developer carrier, in that order. Model-owned collaboration instructions therefore cannot replace them. Native Codex sub-agents intentionally inherit the persona and memory guidance, with no extra isolation layer. `MEMORY.md` content is not pasted into every native Codex turn when memory tools are available for the agent workspace; the carrier instead includes a small workspace-memory note directing the model to `memory_search` or `memory_get`. If tools are disabled or memory search is unavailable, `MEMORY.md` follows the normal bounded turn-context path. `BOOTSTRAP.md` keeps the normal turn-context role.
