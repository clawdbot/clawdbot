...
On the native Codex harness, OpenClaw avoids repeating stable workspace files in every user turn. When the configured agent workspace is also the execution folder, Codex loads the root-to-working-directory `AGENTS.md` hierarchy through native project-doc discovery; OpenClaw does not add a second same-workspace copy on the initial thread start. Snapshot-enabled host-local threads explicitly select Codex's local environment so user-configured remote defaults cannot substitute same-path instruction sources. Before that request, OpenClaw snapshots the bounded default and thread-configured candidate filenames along each host-local selected environment's project path. Once Codex reports the exact selected sources, OpenClaw verifies those identities and freezes that bounded hierarchy in the thread binding. Warm threads keep Codex's already-loaded authority, while a physical cold resume or replacement replays the frozen snapshot with native rediscovery disabled so a later file change cannot remove nested policy. Experimental sandbox exec-server threads are stricter because Codex reads project documents inside the selected environment and reports paths without their authoritative bytes: OpenClaw permits warm reuse while that environment-backed thread remains live, but rejects every physical cold resume or replacement, including one under the same environment fingerprint. When execution uses another folder or ordinary policy restrictions make native discovery unavailable from the outset, OpenClaw carries the configured workspace's bounded root `AGENTS.md` snapshot in thread-level developer instructions instead. `SOUL.md`, `IDENTITY.md`, and `USER.md` remain turn-scoped collaboration developer instructions and intentionally do not flow to native sub-agents. `MEMORY.md` content is not pasted into every native Codex turn either: when memory tools are available for the agent workspace, Codex turns get a small workspace-memory note directing the model to `memory_search` or `memory_get`. If tools are disabled or memory search is unavailable, `MEMORY.md` falls back to the normal bounded turn-context path. `BOOTSTRAP.md` keeps the normal turn-context role.
