...
For a single-chunk summary on the same Anthropic Messages or OpenAI Responses model and route, OpenClaw can reuse the foreground system prompt, tool definitions, and unchanged native history prefix. Reuse requires matching request fingerprints still held by the current process; changed history, split turns, and multi-chunk summaries use serialization. Native replay may include image content. Summarization never executes foreground tools, and a rejected request, invalid summary, or tool-call response falls back to serialized summarization. A prefix response must start with the expected summary heading and include all required sections in order; conversational preambles trigger serialization. Both requests count toward usage when fallback is needed. Embedded compaction diagnostics report each completed request's path and a serialization reason, such as changed history (with message index and role), a changed model route, or a rejected prefix request. Summary formatting and identifier preservation remain the same.

Built-in serialized summarization receives text, not image pixels. Omitted images and other non-text input receive markers such as `[image data omitted from summary input]`, without claiming that a model processed the data. The first eight affected messages receive at most two fixed markers each; further omissions receive one aggregate statement. These additions, including newly retained role labels and separators, total at most 847 UTF-8 bytes per summarizer request and count toward token estimates. Existing text is not capped by this omission budget. Custom compaction providers still receive the original message content.
