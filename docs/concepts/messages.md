...
Feishu and iMessage back this pending window with the host-owned plugin-state SQLite store. Their windows survive gateway restarts, retain bounded local image and document references for up to 24 hours, and consume only the exact snapshot used by a turn with authoritative evidence of visible delivery. A silent, failed, cancelled, or otherwise zero-visible turn leaves that snapshot available for recovery, while messages that arrive during the run remain pending behind its high-water mark. Other channels can continue using the in-memory compatibility window until they adopt the same persistence seam.

Managed pending media is a recovery cache, not an unbounded archive. Across all channels and accounts, the shared `channel-history` media subtree is capped at 512 MiB and 4096 regular files. Cleanup applies the 24-hour TTL first, then evicts the oldest remaining files by stable modification-time and path order until both caps are satisfied. Text journal entries remain when a referenced file expires or is evicted; prompt assembly skips the missing attachment and preserves the surrounding text. These fixed limits bound disk use and privacy exposure without turning mention gating into a full-chat transcript service.
