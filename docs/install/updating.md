...
canary boot on copied state finish before the service stops. The offline window
contains the swap, required migrations, and service start. The final report
records downtime and verification results. See
...
Backups retained by an update whose status is still `running` are protected
from cleanup until verification or rollback finishes.
