...
<AccordionGroup>
  <Accordion title="Chat and Talk">
    ...
    - Expanding a Terminal, Browser, or Desktop side panel keeps the selected session's composer above the surface, so drafts, attachments, model controls, and Send/Stop behavior keep working without leaving the panel. Active tool and run status stay in the composer's surface header; on phones the overlay stays compact until focused, then reveals its footer controls.
