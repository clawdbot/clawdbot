...
- Browser avatar URLs include an opaque `v` revision. These URLs serve a cached preview of static raster images with a maximum side of 128 pixels; GIF, animated PNG/WebP, and SVG avatars retain their animation or vector form. The sidebar and chat panes share fetched images, and private browser caching avoids downloading unchanged bytes on reload.
