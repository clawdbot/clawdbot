...
<AccordionGroup>
  ...
  <Accordion title="Channels, sessions, memory">
    ...
    - Memory (a tab on the Agents page, scoped to the selected agent): effective Dreaming status, a per-agent participation toggle (`agents.entries.<id>.memory.dreaming.enabled`), and the Dream Diary reader (`doctor.memory.status`, `doctor.memory.dreamDiary`, `config.patch`). The global Memory settings page and `/dreaming on|off` continue to own the global master switch. When the `memory-wiki` plugin is enabled, the Diary view adds **Imported Insights** and **Memory Wiki** sub-tabs that browse imported source chats and the compiled wiki — clustered synthesis, entity, and concept pages plus annotated sources and reports, with claims, open questions, contradictions, and inline page previews (`wiki.importInsights`, `wiki.overview`, `wiki.get`).
