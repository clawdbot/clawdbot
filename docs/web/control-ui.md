...
<AccordionGroup>
  ...
  <Accordion title="Automations panel notes">
    ...
    - Set `cron.webhookToken` to send a dedicated bearer token, and list your receiver hostnames in `cron.webhookTokenHosts`; without the token no auth header is sent, and with it the header reaches only listed hosts, so a job-supplied destination that is not listed is sent unauthenticated. A `cron.failureAlert` webhook destination is declared in config rather than on a job, so its host is trusted automatically and needs no entry.
