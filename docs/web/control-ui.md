...
Use **Color vision** to adjust semantic status colors without changing the selected built-in theme family, light/dark mode, or accent. Choose **Protanopia (red-blind)**, **Deuteranopia (green-blind)**, or **Tritanopia (blue-yellow)**, or keep **Standard** for the theme's original palette. Imported tweakcn themes keep their own colors, so the selector is unavailable until you switch to Claw, Knot, or Dash. Status text and icons remain the authoritative cues; the palette is additional differentiation, not a color-only signal.
