...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - The **Files** tab in each Chat pane's unified side panel lists thread files, project files, and artifacts. Reopen it with ⇧⌘B, **Panels → Show session files** in the chat header menu, or the panel's **+** menu; the menu action carries a changed-file count badge. The shortcut reveals a hidden Files tab, switching from Dashboard to Split when needed; using it while Files is visible closes the tab.
