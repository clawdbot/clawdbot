...
**Settings → Profile → Model accounts** shows the selected **Gateway**, saved **Person**, and **Scope: Personal** before the **Sign in** controls. These are the same personal accounts managed by `openclaw models accounts login <provider>`, not the machine-local system/agent credentials managed by `models auth`. If the Gateway connection has no verified person, the section explains the missing identity and offers **Connection settings** without showing provider credential inputs. A shared Gateway token or device pairing does not establish personal identity. See [Per-person model accounts](/concepts/multi-user#per-person-model-accounts).
