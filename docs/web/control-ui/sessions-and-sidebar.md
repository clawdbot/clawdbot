...
Enable **Show all agents in the sidebar** in the agent switcher to replace the single-agent chip with an **Agents** section. It is off by default, and the browser remembers your choice. Rows show an avatar and status dot, the agent's name, **Working:** with a preview or **Active** with a relative time, and an unread count when available. Working agents appear first, followed by the most recently active; with more than six agents, the section shows six rows and **See all N agents** opens `/agents`. **New agent** opens the same creation flow as the switcher. Selecting a row switches the active agent and keeps the sessions below scoped to that agent. Use the section header's switch control to reopen the menu and turn roster mode off. Activity uses the same 300-recent-session window as [Agents home](/web/control-ui#agents-home).
