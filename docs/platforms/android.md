...
- Offline history: cached transcripts update in the order live histories are accepted, so a delayed reconnect health check cannot restore an older snapshot. Switching sessions preserves queued cache updates for the session you left. The cache records the session instance as well as its Gateway, agent, and key: a delayed deletion for an older instance cannot erase a replacement transcript. Cache identities do not enable live session actions or bookmark restoration before history refresh. A matching deletion removes that cached instance, but does not erase drafts or queued input whose instance cannot be established.
- Local cache upgrades preserve existing offline history; older unidentified rows gain an instance ID from a live response. Downgrading to an app using cache schema v3 rebuilds disposable chat history, but leaves queued messages and reader bookmarks in the separate client-state database intact.
