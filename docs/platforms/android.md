...
- Reading position: reopening the same session restores the last stable message and offset after live history identifies that session. Bookmarks use existing client-state metadata without changing either database schema; missing anchors use the usual initial position. **Jump to latest** returns to live content.
