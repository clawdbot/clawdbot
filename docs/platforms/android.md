...
- Reading position: if you leave a chat following the live edge, reopening shows the current latest output, including messages received while away. If you read older content, reopening restores that message and offset after live history identifies the same session. **Jump to latest** clears the bookmark and resumes live following. Bookmarks use existing client-state metadata without changing either database schema; missing anchors use the usual initial position.
