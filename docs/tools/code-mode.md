...
- `codeModeStats`: detailed host-side accounting for Code Mode:
  ...
  attempts/dispositions/bytes/serialization time, and control-call result
  `outcomes`. Snapshot conservation is
  `accepted + rejected + incomplete = attempted`; `produced` counts attempts
  with measured bytes and serialization time. Snapshot `coverage` applies only
  to those measurements: `"lower_bound"` means at least one started attempt
  ended before measurement metadata reached the host, while `"exact"` does not
  turn an `incomplete` terminal disposition into a completed attempt.
