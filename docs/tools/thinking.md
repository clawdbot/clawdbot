...
- On Codex harness turns for `openai/*` models, the shared runtime control supersedes a configured native app-server tier: Fast on sends `priority`, Fast off sends `null` to clear the OpenClaw-owned tier, and auto decides for each model call. Native app-server turns receive the tier only on `turn/start` or thread start/resume, so auto cannot retier an already-running turn; it applies to the next model turn OpenClaw starts. A configured Codex tier is used only when no shared Fast-mode run control is supplied. Legacy `openai-codex/*` refs are migration inputs for `openclaw doctor --fix`, not default live routes. See [Codex harness](/plugins/codex-harness#shared-fast-mode-and-codex-fast-mode).
