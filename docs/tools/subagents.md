...
<AccordionGroup>
  ...
  <Accordion title="Completion delivery">
    ...
    - API-driven requester sessions (`POST /v1/responses`) have no inbound channel message to record a target from; pass `x-openclaw-message-to` (see the [OpenResponses HTTP API](/gateway/openresponses-http-api)) so direct completion delivery can engage.
