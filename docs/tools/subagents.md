...
<AccordionGroup>
  ...
  <Accordion title="Completion delivery">
    ...
    - API-driven requester sessions (`POST /v1/responses`, `POST /v1/chat/completions`) have no inbound channel message to record a route from; pass `x-openclaw-message-channel` (a deliverable channel) together with `x-openclaw-message-to` (see [OpenAI chat completions](/gateway/openai-http-api)) so direct completion delivery can engage.
