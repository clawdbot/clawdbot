...
<AccordionGroup>
  ...
  <Accordion title="Completion delivery">
    ...
    - Missing or empty external delivery receipts remain unconfirmed and follow that bounded retry policy. Empty requester output still uses the existing completion fallback; it is not an outbound-hook cancellation. A confirmed message-tool send to the requester still counts as delivery.
