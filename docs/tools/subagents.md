...
<AccordionGroup>
  ...
  <Accordion title="Completion delivery">
    ...
    - Missing or empty external delivery receipts remain unconfirmed and follow that bounded retry policy. A send without a platform message identity remains ambiguous, never intentional suppression. Empty requester output still uses the existing completion fallback; it is not an outbound-hook cancellation. A confirmed message-tool send to the requester still counts as delivery.
