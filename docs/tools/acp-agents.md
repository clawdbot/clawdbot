...
| `PermissionPromptUnavailableError: Permission prompt unavailable in non-interactive mode` | Default `nonInteractivePermissions=fail` aborts when ACP requests write/exec permission.                               | Set `nonInteractivePermissions=plugin`, enable `approvals.plugin`, and restart the Gateway. See [Permission configuration](/tools/acp-agents-setup#permission-configuration).                            |
