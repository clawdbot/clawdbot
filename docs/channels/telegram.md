...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ...
    <Note>
      Selected quote replies are the exception for `partial` and `block` modes. When `replyToMode` is `first`, `all`, or `batched` and the inbound message has selected quote text, OpenClaw sends the final answer through Telegram's native quote-reply path and skips answer draft previews for that turn. In `progress` mode, transient progress drafts remain visible and the final answer still uses the native quote reply. Current-message replies without selected quote text stream normally. To keep native quote replies and hide tool-progress lines, use `streaming.progress.toolProgress: false` in `progress` mode or `streaming.preview.toolProgress: false` in `partial` and `block` modes.
    </Note>
