...
- If Slack expires an established native stream, subsequent progress and text continue by updating the same message. This recovery applies only to an explicit `message_not_in_streaming_state` rejection with a known message timestamp and preserves user Stop requests. Recovered task details and outputs retain their text without interpreting embedded Markdown. If the complete message cannot fit Slack's block limits, definitely undelivered text uses normal chunked delivery instead.
