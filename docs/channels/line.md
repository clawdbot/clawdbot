...
- `channels.line.contextVisibility` (also per account, or globally through `channels.defaults.contextVisibility`) decides whether a quoted message's author and text reach the agent; see [Groups](/channels/groups).
- A group reply made with LINE's quote gesture carries the quoted message's author, and its text where the message had text, into the turn, so the agent can answer "what about this?" without guessing. LINE's webhook reports only the quoted message's ID, so the text is resolved from what this conversation already admitted: the most recent few hundred messages per account, kept in memory and scoped to the conversation they arrived in. That store is larger than the chat window the agent sees, so a quote can resolve to a message that has already scrolled out of it. A quote of anything older, of a message from another chat, of a group message that only ever sat in the ambient window while that window is switched off (`historyLimit: 0`), or of anything at all after a Gateway restart reaches the agent as a bare reference with no author and no body under the default `contextVisibility: "all"` and under `"allowlist_quote"`; under `"allowlist"`, in a group that actually has an allowlist to check, an unresolved quote has no author to clear, so the reference is dropped with it. A quote of one of the bot's own messages carries its author but no body: the store holds no outbound text, so the quote is named as the assistant's own and the transcript is what carries the rest of that turn. That author is also what tells it apart from a quote this account can no longer resolve, which arrives with no author at all. A quote of a photo, video, file, or sticker comes back as the same short marker the chat window shows for it — `<image>`, or the sticker's description — not as the attachment itself; only text is retained. A long quoted message keeps its head and tail with `…[omitted]…` between them.
- The `"allowlist"` and `"allowlist_quote"` modes only narrow anything in a group whose effective `groupPolicy` is `allowlist` — either set as such, or forced there by giving that group its own `allowFrom`; in a DM, or in a group that is open, there is no allowlist to check and every quote is kept. Under `contextVisibility: "allowlist"` in such a group, the quoted author must still pass the group allowlist as it is configured now, not as it was when the message arrived, including through an `accessGroup:<name>` entry, which is expanded for the quoted author the same way it is for the sender asking; `"allowlist_quote"` keeps an explicitly quoted message from a sender who has since been removed. Neither mode can recover a message from a sender who was never admitted: nothing was stored for them, so `"allowlist_quote"` has only a bare reference to keep and `"allowlist"` drops the quote entirely.
