...
- `requireMention` covers every message a group carries, attachments and stickers included. LINE puts mention data only on text, so a photo, video, file or sticker never addresses the bot on its own; an unaddressed one is kept as room context instead of answered. A kept photo is still available to the mention that follows it; other attachment kinds are recorded by what they are, not by their contents.
