...
- Under `contextVisibility: "allowlist"`, the quoted author must still pass the group allowlist as it is configured now, not as it was when the message arrived, and must be named there by ID: an `accessGroup:<name>` entry is not expanded for this check, so an author admitted only through an access group counts as no longer named; `"allowlist_quote"` keeps an explicitly quoted message from a sender who has since been removed. Neither mode can recover a message from a sender who was never admitted: nothing was stored for them, so `"allowlist_quote"` has only a bare reference to keep and `"allowlist"` drops the quote entirely.
