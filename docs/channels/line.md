...
- `requireMention` covers every message a group carries, attachments and stickers included. LINE puts mention data only on text, so a photo, video, audio message, file, sticker or location never addresses the bot on its own; an unaddressed one is kept as room context instead of answered. A kept photo is still available to the mention that follows it — that one mention, after which it is consumed — and only while the Gateway keeps running, since the kept window lives in the process. Other kinds are recorded by what they are, not by their contents, and a photo the bot could not fetch says so in the room context instead of reading as one it can still see.
- Upgrade check: earlier releases answered an unaddressed attachment in a `requireMention` group, because the mention requirement only ever applied to text. It now applies to every kind, which also means a video, audio message or file cannot reach the agent in such a group at all: LINE carries no mention or quote on those kinds and does not allow a caption alongside them, and only photos are kept for a following mention. Groups that receive documents or voice notes for the bot need `channels.line.groups.<groupId>.requireMention: false`, which answers every message in that group.
