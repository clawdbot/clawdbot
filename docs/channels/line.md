...
- A group reply made with LINE's quote gesture carries the quoted message's author and text into the turn, so the agent can answer "what about this?" without guessing. LINE's webhook reports only the quoted message's ID, so the text is resolved from what this account already showed the agent: the most recent few hundred messages per account, kept in memory and scoped to the conversation they arrived in. A quote of anything older, of a message from another chat, or of anything at all after a Gateway restart reaches the agent as a bare reference with no author and no body under the default `contextVisibility: "all"` and under `"allowlist_quote"`; under `"allowlist"` an unresolved quote has no author to clear, so the reference is dropped with it. A quote of one of the bot's own messages reaches the agent as a bare reference too: the store holds no outbound text, and LINE's own message id is not something the agent has seen, so the reply is marked as a quote without naming what it quoted — the transcript is what carries that turn. A long quoted message is trimmed and marked as truncated.
