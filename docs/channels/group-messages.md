...
- Activation modes: `mention` (default) or `always`. `mention` requires a ping: a real WhatsApp @-mention (`mentionedJids`), a configured regex pattern, the bot's phone number written out as one contiguous number in the text, or a quoted reply to one of the bot's messages (except shared-number self-chat setups). The number fallback accepts an optional leading `+` and ignores spaces, hyphens, parentheses, and dots between digits, so `+1 555 123 4567`, `+1 (555) 123-4567`, and `1.555.123.4567` all ping. It does not combine digits that belong to separate numbers elsewhere in the message, and it does not match the number embedded inside a longer run of digits such as an order or invoice id. `always` wakes the agent on every message, but the injected group prompt tells it to reply only when it adds value and to return the exact silent token `NO_REPLY` (case-insensitive) otherwise. Defaults come from config (`channels.whatsapp.groups` `requireMention`) and can be overridden per group via `/activation`.
