...
- The number of webhook body reads running before signature verification is bounded. While that budget is saturated, a delivery is answered HTTP 503 before its body is read, including a correctly signed one, and is accepted again once capacity frees. The bound is fixed and not configurable, so a 503 here reports load rather than a misconfiguration. Whether Nextcloud redelivers a refused webhook depends on its own sender behavior.
