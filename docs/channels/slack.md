...
Channel management is gated by an owner-or-admin authorization check, mirroring the Microsoft Teams group-management surface. A request is allowed only when the sender is the conversation owner (`senderIsOwner`) or the gateway client carries the `operator.admin` scope. A direct-operator conversation-read origin is **not** sufficient on its own — it only proves read visibility, so it is rejected unless it also carries owner or `operator.admin` authority. Every other caller is rejected before any Slack API call is made, and the action stays hidden from the model unless the gate is on.

These actions mutate workspace state, so the Slack app or user token must hold the matching OAuth scopes: `channels:write` (and `groups:write` for private channels) for create/rename/archive, and `channels:write` / `groups:write` plus `channels:read` / `groups:read` for invite/kick. The default manifest generated by `openclaw` setup (`buildSlackManifest`) requests `channels:read` and `groups:read` but **does not** request `channels:write` or `groups:write`, because workspace mutation is an optional, default-off capability. Reauthorization alone cannot grant scopes the manifest never declared, so on a freshly generated app an operator who enables this gate must add `channels:write` and `groups:write` as optional bot scopes in the Slack app settings (OAuth & Permissions → Bot Token Scopes) and reinstall the app before the new token is issued. Keep these scopes out of the default manifest so unrelated installs do not request workspace-write permission. Alternatively, use a user token (`xoxp-`) whose owner already holds the scopes when the bot token lacks workspace-admin privileges.
