...
- `channels.slack.thread.historyScope` default is `thread`. When `thread.inheritParent` is unset, a parent copy remains provisional until OpenClaw delivers a visible reply in the thread; silent and message-tool-only turns retire that copy, so a later user-created thread stays isolated. Set it to `true` to inherit all eligible threads or `false` to isolate all threads. A model-selection-locked parent remains protected, so its child thread starts isolated instead of failing or copying the locked transcript.
