...
- Outbound replies and the `send` message action support text plus remote HTTP(S) media links. The plugin renders each remote media URL into the message text as `Attachment: <url>`; it does not upload the referenced file. Captions stay above the links, and multiple URLs become separate attachment lines.
- Native outbound file upload remains unavailable. Google Chat's media upload method requires user authentication, while this plugin uses service-account authentication. Local paths and non-HTTP(S) URLs are rejected.
