...
- Set `serviceAccountAdc: true` (and provide no explicit `serviceAccount` / `serviceAccountRef` / `serviceAccountFile`) to authenticate outbound calls via Application Default Credentials instead of a downloaded key. The standard ADC discovery order is honored — `GOOGLE_APPLICATION_CREDENTIALS`, then the well-known `gcloud` credentials file, then the GCE metadata server (attached service account / workload identity) — and every token request is routed through the SSRF guard. The resolved identity must be authorized for the Chat app with the `chat.bot` scope. Federation to non-Google workload-identity endpoints is intentionally not reachable through the guard. An explicit credential, if also present, takes precedence.
