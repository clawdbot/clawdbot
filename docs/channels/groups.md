...
<AccordionGroup>
  <Accordion title="Per-channel notes">
    ...
    - Matrix: allowlist uses `channels.matrix.groups`. Use room IDs (`!room:server`; server-less `!room` IDs from homeservers that omit the `:server` suffix are also accepted verbatim) or aliases (`#alias:server`); room-name keys match only with `channels.matrix.dangerouslyAllowNameMatching: true`, and unresolved entries are ignored at runtime. Use `channels.matrix.groupAllowFrom` to restrict senders; per-room `users` allowlists are also supported.
