...
`moved`, `linked`, `link_removed`, `specified`, `decomposed`, `claimed`, `heartbeat`,
...
| `workboard_unlink_dependency`                                                                                                                    | Remove both sides of a hard parent/child dependency link; safe to repeat and records link-removal events.                                                                                                                                          |
| `workboard_repair_decomposition`                                                                                                                 | Dry-run by default; apply only stale hard links whose parent and child are both explicitly marked as orchestration decomposition. Preserves card status, comments, proof, artifacts, and history.                                                  |
...
| `workboard_decompose`                                                                                                                            | Fan a parent card into children, inheriting board/tenant metadata; children use hard dependencies by default, or non-blocking provenance with explicit `decompositionMode: "orchestration"`; can complete the parent with a created-card manifest. |
...
### Repairing explicitly orchestrated relationships

Use `workboard_repair_decomposition` on an aggregate parent first without
`apply` to inspect candidates. A relationship is eligible only when both the
parent and child are explicitly marked with
`decompositionMode: "orchestration"`, the parent's decomposition manifest
contains the child, and reciprocal hard links still connect them. Review the
dry-run result, then rerun with `apply: true` to remove the reciprocal
dependency and promote eligible children.

Missing or unmarked historical decomposition modes are treated as hard intent
and are never rewritten automatically. Manual or ambiguous links are skipped;
use `workboard_unlink_dependency` only when an operator has explicitly verified
the parent/child relationship. Both repair tools are idempotent and record
...
| `operator.write` | `cards.diagnostics.refresh`, create/update/move/delete/comment/link/linkDependency/unlinkDependency/repairDecomposition/proof/artifact, attachment add/delete, worker log, protocol violation, claim/heartbeat/release/promote/reassign/reclaim/complete/block/unblock, `cards.dispatch`, `cards.bulk`, archive, `boards.upsert`/`archive`/`delete`, `cards.specify`/`decompose`, notification subscribe/delete/advance |
