...
| `nativeHookRelay`                             | `{ enabled: true }`                                    | Native hook relay controls. `enabled: false` removes the relay entirely (relay hook arrays cleared and the OpenClaw session-layer `hooks.state` entries pinned disabled, so lower-precedence copies cannot reappear) only when **both** hold: the **effective** approval policy for the thread is `never`, **and** no OpenClaw before-tool policy is active (no `before_tool_call` hook and no trusted-tool policy). The effective policy is the runtime-resolved value, which includes a stock `mode: "yolo"` deployment and any forced override (unknown-model or exec-mode reviewer forcing, forced guardian reviewer, forced danger-full-access sandbox). If either condition fails, the opt-out narrows to the required before-tool policy relay (`pre_tool_use`) and logs a warning naming the reason — otherwise a run with an active policy would drop the relay that enforces it. The opt-out never touches `features.hooks`: disabling that flag would switch off the whole Codex hook engine and suppress independent user, project, plugin, and managed hooks the relay never installed. Approvals themselves ride the app-server approval bridge and cannot be disabled by this key. |
