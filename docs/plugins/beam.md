...
The catalog is intentionally shared across the Gateway operator domain. Clients with `operator.read` can view every beamed session; on role-restricted Gateways, viewers also need a profile whose role allows viewing others' sessions (`view`, `suggest`, or `write`). Owner-only and unprofiled restricted viewers cannot see published rows. Uploads require `operator.write` or `operator.admin`. Any write-authorized operator that knows a Beam id can update that row. Uploader attribution does not grant ownership or change access. OpenClaw operator scopes are not tenant isolation; use a separate Gateway when sessions must be isolated between teams or machines.
