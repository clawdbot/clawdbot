...
- `subagent_progress` is the transient, source-message-oriented presentation signal for one user-visible background child lifetime. OpenClaw-managed children and native harness children that their harness classifies as user-visible emit `phase: "started"` after accepted creation and `phase: "ended"` only at a non-resumable terminal state. Both phases carry the same collision-resistant `runId` and stable opaque `childSessionKey`; ended events add `outcome` (`"ok"`, `"error"`, `"timeout"`, `"killed"`, or `"unknown"`). Optional `requester` metadata (`channel`, `accountId`, `to`, `threadId`, `channelId`, and `messageId`) identifies the source message when the host captured it. Consumers should ignore events without enough routing metadata and treat repeated stable IDs idempotently because recovery may replay this transient signal. Observation handlers have a five-second default timeout and do not block native harness notification processing.
