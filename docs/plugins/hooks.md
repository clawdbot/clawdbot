...
| Option             | Effect                                                                                                                                                                                                                  |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `matcher`          | Tool names handled by `before_tool_call` or `after_tool_call`. Shell aliases (`Bash`, `exec`, `exec_command`) and patch aliases (`apply_patch`, `Write`, `Edit`) share one scope. Omit or pass `[]` to match all tools. |
...
Trusted policies may set `matcher` to the same tool-name list accepted by
`before_tool_call`. Omitted and empty matchers retain the match-all behavior.
