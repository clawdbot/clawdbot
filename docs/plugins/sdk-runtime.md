...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    Scoped runtime `resetSessionEntryLifecycle(...)` helpers rotate one existing session row into a fresh canonical session id and transcript while holding the active-work fence. Callers should pass `expectedSessionId` and `expectedUpdatedAt` when reacting to an external lifecycle signal so a stale signal cannot reset a newer conversation. The `update` callback receives the reserved current entry plus the next session id/file and must return the complete metadata patch for the replacement row; returning `null` skips the reset before a new row is published. Replacement rows intentionally start with `updatedAt: 0` when the callback asks for durable-expired behavior.

    Locked harness-owned sessions require an owner-coordinated reset. Only the plugin that owns the active harness may use `api.runtime.agent.session.resetSessionEntryLifecycle(...)`; channel plugins handling provider lifecycle events use `api.runtime.channel.session.resetSessionEntryLifecycle(...)` with their registered channel id. The runtime verifies the caller's harness or channel ownership, persists an `initializationPending` removal boundary, calls the actual harness `reset(...)` hook to release the physical owner, and only then publishes the new persisted session. If draining work, the harness hook, or final persistence fails, the durable boundary remains and blocks reuse of the old transcript until the lifecycle reset is retried. Delegated execution plugins may admit and run exact locked sessions, but they cannot reset or mutate ownership.
