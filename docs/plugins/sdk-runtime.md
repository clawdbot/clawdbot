...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    Locked harness-owned sessions require an owner-coordinated reset. Harness plugins use `api.runtime.agent.session.resetSessionEntryLifecycle(...)`; channel plugins handling provider lifecycle events use `api.runtime.channel.session.resetSessionEntryLifecycle(...)` with their registered channel id. The runtime verifies the caller's harness or channel ownership, calls the actual harness `reset(...)` hook to release the physical owner, and only then publishes the new persisted session. If the harness hook is missing or fails, the reservation is rolled back and the old row remains current. Delegated execution plugins may admit and run exact locked sessions, but they cannot reset or mutate ownership.
