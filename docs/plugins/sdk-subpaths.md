...
are private-local.
  ...
  <Accordion title="Provider subpaths">
    ...
    | `plugin-sdk/provider-catalog-live-runtime` | Private-local after July 2026; Live provider model catalog helpers for guarded `/models`-style discovery: outcome-bearing `buildLiveModelProviderCatalog` and `buildOpenAICompatibleLiveModelProviderCatalog`, config-only `buildLiveModelProviderConfig`, provider-owned `projectRows`, fetch/cache helpers, `LiveModelCatalogHttpError`, TTL cache, and static fallback |
