...
```json
{
  "contracts": {
    ...
    "memoryDreamingEngines": ["memory-core"],
...
| `memoryDreamingEngines`          | `string[]` | Dreaming engine plugin ids this selected memory plugin explicitly supports as sidecars.                                              |
...
Memory plugins that support automatic Dreaming must declare each supported sidecar engine in `contracts.memoryDreamingEngines`. For example, a memory plugin that supports the bundled Dreaming engine declares `["memory-core"]`. When that plugin is selected through `plugins.slots.memory`, OpenClaw loads only a declared engine; an omitted or nonmatching declaration keeps the sidecar disabled.
