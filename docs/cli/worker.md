...
- The `read`, `write`, `edit`, `apply_patch`, `exec`, and `process` coding tools
  ...
  authority. The Gateway includes `exec` and `process` when the resolved source
  policy allows commands without an allowlist or approval, including an
  explicitly unconditional sandbox route, and does not target a paired node;
  otherwise both tools are unavailable in the worker. An empty
