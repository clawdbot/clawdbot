...
envelope. The token is materialized in a private profile inside the throwaway state
directory and scrubbed when that directory is removed. The sealed worker launcher
binds it to each `exec` child. GitHub CLI must be installed on the worker host;
the bundle includes the launcher, not `gh`.
