...
<AccordionGroup>
  ...
  <Accordion title="Managed Gateway heap sizing">
    ...
    - Existing managed service heap controls are preserved across forced reinstalls and doctor repairs, including absolute old-space, percentage old-space, and total-heap flags. Only heap flags survive managed `NODE_OPTIONS` sanitization; arbitrary preload/debug flags do not. Existing stored numbers are preserved even when they resemble an older automatic default or exceed the new recommendation.
