...
<AccordionGroup>
  <Accordion title="Status semantics">
    ...
    - With `gateway.auth.mode="trusted-proxy"`, local probes use the configured shared password, including SecretRefs, because the loopback connection bypasses the proxy. Without a shared password, the probe sends no shared credentials. Explicit `--token`/`--password` take precedence; `--url` uses only explicitly supplied credentials.
