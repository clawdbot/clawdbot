...
<Steps>
  ...
  <Step title="Preflight build (dev only)">
    ...
    A candidate must also leave a clean checkout. Successful commands that change tracked source or leave untracked files outside generated Control UI assets do not qualify. Rejection happens before the final checkout is changed or the CLI stops its managed Gateway service.
  ...
  <Step title="Install dependencies">
    Uses the repo package manager. For pnpm checkouts, the updater checks the launcher outside the target workspace before trusting its version. A missing or mismatched launcher is provisioned through temporary Corepack shims, or a temporary npm installation of the target checkout’s exact pnpm version. This does not replace the host’s global package-manager shims. Install and build keep their workspace and lockfile paths bound to the selected checkout, not an inherited project. Rollback resolves the previous checkout’s own pin. If bootstrap fails, the updater reports a package-manager-specific error instead of trying `npm run build` in a pnpm workspace. Older installed updaters may need a one-time launcher correction before crossing the pnpm 11-to-12 boundary; see [source bootstrap troubleshooting](/install/update-troubleshooting#source-builds-that-change-the-pnpm-lockfile).
  </Step>
