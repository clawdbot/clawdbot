...
| `--tag <dist-tag\|version\|spec>`                | Override the package target for this update only. It cannot be combined with an effective `extended-stable` channel, whose verified exact target is mandatory. Official OpenClaw Git source targets, including `main`, are rejected for package installs; use `--channel dev` for the supported checkout and build flow. |
