...
- When the CLI times out or the Gateway connection closes after accepting a run, the CLI returns `status: "accepted_timeout"` with the Gateway-assigned `runId`. The preserved run ID lets operators check `openclaw gateway status` and the session transcript to see whether the Gateway finished the turn, instead of losing track of the in-flight request.
