...
Run one agent turn through the Gateway. If the connection becomes uncertain, the CLI first waits for the original run and replays its cached terminal result; the explicit `--local` flag is the only embedded execution path.
...
- A transport close or timeout is ambiguous even if the CLI never received the Gateway's `accepted` response. The CLI reconnects and probes that run briefly before giving up; once the Gateway has accepted or confirmed it in flight, the CLI waits for its terminal result. Each invocation uses a separate random recovery capability, so knowing another run id is not enough to read its cached result.
- During a mixed-version upgrade, if an older Gateway rejects the recovery capability field, the CLI retries that initial request once with the legacy parameter set. Cached-result recovery is unavailable for that request, so a later ambiguous transport failure fails the command directly.
- A terminal `agent.wait` result is authoritative: if its payload cannot be recovered, the CLI reports that failure instead of executing the turn twice. A cache replay that reports `status: "in_flight"` positively confirms that the original run still owns the work; the CLI keeps waiting until its deadline, then reports `in_flight`.
- After transient handshake retries and run recovery are exhausted, an unrecoverable Gateway timeout or closed connection fails the command; the CLI never silently reruns the turn embedded. Transport loss is ambiguous — the Gateway may have accepted and may still finish the turn — so the stderr hint says to check `openclaw gateway status` and the session transcript before retrying or rerunning with `--local`, to avoid executing the turn twice.
