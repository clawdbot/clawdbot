...
- When the CLI times out or the Gateway connection closes after accepting a run, the CLI returns `status: "accepted_timeout"` with the Gateway-assigned `runId`. The Gateway-owned turn continues executing under its default 48-hour deadline. The preserved run ID lets operators check `openclaw gateway status` and re-attach with `--run-id` instead of losing track of the in-flight turn.
