...
This page covers two related commands:

- **`openclaw agent`** — a Gateway-backed (or `--local`) turn, documented in [Options](#options) through [JSON delivery status](#json-delivery-status) below.
- **`openclaw agent exec`** — a fully embedded, ephemeral one-shot run, documented immediately below.
...
By default, the command creates and later removes a temporary state directory, and it runs against your ordinary OpenClaw config — configured providers, credentials, and `agentRuntime` harness selection apply exactly as they do elsewhere. `--cwd` defaults to the process working directory and is passed as both the agent workspace and tool working directory.

### Config layering

Config is composed in three layers, entirely in memory. `exec` builds the run config and publishes it as this process's runtime config, rather than writing a copy to disk:

1. **Exec defaults** apply only where your config leaves a setting unset: workspace bootstrap files are skipped, the agent sandbox is off, the `coding` tool profile is selected, filesystem tools are restricted to `--cwd`, and the run uses the full execution policy a headless turn needs.
2. **Your config** wins over those defaults — a configured sandbox, shell env, or tool profile is never downgraded, and exec host routing stays with the sandbox when your config enables one.
3. **The invocation itself** wins last: the run is always scoped to `--cwd` and never bootstraps.

### State directory

Use `--state-dir <dir>` to retain sessions and other run state. The directory must already exist; it is never created or deleted by the command. A retained state directory requires exclusive ownership — `exec` refuses to start while a Gateway or another embedded writer owns it, then holds the state lock for the full run.

Omit `--state-dir` for isolated, temporary state, or stop the Gateway first with `openclaw gateway stop`.

When `exec` uses the ambient or a pinned config, installed plugins still resolve from the operator's ordinary plugin roots while sessions and other run state use the ephemeral directory. In those modes, `--state-dir` controls run state only — it is not required for configured providers, channels, or harnesses supplied by installed plugins.

### Reproducible runs

For reproducible runs, pin the config instead of inheriting it:

- **`--config <path>`** runs against exactly that config file, read through the normal loader so JSON5 syntax and `$include` resolve relative to it. A missing or invalid file fails the run rather than falling back to defaults — as does an ambient config that exists but cannot be parsed.
- **`--isolated`** ignores the ambient config entirely and uses only the exec defaults above.

Both are the right choice for CI, where inheriting operator state would make runs machine-dependent.

### Credentials

Stored credentials are used by default, so a folder-scoped run reaches the same logins as the rest of the CLI.

Pass `--auth-env-only` to restrict the run to provider keys already present in the process environment. This mode:

- loads no config at all (pairing it with `--config` is rejected rather than silently ignored, since a config can supply provider credentials through several surfaces at once — [inline keys and secret headers](/reference/secretref-credential-surface), an `env` block, and login-shell import)
- skips OpenClaw auth profiles and external Codex, Claude, or other CLI credential stores
- still allows provider auth variables for model authentication, but omits them from agent-launched host commands
...
For this command only, explicit `--fallback` values remain active alongside an explicit `--model`. Other agent entry points keep their existing rule that a user-selected model disables configured fallbacks.
...
- `--code-mode direct` disables Code Mode.
- `--code-mode auto` uses model capability metadata.
- `--code-mode code` forces the generic Code Mode surface for tool-capable runs.
- `--local-model-lean` removes high-latency and channel-dependent tools and enables the bounded Tool Search defaults for the isolated run.

### Timeouts and exit codes

The timeout defaults to 600 seconds for `agent exec`; this does not change the existing embedded `agent --local` default. A successful run exits `0`; any model or result error exits `1`; a timeout exits `2`. Failure includes `meta.error`, aborted runs, exhausted model fallbacks, an error stop reason, and any error payload.

### Output

Plain output writes only the final assistant text to stdout; diagnostics go to stderr. `--json` reserves stdout for this stable envelope:
...
The output directory contains the canonical QA Lab `qa-evidence.json`. `summary.json` and `results.jsonl` are supporting aggregate and per-cell artifacts; `manifest.json` records the requested matrix and source identity.
