...
When the registry is stale, the command lists each differing plugin with its persisted and derived source. JSON output returns the same rows in `differences`. A refresh rereads and verifies its persisted replacement before it reports success. If plugin package files keep changing during verification, stop those updates and run `openclaw plugins registry --refresh` again.
