...
`channels remove` only operates on installed/configured channel plugins. Use `channels add` first for installable catalog channels. Without `--delete` it asks to disable the account and keeps its config; `--delete` removes the config entries without prompting. `--delete` also discards the inbound events that account still had stored in the Gateway's ingress queue, including failed ones that [`channels dead-letters resubmit`](#inbound-dead-letters) could have replayed, and reports how many it discarded. Disabling keeps them, because re-enabling the account drains them. An account whose channel is served by a plugin that also serves other channels keeps its stored events either way: those channels share one queue and the stored events do not record which channel they arrived on, so this command cannot tell one channel's events from another's.
