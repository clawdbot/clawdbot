...
- After successful daemon setup, the final Gateway status allows a short startup grace period before reporting reachability. Service installation and Gateway reachability are separate outcomes; if the Gateway is still not detected, run `openclaw health` to check it again.
