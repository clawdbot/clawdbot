...
- `--gateway-auth password --gateway-password <password>` stores a plaintext password for password auth mode. To store it as an env-backed SecretRef instead, combine `--gateway-auth password` with `--secret-input-mode ref` and set `OPENCLAW_GATEWAY_PASSWORD` in the onboarding process environment; onboarding writes `gateway.auth.password` as a SecretRef using that env var (see [Configuration reference](/gateway/configuration-reference)).
