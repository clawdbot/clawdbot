...
The repair prompt instructs the agent to change only the target installation and its OpenClaw state. It forbids editing credentials or auth stores, deleting state or databases, package-manager writes outside the target root, and starting, stopping, or restarting services or the Gateway. These are instructions for the agent; host commands and filesystem tools are not restricted to those paths. Allowed checks include `openclaw doctor --lint --json`, `openclaw doctor --fix`, and `openclaw health --json`. The repair loop does not activate updates, restart services, take snapshots, or undo files.
