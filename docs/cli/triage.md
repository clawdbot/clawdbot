...
Embedded triage supports local OpenClaw tools, local CLI harness children, and local Codex native shells, including local stdio, loopback, and Unix-socket app-server connections. It refuses selected ACP turns, OpenClaw-provisioned sandboxes, and remote/node execution when they cannot preserve this local target, including a Codex app-server with `remoteWorkspaceRoot` even through a loopback connection. It leaves the saved prompt and external/manual commands available. It does not redirect those routes onto the host or relax native sandbox and approval policy.
