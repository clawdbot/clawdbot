...
- OpenClaw sends `prompt_cache_key` to keep cache routing stable across turns. Direct `api.openai.com` (including regional subdomains such as `us.`/`eu.api.openai.com`) and dedicated Azure OpenAI hosts (`*.openai.azure.com`) get this automatically. On multi-model Azure AI Foundry hosts (`*.services.ai.azure.com`, `*.cognitiveservices.azure.com`, and the regional `*.api.cognitive.microsoft.com`) the default applies only to OpenAI-family deployments (`gpt-*` excluding `gpt-oss*`, `chatgpt-*`, `codex*`, and the `o*` reasoning series), judged by the model display name; the operator-chosen deployment id only counts when no display name is configured, so a non-OpenAI model behind an alias like `gpt-prod` never qualifies. Custom-provider setup pins `compat.supportsPromptCacheKey: false` on models it adds for Foundry hosts because the display name it synthesizes derives from the deployment alias rather than canonical model metadata; flip it to `true` for a verified OpenAI-family deployment. Other deployments (MAI, Llama, ...) need `compat.supportsPromptCacheKey: true`. OpenAI-compatible proxies (oMLX, llama.cpp, custom endpoints) need `compat.supportsPromptCacheKey: true` in model config to opt in - this is never auto-detected for a proxy.
