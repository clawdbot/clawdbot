...
- GPT-5.6 Responses models use explicit prompt caching. OpenClaw maps its stable/dynamic system-prompt boundary to an `input_text.prompt_cache_breakpoint` and sends `prompt_cache_options.mode: "explicit"`, so changing turn context stays outside the cached prefix. Other model families retain automatic caching unless a verified provider plugin declares equivalent support.
- On native `previous_response_id` continuations, OpenClaw compares only the stable input through that breakpoint and appends the current dynamic developer context with the new turn. Changing runtime facts therefore preserves both continuation reuse and the cached prefix.
...
- A verified GPT-5.6 Responses-compatible provider plugin can declare explicit-cache support while normalizing its runtime model. Unknown compatible endpoints stay on implicit caching by default so OpenClaw does not send unsupported fields.
- GPT-5.6 explicit caching uses the API's current 30-minute TTL and does not send deprecated `prompt_cache_retention: "24h"`. `cacheRetention: "none"` sends explicit mode without a breakpoint or cache key, disabling cache writes. Earlier models retain the existing long-retention behavior.
