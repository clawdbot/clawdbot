...
coverage declarations, or the guard and its routing. All-lane checks include it
too. Every gateway event must have a handler or an explicitly approved
non-consumption declaration for each mobile client. To run only this guard,
use `pnpm check:protocol-coverage`.
