...
| `pnpm test:docker:hosting-profiles`                                         | Blocking package-acceptance release proof for built-in hosting-profile selection and `/readyz` condition aggregation. The lane proves unchanged unprofiled readiness; explicit `local`; ready and loopback-invalid `container` bindings; ready trusted-proxy and invalid token-auth `reverse-proxy` postures; `node-mode` transitioning from `503` to `200` after a real node host is paired, connected, and advertising an approved command; and workspace writability transitioning from ready to `WorkspaceStorageFull` and back without restarting. It also checks required versus advisory conditions and stable failure/advisory arrays.                                                                                                        |
