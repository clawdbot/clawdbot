...
- The macOS workflow retains attempt-specific recovery artifacts. Its recovery preflight accepts `resume_notarization_run_id` and `resume_notarization_run_attempt`, then produces a new successful preflight for the existing promotion flow and matching Swift validation. Recovery does not bypass environment approval. Older failed runs without checkpoint artifacts cannot be recovered this way.
