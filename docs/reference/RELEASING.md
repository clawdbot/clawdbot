...
9. Tag the Release SHA, then run the candidate helper with the successful Release-SHA validation parent and npm preflight instead of dispatching either again:
   ...
   `pnpm release:candidate` validates the current frozen branch tip by default (or an explicit `--target-sha`), and rejects a tag that already exists. It records evidence before the final signed tag is pushed; create that tag only after the helper is green.
