...
- **System events** (heartbeat, cron wakeups, exec notifications, gateway bookkeeping) may mutate the session row but never extend daily/idle reset freshness. A successfully committed, operator-facing direct cron-delivery mirror is the exception: because it becomes part of the destination channel transcript, it refreshes that session's idle timestamp. Reset rollover discards queued system-event notices for the previous session before the fresh prompt is built.
