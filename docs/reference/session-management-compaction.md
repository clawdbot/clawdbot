...
- **System events** (heartbeat, cron wakeups, exec notifications, gateway bookkeeping) may mutate the session row but never extend daily/idle reset freshness. A successfully committed, operator-facing direct cron-delivery mirror is the exception: because it becomes part of the selected destination session transcript, it refreshes that session's idle timestamp. Reset rollover discards queued system-event notices for the previous session before the fresh prompt is built.
...
- `lastInteractionAt`: last real user/channel interaction timestamp; idle reset freshness uses this so heartbeat, cron wakeups, and exec events do not keep sessions alive. A successfully committed, operator-facing direct cron-delivery mirror also updates this timestamp because it is written into the selected destination session transcript. Legacy rows without this field fall back to the recovered session start time.
