...
Operators can also opt in to [trusted Discord administrators](/tools/slash-commands#trusted-discord-administrators) with `commands.channelAdministrators`. A fresh authenticated owner message must match the explicitly configured Discord account, native user, and conversation. This role permits cross-session automation management through available tools; ordinary owner or channel access alone does not.

The Gateway grants either role from authenticated turn admission, not copied sender IDs, account IDs, or session routes. Each operation uses a one-use grant that expires after 60 seconds and remains bound to that exact active run. Ordinary channel turns and Control UI turns without `operator.admin` receive no such grant. If access is denied or a grant expires, retry from a fresh authorized administrator turn, or use the **Automations** page.

Control UI admin management requests record their method, run, operational instance, and success or failure in the Gateway's `cron: admin management` log, alongside the ordinary tool audit record. Trusted Discord administrator requests are identified by the Gateway's `channel administrator request` log and retain ordinary tool audit records. Management authority does not transfer creator attribution or replace the job's scheduled execution policy.
