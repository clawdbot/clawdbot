...
The agent `automations` tool returns compact job summaries (`id`, `name`, `enabled`, `autoDisabled`, `nextRunAtMs`, `scheduleKind`, `lastRunStatus`) from `automations(action: "list")`. `autoDisabled: true` is present only when the scheduler disabled the job after repeated failures or schedule errors; it is omitted when an operator disabled the job. Use `automations(action: "get", jobId: "...")` for one full job definition. Direct Gateway callers can pass `compact: true` to `cron.list`; omitting it preserves the full response with delivery previews. `cron.add` includes the same dry-run preview on the created job so create-time output names a resolved route or fail-closed outcome.
