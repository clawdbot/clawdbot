...
- Per-run token budget: the payload `tokenBudget` field (UI "Token budget") sets an **observed-use threshold**, not a hard cap. Leave it blank for unlimited (the default). When set, the automation run is cancelled once it has spent roughly that many tokens across its candidates — the count accumulates across every candidate in the run, including model-fallback retries, not per individual call. Cancellation fires _after_ a candidate finishes, so one in-flight candidate can overshoot before the run stops; treat the value as a soft target rather than a guaranteed ceiling, and size it with that overshoot in mind. To remove a budget you previously set, clear the field (an explicit `null` is persisted on update) — the run then falls back to unlimited. The token budget is independent of the wall-clock budget above and of the [session context budget](/gateway/configuration-reference): the token budget bounds total run spend, while the context budget bounds how much history a single turn retains. CLI-backed providers observe spend post-hoc, so a CLI candidate can overshoot further than an embedded candidate before the guard trips; embedded providers check the guard at candidate boundaries.
