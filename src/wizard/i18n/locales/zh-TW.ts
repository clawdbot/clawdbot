...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 OpenCrustacean 設定",
    ...
    memoryImport: {
      ...
      confirm: "將這些 memory 匯入你的 OpenCrustacean 工作區？",
    ...
    remote: {
      ...
      plaintextPasswordHint: "直接把密碼存入 OpenCrustacean 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 OpenCrustacean 設定",
    ...
    guided: {
      ...
      custodianIntro: "你好 — 我是 OpenCrustacean，負責維護這套系統。我們開始設定吧。",
      ...
      appliedTitle: "推理已就緒",
      complete: "OpenCrustacean 已準備就緒。",
      completeWithoutAi: "OpenCrustacean 設定已儲存。連接 AI 後再開啟聊天。",
      detected: "AI 偵測完成。",
      ...
      intro: "連接你的 AI",
      invalidConfigDetails: "OpenCrustacean 設定 {path} 無效：\n{issues}",
      invalidConfigRepair:
    ...
    security: {
      ...
      attribution:
        "OpenCrustacean 是由 OpenCrustacean 基金會（非營利組織）開發的開源助手，會與你一同學習成長。",
      baselineDmSessions:
      ...
      confirm: "我理解 OpenCrustacean 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉安全加固和存取控制，請不要執行 OpenCrustacean。",
      learnMore: "了解更多",
      notMultitenant: "OpenCrustacean 預設不是敵對多租戶安全邊界。",
      personalAgent: "預設情況下，OpenCrustacean 是個人 agent：一個可信操作者邊界。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作區設定 → Integrations → OpenCrustacean → 建立 bot → 複製 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "OpenCrustacean" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 OpenCrustacean 將你加入允許清單。",
      ...
      separatePhoneLabel: "OpenCrustacean 專用獨立手機",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 OpenCrustacean 用來向 Chat 傳送回覆的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "OpenCrustacean"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 OpenCrustacean，並啟用完整 agent 能力。",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 OpenCrustacean。",
      dashboardLinkWithToken: "Dashboard 連結（含權杖）：{url}",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 OpenCrustacean。",
      ...
      daemonRuntimeNodeHint:
        "OpenCrustacean 狀態使用 node:sqlite，因此必須使用 Node；Bun 無法執行閘道。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 OpenCrustacean。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 OpenCrustacean。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。OpenCrustacean 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
