...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 OpenCrustacean 配置",
    ...
    memoryImport: {
      ...
      confirm: "将这些 memory 导入你的 OpenCrustacean 工作区？",
    ...
    remote: {
      ...
      plaintextPasswordHint: "直接把密码存入 OpenCrustacean 配置",
      plaintextPasswordLabel: "现在输入密码",
      plaintextTokenHint: "直接把令牌存入 OpenCrustacean 配置",
    ...
    guided: {
      ...
      custodianIntro: "你好 — 我是 OpenCrustacean，负责维护这套系统。我们开始设置吧。",
      ...
      appliedTitle: "推理已就绪",
      complete: "OpenCrustacean 已准备就绪。",
      completeWithoutAi: "OpenCrustacean 设置已保存。连接 AI 后再打开聊天。",
      detected: "AI 检测完成。",
      ...
      intro: "连接你的 AI",
      invalidConfigDetails: "OpenCrustacean 配置 {path} 无效：\n{issues}",
      invalidConfigRepair:
    ...
    security: {
      ...
      attribution:
        "OpenCrustacean 是由 OpenCrustacean 基金会（非营利组织）开发的开源助手，会与你一同学习成长。",
      baselineDmSessions:
      ...
      confirm: "我理解 OpenCrustacean 默认面向个人使用；共享/多用户使用需要加固。继续？",
      hardeningRequired: "如果你不熟悉安全加固和访问控制，请不要运行 OpenCrustacean。",
      learnMore: "了解更多",
      notMultitenant: "OpenCrustacean 默认不是敌对多租户安全边界。",
      personalAgent: "默认情况下，OpenCrustacean 是个人 agent：一个可信操作者边界。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作区设置 → Integrations → OpenCrustacean → 创建 bot → 复制 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "OpenCrustacean" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 OpenCrustacean 将你加入允许列表。",
      ...
      separatePhoneLabel: "OpenCrustacean 专用独立手机",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 OpenCrustacean 用来向 Chat 发送回复的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "OpenCrustacean"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 OpenCrustacean，并启用完整 agent 能力。",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 OpenCrustacean。",
      dashboardLinkWithToken: "Dashboard 链接（含令牌）：{url}",
      dashboardOpened: "已在浏览器中打开。保留该标签页以控制 OpenCrustacean。",
      ...
      daemonRuntimeNodeHint:
        "OpenCrustacean 状态使用 node:sqlite，因此必须使用 Node；Bun 无法运行网关。",
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 OpenCrustacean。",
      outroDashboardOpened:
        "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 OpenCrustacean。",
      ...
      containerSystemdUnavailable:
        "此容器内没有 systemd 用户服务。OpenCrustacean 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
