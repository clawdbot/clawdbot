...
export const zh_CN = {
  ...
  wizard: {
    ...
    security: {
      askForHelp: "启用工具或将其暴露到互联网之前，请向有经验的人寻求帮助。",
      attribution:
      ...
      confirm: "我理解 OpenClaw 默认面向个人使用；共享使用需要额外的安全措施。继续？",
      ...
      runRegularly: "定期运行",
      sharedAuthority:
        "按发送者设置的工具策略可以限制某人能做什么，但不能提供完全隔离：人们仍可影响同一个 agent 如何使用共享的文件、账户和工具。",
      title: "安全提示：启用工具前请阅读",
      toolAccess: "启用工具后，它可以读取文件，并通过你的账户或已连接的服务执行操作。",
    },
    skills: {
      configure: "现在配置技能？（推荐）",
      containerBrewHidden:
