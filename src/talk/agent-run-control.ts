...
export async function controlRealtimeVoiceAgentRun(
  ...
  if (mode === "cancel") {
    if (!sessionId) {
      return {
        ...
        message: "There is no active OpenCrustacean run to cancel.",
        speak: true,
        show: true,
        suppress: false,
      };
    ...
    const message = aborted
      ? "Cancelled the active OpenCrustacean run."
      : "OpenCrustacean could not cancel the active run.";
    return {
      ok: aborted,
      mode,
      sessionKey,
  ...
  if (!sessionId) {
    return {
      ...
      queued: false,
      reason: "no_active_run",
      message: "There is no active OpenCrustacean run to steer.",
      speak: true,
      show: true,
      suppress: false,
    };
  ...
  return {
    ...
    message:
      mode === "followup"
        ? "Queued that follow-up for the active OpenCrustacean run."
