// Shared lazy realtime media runtime: werift peer plus WASM Opus framing and PCM conversion.
...
} from "./webrtc-audio-buffer.js";

const WEBRTC_SAMPLE_RATE = 48_000;
const RELAY_SAMPLE_RATE = 24_000;
const WEBRTC_CHANNELS = 2;
...
export type RealtimeWebRtcAudioPeerCallbacks = {
  onAudio: (audio: Buffer) => void;
  onError: (error: Error) => void;
  onRtpPacket?: () => void;
};

export type RealtimeWebRtcAudioPeerContract = {
  createOffer(): Promise<string>;
  applyAnswer(answerSdp: string): Promise<void>;
  adoptPendingAudio(pendingAudio: RealtimeWebRtcPendingAudio): void;
...
function convertRelayPcmToWebRtcPcm(pcm24kMono: Buffer): Int16Array {
  const mono48k = pcmBufferToInt16(resamplePcm(pcm24kMono, RELAY_SAMPLE_RATE, WEBRTC_SAMPLE_RATE));
  const stereo48k = new Int16Array(mono48k.length * WEBRTC_CHANNELS);
...
function convertWebRtcPcmToRelayPcm(pcm48kStereo: Int16Array): Buffer {
  const frameCount = Math.floor(pcm48kStereo.length / WEBRTC_CHANNELS);
  ...
  return resamplePcm(mono48k, WEBRTC_SAMPLE_RATE, RELAY_SAMPLE_RATE);
}
...
export class RealtimeWebRtcAudioPeer implements RealtimeWebRtcAudioPeerContract {
  static async create(params: {
    ...
    try {
      ...
      return new RealtimeWebRtcAudioPeer({
        callbacks: params.callbacks,
        diagnosticLabel: params.diagnosticLabel ?? "Realtime",
        decoder,
        encoder,
        peer,
        transceiver,
  ...
  static convertRelayPcm(pcm24kMono: Buffer): Int16Array {
    return convertRelayPcmToWebRtcPcm(pcm24kMono);
  }

  static convertWebRtcPcm(pcm48kStereo: Int16Array): Buffer {
    return convertWebRtcPcmToRelayPcm(pcm48kStereo);
  }
  ...
  private pendingAudio = new RealtimeWebRtcPendingAudio();
    ...
    state.peer.connectionStateChange.subscribe((connectionState) => {
      ...
      if (connectionState === "connected") {
        ...
        this.state.callbacks.onError(
          new Error(`${this.state.diagnosticLabel} WebRTC media connection ${connectionState}`),
        );
      }
    });
  }
  ...
  async createOffer(): Promise<string> {
    ...
    if (!sdp?.trim()) {
      throw new Error(`werift did not produce a ${this.state.diagnosticLabel} SDP offer`);
    }
    return sdp;
  }
  ...
  adoptPendingAudio(pendingAudio: RealtimeWebRtcPendingAudio): void {
    ...
    if (this.pendingAudio.length > 0) {
      pendingAudio.clear();
      throw new Error(`${this.state.diagnosticLabel} WebRTC peer already owns pending audio`);
    }
  ...
  private handleInboundRtp(packet: WeriftRtpPacket): void {
    ...
    try {
      ...
      if (this.activeInboundSsrc === undefined) {
        ...
        throw new Error(`${this.state.diagnosticLabel} WebRTC audio source changed unexpectedly`);
      }
      ...
      if (distance >= RTP_SEQUENCE_HALF_RANGE) {
        ...
        throw new Error(`${this.state.diagnosticLabel} WebRTC RTP sequence changed unexpectedly`);
      }
      ...
      this.state.callbacks.onError(
        toErrorObject(error, `${this.state.diagnosticLabel} WebRTC media failed`),
      );
    }
  }
  ...
  private sendNextAudioFrame(): void {
    ...
    try {
      const opusPacket = this.state.encoder.encode(convertRelayPcmToWebRtcPcm(frame), {
        frameSize: OPUS_FRAME_SAMPLES,
      });
  ...
  private takeNextRelayFrame(): Buffer {
    ...
    const frame = Buffer.alloc(REALTIME_WEBRTC_RELAY_FRAME_BYTES);
