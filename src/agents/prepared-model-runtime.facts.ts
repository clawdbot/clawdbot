...
import {
  ...
  associatePreparedPluginGenerationResources,
...
export async function prepareWorkspaceBuildGroup(
  ...
  const resources = reusablePluginGeneration
    ? retainPreparedPluginGenerationResources(reusablePluginGeneration)
  ...
  try {
    return await withPluginRegistryResourceScope(resources, async () => {
      ...
      const prepare = async () => {
          ...
          ...new Set([
            ...inputs.flatMap(({ config, agentId }) =>
              withAgentRosterFactsBatch(config, () => [
                ...
                ...parseConfiguredModelVisibilityEntries({ cfg: config, agentId })
                  .providerWildcards,
              ]),
            ),
        ...
        associatePreparedPluginGenerationResources(pluginGeneration, resources);
        return {
          agentFacts,
          resources,
          buildStats: {
