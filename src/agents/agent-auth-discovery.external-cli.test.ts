...
import { resolveAgentAuthForDiscovery } from "./agent-auth-discovery.js";
import { externalCliDiscoveryForProviders } from "./auth-profiles/external-cli-discovery.js";

describe("resolveAgentAuthForDiscovery external CLI scoping", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("threads scoped external CLI discovery into writable auth store loading", () => {
    ...
    resolveAgentAuthForDiscovery("/tmp/openclaw-agent", {
      config: cfg,
      env: {},
      externalCli,
    });
  ...
  it("merges prepared ambient credentials without repeating ambient discovery", () => {
    ...
    const { credentials } = resolveAgentAuthForDiscovery("/tmp/openclaw-agent", {
      ambientCredentials: {
