...
describe("sanitizeUserFacingText", () => {
  ...
  it("strips copied inbound metadata blocks from user-facing assistant text", () => {
    const input = [
      ...
      "```json",
      '{"chat_id":"channel:123","sender":"OpenCrustacean"}',
      "```",
      "",
      markInboundContextLabel("Sender:"),
      "```json",
      '{"label":"OpenCrustacean (123)"}',
      "```",
      "",
      "Pong",
      "",
  ...
  it("does not strip ordinary text that merely mentions internal marker strings", () => {
    const input = [
      "The literal header `OpenCrustacean runtime context (internal):` appears in this note.",
  ...
  it("does not strip text that starts with the legacy header phrase but is not the canonical block", () => {
    const input =
      "OpenCrustacean runtime context (internal): is the label used by the old runtime block formatter.";
