...
describe("harness context engine lifecycle", () => {
  ...
  it.each([
    ["skips exact no-debt background maintenance", "background", { maintenanceDebt: "none" }, 0],
    ["runs pending background maintenance", "background", { maintenanceDebt: "pending" }, 1],
    ["preserves legacy void background maintenance", "background", undefined, 1],
    ["fails open for malformed background results", "background", { maintenanceDebt: false }, 1],
    [
      "ignores no-debt admission for foreground maintenance",
      "foreground",
      { maintenanceDebt: "none" },
      1,
    ],
  ] as const)("%s", async (_name, turnMaintenanceMode, afterTurnResult, expectedCalls) => {
    const runMaintenance = vi.fn(async () => undefined);

    await finalizeHarnessContextEngineTurn({
      contextEngine: createContextEngine({
        info: { id: "test", name: "Test context engine", turnMaintenanceMode },
        afterTurn: vi.fn(async () => afterTurnResult as never),
      }),
      promptError: false,
      aborted: false,
      yieldAborted: false,
    ...
    expect(runMaintenance).toHaveBeenCalledTimes(expectedCalls);
  });

  it("recovers durable maintenance debt through unconditional bootstrap maintenance", async () => {
    // Model a fresh engine restoring plugin-owned debt after the prior process
    // exited before its ephemeral turn-maintenance admission could be queued.
    let maintenanceDebt: "pending" | "none" = "pending";
    const afterTurn = vi.fn(async () => ({ maintenanceDebt }) as const);
    ...
    expect(maintenanceDebt).toBe("none");
  });
