...
describe("createCliJsonlStreamingParser events", () => {
  ...
  it("still merges input_json_delta chunks after an empty start-block input", () => {
    // The empty input object is the streaming placeholder: seeding from it
    // must not corrupt subsequent delta concatenation.
    const starts: CliToolUseStartDelta[] = [];
    const parser = createCliJsonlStreamingParser({
      backend: {
        command: "local-cli",
