...
let resolveAgentAuthForDiscovery: typeof import("./agent-auth-discovery.js").resolveAgentAuthForDiscovery;
...
describe("agent model discovery synthetic auth", () => {
  beforeAll(async () => {
    ({ resolveAgentAuthForDiscovery } = await import("./agent-auth-discovery.js"));
  });
  ...
  it("mirrors plugin-owned synthetic cli auth into credential discovery", async () => {
    await withAgentDir(async (agentDir) => {
      const { credentials } = resolveAgentAuthForDiscovery(agentDir, { readOnly: true });
