...
describe("resolveSandboxedMediaSource", () => {
  ...
  it("rejects symlinked OpenCrustacean tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked OpenCrustacean tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked OpenCrustacean tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenClawTmp(
