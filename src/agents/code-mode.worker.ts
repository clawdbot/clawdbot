...
function createHostRequestState(
  pendingRequests: PendingBridgeRequest[],
  ...
  return {
    ...
    bridgeFailure:
      pendingRequests.length > config.maxPendingToolCalls
        ? new CodeModeWorkerFailure("invalid_input", CODE_MODE_PENDING_TOOL_CALLS_ERROR)
...
function createHostRequestHandler(params: {
  ...
  return (methodHandle, argsHandle, bridgeIdHandle) => {
    if (params.state.pendingRequests.length >= params.config.maxPendingToolCalls) {
...
async function createVm(params: {
  ...
  const state = createHostRequestState(params.pendingRequests, params.config);
  vm.newFunction(
