...
export function createDashboardTool(opts: DashboardToolOptions = {}): AnyAgentTool {
  ...
  return {
    ...
    name: "dashboard",
    description:
      "Native per-session dashboard and Split view, not a standalone web app. Use for an explicit dashboard request or multiple non-code visualizations; keep one ad hoc visualization inline. Workflow: read layout, create or update content through its owning authoring capability, focus_tab, then set_chat_dock to left/right/bottom for Split view. Preserve the tab's existing visible dock; otherwise use right. Use hidden only when the user explicitly asks for a full Dashboard without chat. focus_tab and set_chat_dock require a connected Control UI; if unavailable, leave content pinned and report that limitation instead of substituting a standalone server preview. Read and arrange this session dashboard: read snapshot; tab_create/tab_update/tab_delete/tabs_reorder; widget_put/widget_move/widget_resize/widget_remove. Widgets use stable names. widget_put creates or updates trusted plugin widgets only; update other content through its owning authoring capability discovered in the tool catalog. Plugin examples: session:progress props {sessionKey?} renders the session's live progress card (omit sessionKey for the current session), workboard:card props {cardId}, workboard:mini props {boardId, limit}, workboard:board props {boardId}. Sizes: sm=3x3, md=6x4, lg=8x6, xl=12x8, full=12x8 single-widget emphasis.",
