  ...
  it("does not reuse cached responses across configured user agents", async () => {
    ...
    const first = requireDetails(
      (await createContractTool({
        cacheTtlMinutes: 1,
        userAgent: "OpenCrustacean-Test-UA-A",
      })?.execute("user-agent-a", args))!,
    );
    const second = requireDetails(
      (await createContractTool({
        cacheTtlMinutes: 1,
        userAgent: "OpenCrustacean-Test-UA-B",
      })?.execute("user-agent-b", args))!,
    );

    expect(first.text).toContain("OpenCrustacean-Test-UA-A");
    expect(second.text).toContain("OpenCrustacean-Test-UA-B");
