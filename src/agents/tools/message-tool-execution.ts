    ...
    execute: async (toolCallId, args, signal) => {
      ...
      try {
        result = await runMessageActionForTool({
          ...
          abortSignal: signal,
          // Revalidate the admitted turn capability at the final-effect boundary.
          // The capability is resolved once at tool entry, then `runMessageAction`
          // awaits client/runtime preparation before a privileged plugin mutation
          // (e.g. Feishu chat/membership changes) issues provider I/O. A delegated
          // turn revoked or expired during that interval must not reach the
          // provider, so the closure re-resolves the capability synchronously and
          // throws when it is no longer active. Direct operator/CLI turns carry
          // no capability token and need no revalidation.
          revalidateRuntimeAuthority: normalizeOptionalString(options?.messageActionTurnCapability)
            ? () => {
                const stillActive = messageActionTurnCapability.resolveMessageActionTurnCapability({
                  token: options?.messageActionTurnCapability,
                  agentId: resolvedAgentId ?? "",
                  runId: options?.runId,
                  sessionKey: options?.agentSessionKey ?? "",
