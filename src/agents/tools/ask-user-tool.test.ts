...
describe("ask_user prompt delivery", () => {
  ...
  it("keeps a renewed reservation ready after an earlier status read expires", async () => {
    ...
    try {
      ...
      const readiness = waitForAskUserPromptReady(reservation.questionId, gateway.call);
      const execution = createAskUserTool({ sessionKey, gatewayCall: gateway.call }).execute(
        toolCallId,
        { ...validArgs, timeoutSeconds: 30 },
      );
      await vi.waitFor(() =>
