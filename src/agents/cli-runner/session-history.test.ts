...
describe("buildCliSessionHistoryPrompt", () => {
  it("renders OpenCrustacean transcript history around the next user message", () => {
  ...
  it("caps rendered reseed history before adding the next user message", () => {
    ...
    expect(prompt).toContain("[OpenCrustacean reseed history truncated; older turns dropped]");
  ...
  it("keeps a whole code point when the retained history tail starts inside an emoji", () => {
    ...
    expect(prompt).toContain(
      "<conversation_history>\n[OpenCrustacean reseed history truncated; older turns dropped]\ntail\n</conversation_history>",
    );
  });
