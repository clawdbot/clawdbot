...
export function describeSessionsSpawnTool(options?: {
  ...
  return [
    ...
    '`visible=true`: persistent sidebar dashboard session; use when the user asks to create/open a thread; `category` groups it and omission inherits the parent category; subagent only; omit `mode` (no `mode="run"`), `thread`, `thinking`, `lightContext`, `attachments`, `attachAs`; inherits the caller tool-policy ceiling; may check out a git worktree via `worktree`/`worktreeName`/`worktreeBaseRef`. When its accepted result includes `sessionUrl`, channel acknowledgements put the session URL on the first line and `Owner: <label>` on the second line.',
    visibilityLine,
