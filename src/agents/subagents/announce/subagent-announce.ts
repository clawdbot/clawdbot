...
function buildAnnounceReplyInstruction(params: {
  ...
  if (params.expectsCompletionMessage) {
    return `A completed ${params.announceType} is ready for parent review. This completion ends one child run, not necessarily the original user request. Compare the result with the requested outcome before deciding the task is done.${modelRouteInstruction} Reviews, failed checks, and other in-scope fixable blockers require continued work or a follow-up in the kept child session; report a blocker only when progress needs new user authority or an unavailable external decision. Otherwise send a truthful user-facing update. Keep this internal context private (don't mention system/log/stats/session details or announce type). Reply ONLY: ${SILENT_REPLY_TOKEN} only when this exact result is already visible to the user in this same turn.`;
  }
