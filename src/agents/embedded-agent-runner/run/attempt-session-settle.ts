...
export function createEmbeddedAttemptSessionSettleTracker(
  ...
  const recordAbortCleanupOutcome = () => {
  ...
  return {
    ...
    buildAbortSettlePromise: () => {
      // Abort callbacks can run outside the joining caller's async context.
      // Replay their retained failure here, including after all work settled.
      recordAbortCleanupOutcome();
      return inFlight.size === 0
        ? null
        : Promise.allSettled(inFlight).then(recordAbortCleanupOutcome);
    },
