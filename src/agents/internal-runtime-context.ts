...
/**
 * Keeps transient context after the active user, before that turn's tool loop.
 * Moving it past tool results would change the complete prior request prefix
 * required by Responses continuation. Historical carriers are stripped first.
 */
export function relocateCurrentRuntimeContextCarrierAfterUser<T>(messages: T[]): T[] {
  ...
  const insertionIndex = rest.findLastIndex(isUserMessage) + 1;
  if (insertionIndex === 0) {
    return messages;
  }
  const relocated = [...rest.slice(0, insertionIndex), ...carriers, ...rest.slice(insertionIndex)];
  return relocated.every((message, index) => message === messages[index]) ? messages : relocated;
}
