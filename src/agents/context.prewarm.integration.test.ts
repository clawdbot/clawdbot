...
describe("post-ready context cache prewarm", () => {
  it("yields through the real prepared catalog lifecycle and converges atomically", async () => {
    ...
    let warmCompletedAt = warmStartedAt;
    try {
      ...
      warmCompletedAt = performance.now();
    ...
    const warmMs = warmCompletedAt - warmStartedAt;
    const elu = performance.eventLoopUtilization(performance.eventLoopUtilization(), eluStart);
    const maxHeartbeatGapMs = Math.max(...heartbeatGaps);
    const delayMaxMs = delayMonitor.max / 1_000_000;
    console.log(
    ...
    expect(maxHeartbeatGapMs).toBeLessThan(500);
    expect(delayMaxMs).toBeLessThan(500);
    expect(
