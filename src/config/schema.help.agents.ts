...
export const AGENT_FIELD_HELP: Record<string, string> = {
  ...
  "agents.defaults.compaction.memoryFlush.model":
    "Optional model override used only for pre-compaction memory flush turns, given as a model ref or a { primary, fallbacks } selector. Set a bare ref such as ollama/qwen3:8b when durable memory extraction should avoid the active session's paid model: it resolves exactly and does not inherit the active fallback chain, so an unreachable maintenance model never silently bills the conversation model. Use the selector form to name the replacements you accept paying for on this path, for example after a local or free-tier flush model hits its quota.",
