...
export const CORE_FIELD_HELP: Record<string, string> = {
  ...
  "meta.lastTouchedVersion": "OpenCrustacean version that most recently wrote this config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for OpenCrustacean. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion": "OpenCrustacean version used by the last wizard run.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping OpenCrustacean runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  "update.channel":
  ...
  "gateway.tailscale.serviceName":
    'Optional Tailscale Service name for Serve mode, such as "svc:openclaw". The value must use Tailscale\'s svc:<dns-label> format. When set, OpenCrustacean passes it to tailscale serve --service and reports the derived Service URL.',
  "gateway.tailscale.preserveFunnel":
    "When mode='serve' and an externally configured Tailscale Funnel route already covers the gateway port, skip re-applying tailscale serve on startup. Lets operators keep Funnel exposure managed outside OpenCrustacean without losing it across gateway restarts.",
  "gateway.remote":
  ...
  "talk.realtime.instructions":
    "Additional system instructions appended to OpenCrustacean's built-in realtime Talk prompt. Use this for voice style, tone, and other provider-facing realtime behavior while keeping agent-consult guidance intact.",
  ...
  "talk.realtime.consultRouting":
    "Gateway relay fallback for final user transcripts when the realtime provider skips openclaw_agent_consult. provider-direct preserves provider replies; force-agent-consult routes through OpenCrustacean.",
  ...
  "agents.entries.*.runtime":
    "Optional runtime descriptor for this agent. Use embedded for default OpenCrustacean execution or acp for external ACP harness defaults.",
  "agents.entries.*.runtime.type":
    'Runtime type for this agent: "embedded" (default OpenCrustacean runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.entries.*.runtime.acp.agent":
    "Optional ACP harness agent id to use for this OpenCrustacean agent (for example codex, claude, cursor, gemini, openclaw).",
  ...
  "agents.defaults.systemAgent":
    "Target settings for ambient OpenCrustacean system-agent and Custodian inference.",
