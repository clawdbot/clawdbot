...
describe("isPluginExplicitlySelectedByAlias", () => {
  ...
  // Codex P1 3846202570: the workspace and restrictive-allowlist gates above left one origin
  // uncovered — a BUNDLED plugin without default enablement carrying only material entry config
  // (`config`, hooks, an apiKey) reached the tail and read as hand-picked, even though startup
  // resolves it "bundled-disabled-by-default" (`config-activation-shared.ts`). The successor's
  // `preferOver` was then set aside and schema ownership could keep the inactive bundled fallback
  // while the successor registers the channel. Proven against the REAL bundled registry and the
  // real platform-aware default-enablement rule, not invented records.
  describe("bundled default-enablement gate", () => {
    const bundledRegistry = loadBundledPluginManifestRegistry();
    const bundledCanonicalId = createManifestPluginAliasResolver(bundledRegistry);
    const requireBundledRecord = (pluginId: string) =>
      expectDefined(
        bundledRegistry.plugins.find(
          (plugin) => plugin.id === pluginId && plugin.origin === "bundled",
        ),
        `real bundled plugin record for ${pluginId}`,
      );

    it("does not treat material entry config as selection for a bundled plugin startup leaves off", () => {
      const discord = requireBundledRecord("discord");
      // Fixture reachability through the real contracts: discord is a real bundled channel
      // plugin, and the rule startup consults reports it not enabled by default here.
      expect(discord.channels).toContain("discord");
      expect(isPluginEnabledByDefaultForPlatform(discord)).toBe(false);

      const config = {
        plugins: { entries: { discord: { config: { token: "zz-proof" } } } },
      } as unknown as OpenClawConfig;
      // The real activation decision under this exact config: startup leaves the plugin off, so
      // material entry config alone describes a plugin the runtime never loads.
      expect(
      ...
      expect(
        isPluginExplicitlySelectedByAlias(config, "discord", bundledCanonicalId, bundledRegistry),
      ).toBe(false);
    });

    it("still sees an entry enable as selection for that same bundled plugin", () => {
      const discord = requireBundledRecord("discord");
      const config = {
        plugins: { entries: { discord: { enabled: true, config: { token: "zz-proof" } } } },
      } as unknown as OpenClawConfig;
      // The enable really is the activation cause startup honors.
      expect(
        isActivatedManifestOwner({
          plugin: discord,
          normalizedConfig: normalizePluginsConfig(config.plugins),
    ...
    it("still sees material entry config as selection for a default-enabled bundled plugin", () => {
      const anthropic = requireBundledRecord("anthropic");
      // A default-enabled bundled plugin loads, so its material entry stays the operator's own
      // shaping of a plugin the runtime serves — the gate must not widen past activation.
      expect(isPluginEnabledByDefaultForPlatform(anthropic)).toBe(true);

      const config = {
        plugins: { entries: { anthropic: { config: { baseUrl: "https://zz.example" } } } },
      } as unknown as OpenClawConfig;
      expect(
        isActivatedManifestOwner({
          plugin: anthropic,
      ...
      expect(
        isPluginExplicitlySelectedByAlias(config, "anthropic", bundledCanonicalId, bundledRegistry),
      ).toBe(true);
    });
  });
});
