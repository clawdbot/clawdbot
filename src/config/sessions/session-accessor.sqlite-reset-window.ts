...
import { findLatestSessionTranscriptActiveBoundary } from "./session-accessor.sqlite-active-boundary.js";
...
function findLatestResetMessageWindow(
  ...
  const db = getResetWindowKysely(projection.database);
  const latestBoundaryRow = findLatestSessionTranscriptActiveBoundary(projection, [
    "reset",
    "compaction",
  ]);
