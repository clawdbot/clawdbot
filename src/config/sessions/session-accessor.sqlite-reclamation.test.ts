...
test.each([false, true])(
  ...
  async (rejected) => {
    ...
    const firstDiagnostics: SqliteSessionReclamationDiagnostics = {};
    const secondDiagnostics: SqliteSessionReclamationDiagnostics = {};
    ...
    try {
      ...
      const workerThreadId = observed.workers[0]?.threadId;
      expect(workerThreadId).toBeGreaterThan(0);
      ...
      expect(firstDiagnostics).toEqual({ kind: "history-eviction", workerThreadId });
      expect(secondDiagnostics).toEqual({ kind: "history-eviction", workerThreadId });
...
test("in-process reclamation and rejected worker construction do not invent a worker identity", async () => {
  ...
  const createWorker = archive.createSqliteTranscriptArchiveWorker;
  const construction = vi
    .spyOn(archive, "createSqliteTranscriptArchiveWorker")
    .mockImplementationOnce(() => createWorker({ notCloneable: () => undefined }));
  try {
    await expect(
      runSqliteSessionReclamation({ forceInProcess: false, plan, diagnostics: rejected }),
    ).rejects.toMatchObject({ name: "DataCloneError" });
    expect(rejected).toEqual({ kind: "history-eviction" });
  } finally {
