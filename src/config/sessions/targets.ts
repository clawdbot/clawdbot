...
import {
  resolveSameAgentDiscoveredSessionStoreTargetsSync,
  resolveValidatedDiscoveredStorePathSync,
  toDiscoveredSessionStoreTarget,
} from "./targets-agent-discovery.js";
import {
...
export function resolveExistingAgentSessionStoreTargetsSync(
  ...
  // Existing-store lookups stay agent-scoped: validate the deterministic target, then discover
  // only the requested agent's same-named stores under the template and default roots. Hot reads
  // no longer scan every on-disk store, while alternate retired/manual stores remain visible.
  const deterministicTarget = resolveValidatedExistingSessionStoreTargetSync(requestedTarget);
  // Pass every configured agent id so the helper derives roots from every template expansion,
  // not just the requested agent — a template with `{agentId}` before `agents/<agentId>` yields
  // one root per configured agent and an alternate same-agent store may live under another's root.
  const discoveredTargets = resolveSameAgentDiscoveredSessionStoreTargetsSync(
    cfg,
    requested,
    env,
  ...
  const targets = [...(deterministicTarget ? [deterministicTarget] : []), ...discoveredTargets];
  const uniqueTargets = [
    ...new Map(targets.map((target) => [path.resolve(target.storePath), target])).values(),
  ];
  // The deterministic store is the common single-candidate case. Returning it directly keeps hot
  // reads free of the all-agents registry ownership sweep; sqlite-target dedupe (and its registry
  // probes) runs only when a same-agent alternate store actually exists on disk.
  if (uniqueTargets.length <= 1) {
    return uniqueTargets;
  }
  return dedupeSessionStoreTargetsBySqliteTarget(uniqueTargets, { defaultAgentId, env });
}
