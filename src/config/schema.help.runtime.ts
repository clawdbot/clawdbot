...
export const RUNTIME_FIELD_HELP: Record<string, string> = {
  ...
  "tools.codeMode.enabled":
    'Enables generic code mode. Default is off. `"auto"` engages only models whose catalog flags `compat.codeMode: "preferred"`. `true` engages every tool-capable run and fails closed if the runtime is unavailable instead of exposing the full tool list.',
