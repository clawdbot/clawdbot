...
export const RUNTIME_FIELD_HELP: Record<string, string> = {
  ...
  "tools.agentToAgent":
    "Policy for cross-agent session tool calls: sessions_send to another agent, plus cross-agent list, history, search, and status reads under the default tools.sessions.visibility=all. Cross-agent access is on by default; use allow to restrict agent pairs or set enabled=false to keep agents isolated.",
  "tools.agentToAgent.enabled":
    "Enables cross-agent access through the session tools so one agent can message or read another agent's sessions (default: true). Set false to keep agents isolated; omitted or empty allow permits every agent pair.",
