...
export const RUNTIME_FIELD_HELP: Record<string, string> = {
  ...
  "gateway.nodes.commands.allow":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings; version aliases of one capability are authorized together). Enabling dangerous commands here is a security-sensitive override and is flagged by `openclaw security audit`.",
  "gateway.nodes.commands.deny":
    "Node command names to block even if present in node claims or default allowlist (exact command-name matching, except version aliases of one capability are revoked together; e.g. `system.run`; does not inspect shell text inside that command).",
  nodeHost:
