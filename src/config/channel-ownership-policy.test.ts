...
function policyFor(params: { config: OpenClawConfig; sourceConfig?: OpenClawConfig }) {
  return createConfiguredChannelOwnershipPolicy({
    ...
    // A test that supplies one config treats it as the authored view too, mirroring production
    // callers that hold only an authored config.
    sourceConfig: params.sourceConfig ?? params.config,
    registry,
    env: {},
  });
}
...
// A workspace claimant nothing trusts is the probe that can see ambient configuredness: on the
// ambient-configured channel it IS the auto-enable candidate and reads active, while on an
// unconfigured channel activation leaves it off ("workspace-disabled-by-default").
const AMBIENT_REGISTRY = {
  diagnostics: [],
  plugins: [{ id: "zz-claimant", origin: "workspace", channels: ["telegram"] }],
} as unknown as PluginManifestRegistry;
...
describe("createConfiguredChannelOwnershipPolicy", () => {
  ...
  // seen only through inherited environment variables counts as configured, so the workspace
  // claimant is the candidate auto-enable selects and loads; under "suppress" the channel is not
  // configured at all, and activation leaves the untrusted workspace claimant off. The claimant is
  // active in exactly one of the two, which is the disagreement this pins.
  it.each([
    {
      name: "allow narrows to the selected candidate",
      recorded: "allow" as const,
      active: true,
    },
    {
  ...
  it("reads suppress when no Gateway recorded a policy", () => {
    expect(ambientPolicyFor().isPluginActive("zz-claimant", "telegram")).toBe(false);
  });
  ...
  // Codex P1 3846202570-family, the unconfigured mirror of the narrowing above: the
  // unconfigured-channel arm reported every non-policy-disabled claimant active, so a preferred
  // replacement startup refuses to load — a workspace plugin nothing trusts, a bundled plugin
  // without default enablement — became the computed winner, `collectCededChannelIdsByPlugin`
  // ceded the loading fallback's registration to it, and the channel was left with no runtime
  // owner and only a post-load diagnostic.
  it.each([
    { name: "a workspace replacement nothing trusts", origin: "workspace" },
    { name: "a bundled replacement without default enablement", origin: "bundled" },
  ])("keeps $name from taking an unconfigured channel", ({ origin }) => {
    const unconfiguredRegistry = {
      diagnostics: [],
      plugins: [
        { id: "zzun-fallback", origin: "config", channels: ["zzunchat"] },
        {
          id: "zzun-replacement",
          origin,
          channels: ["zzunchat"],
          channelConfigs: { zzunchat: { preferOver: ["zzun-fallback"] } },
        },
      ],
    } as unknown as PluginManifestRegistry;
    // The channel is NOT configured: no channels section at all.
    const config = {} as OpenClawConfig;
    ...
    expect(policy.isPluginActive("zzun-replacement", "zzunchat")).toBe(false);
    expect(policy.isPluginActive("zzun-fallback", "zzunchat")).toBe(true);
    // Both planes follow the claimant startup actually loads, and the loader stands no cede: the
    // fallback's registration survives instead of yielding to a winner that never loads.
    const { winners } = collectRuntimeChannelOwnership(unconfiguredRegistry, policy);
    expect(winners.get("zzunchat")).toBe("zzun-fallback");
    const { cededChannelIdsByPlugin } = collectCededChannelIdsByPlugin({
      registry: unconfiguredRegistry,
      config,
      sourceConfig: config,
  ...
  // The arm's original allowance, preserved: an unconfigured channel materializes no candidate
  // set, and every claimant startup loads without candidacy — an installed plugin with no adverse
  // policy, a bundled default-enabled owner — must keep reading active there, with operator policy
  // disablement still the off switch.
  it.each([
    { name: "a config-origin claimant", record: { id: "zzun-extra", origin: "config" } },
    { name: "a global claimant", record: { id: "zzun-extra", origin: "global" } },
    {
      name: "a bundled default-enabled claimant",
      record: { id: "zzun-extra", origin: "bundled", enabledByDefault: true },
    },
  ])("still reports $name active on an unconfigured channel", ({ record }) => {
    const unconfiguredRegistry = {
      diagnostics: [],
      plugins: [{ ...record, channels: ["zzunchat"] }],
    } as unknown as PluginManifestRegistry;
    const config = {} as OpenClawConfig;
    const policy = createConfiguredChannelOwnershipPolicy({
      config,
    ...
    expect(policy.isPluginActive("zzun-extra", "zzunchat")).toBe(true);

    const disabledConfig = {
      plugins: { entries: { "zzun-extra": { enabled: false } } },
    } as unknown as OpenClawConfig;
    const disabledPolicy = createConfiguredChannelOwnershipPolicy({
      config: disabledConfig,
      sourceConfig: disabledConfig,
      registry: unconfiguredRegistry,
      env: {},
    });
    expect(disabledPolicy.isPluginActive("zzun-extra", "zzunchat")).toBe(false);
  });
});
