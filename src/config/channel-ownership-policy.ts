...
export function createConfiguredChannelOwnershipPolicy(params: {
   ...
   * for the plugins it turns on and for a replacement chain's displaced middle claimant, and
   * validation seeds `plugins.entries.<id>.config` from plugin config schemas, so reading the
   * materialized config would report every auto-enabled plugin as hand-picked and a synthesized
   * disable as operator policy. `disableImplicitPreferredOverPlugin` checks its own
   * `originalConfig` for the same reason. Required, not defaulted: a `?? config` fallback let a
   * runtime-shaped object silently stand in for the authored one, which reintroduced this exact
   * defect at each new construction site. A caller that genuinely holds only one, authored config
   * passes it as both halves and says so at the call site.
   */
  ...
  return {
    isPluginActive: (pluginId, channelId) => {
      ...
      if (!candidates) {
        ...
        // narrow with — there is no candidate set to compare against. But "no candidates" is not
        // "everything serves": a claimant startup refuses to load — a workspace plugin nothing
        // trusts, a bundled plugin without default enablement — never registers here either, and
        // reporting it active let `collectCededChannelIdsByPlugin` crown it winner and cede the
        // loading fallback's registration, stranding the channel with no owner at all. Ask the
        // same activation read the undeclared-channel arm below uses: every claimant the runtime
        // loads without candidacy (an installed plugin with no adverse policy, a bundled
        // default-enabled owner) stays active exactly as before, and policy disablement — checked
        // above — remains the operator-facing signal this arm has always honored.
 ...
 * policy itself: the policy module owns how explicit selection is read.
 *
 * `sourceConfig` is the authored counterpart of `config` and is required for the same reason it
 * is on the policy factory: validation of a runtime-shaped candidate (the `config.patch` flow
 * merges the patch into the previously validated snapshot) hands this function a `config` that
 * already carries validation-seeded `plugins.entries.<id>.config` records, and reading those as
 * operator selection set aside `preferOver` and validated against a schema startup never serves.
 */
