...
function ownershipPolicy(
  ...
  return createConfiguredChannelOwnershipPolicy({
    config,
    // Deliberately `config` as both halves when no authored snapshot exists: before a Gateway
    // publishes its source snapshot, and for the invalid-snapshot placeholder config, the one
    // config in hand IS the authored view — there is no runtime-materialized sibling to misread.
