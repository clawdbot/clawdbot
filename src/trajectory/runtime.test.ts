...
describe("trajectory runtime", () => {
  ...
  it("records only trusted target-session hashes and consumes snapshots once", () => {
    ...
    const expectedHash = expectedSessionHash(SOURCE_SESSION_HASH_DOMAIN, targetSessionKey);
  ...
  it("preserves target identity and outcome in oversized tool-result fallbacks", () => {
    ...
    expect(event.data).toMatchObject({
      ...
      isError: true,
      success: false,
      targetSessionHash: expectedSessionHash(SOURCE_SESSION_HASH_DOMAIN, targetSessionKey),
    });
