...
import { TailscaleRouteMigrationRequiredError } from "./tailscale-route-migration-error.js";
import {
...
function routeClaimError(message: TailscaleRouteOwnerFailure): Error {
  if (isPort443RouteConflict(message)) {
    return new TailscaleRouteMigrationRequiredError();
  }
...
export async function claimTailscaleRoute(
  ...
  return await withSudoFallback(
    () => startTailscaleRouteOwner([tailscaleBin, ...args]),
    () => startTailscaleRouteOwner(["sudo", "-n", tailscaleBin, ...args]),
  );
}
...
async function withSudoFallback<T>(run: () => Promise<T>, elevated: () => Promise<T>): Promise<T> {
  try {
    return await run();
  } catch (error) {
...
type LegacyTailscaleRouteInspection = "none" | "exact" | "conflict";

function hasTcp443(value: unknown): boolean {
  return Object.hasOwn(readRecord(readRecord(value)?.TCP) ?? {}, "443");
}

/** Classifies the persistent port-443 route left by older managed Serve/Funnel releases. */
function inspectLegacyTailscaleRoute(
  raw: string,
  mode: "serve" | "funnel",
  gatewayPort: number,
): LegacyTailscaleRouteInspection {
  let config: Record<string, unknown>;
  try {
    config = parsePossiblyNoisyJsonObject(raw);
  } catch {
    return "none";
  }
  if (!hasTcp443(config)) {
  ...
  if (
    Object.values(readRecord(config.Foreground) ?? {}).some(hasTcp443) ||
    Object.values(readRecord(config.Services) ?? {}).some(hasTcp443)
  ) {
    return "conflict";
  }

  const tcpHandler = readRecord(readRecord(config.TCP)?.["443"]);
  const webEntries = Object.entries(readRecord(config.Web) ?? {}).filter(([key]) =>
    key.endsWith(":443"),
  );
  if (!tcpHandler || webEntries.length !== 1) {
    return "conflict";
  }
  const host = webEntries[0]?.[0];
  const webServer = readRecord(webEntries[0]?.[1]);
  const handlers = readRecord(webServer?.Handlers);
  const rootHandler = readRecord(handlers?.["/"]);
  if (!webServer || !handlers || !rootHandler) {
    return "conflict";
  }
  const proxy = typeof rootHandler?.Proxy === "string" ? rootHandler.Proxy : "";
  const funnelEnabled = host ? readRecord(config.AllowFunnel)?.[host] === true : false;
  return Object.keys(tcpHandler).length === 1 &&
    tcpHandler.HTTPS === true &&
    Object.keys(webServer).length === 1 &&
    Object.keys(handlers).length === 1 &&
    Object.keys(rootHandler).length === 1 &&
    tailscaleProxyMatchesLoopbackPort(proxy, gatewayPort) &&
    funnelEnabled === (mode === "funnel")
    ? "exact"
    : "conflict";
}

/** Removes only the exact persistent route generated by pre-foreground OpenClaw releases. */
export async function reconcileLegacyTailscaleRoute(
  mode: "serve" | "funnel",
  gatewayPort: number,
): Promise<boolean> {
  const tailscaleBin = await getTailscaleBinary();
  let stdout: string;
  try {
    ({ stdout } = await runExec(tailscaleBin, ["serve", "status", "--json"], {
      timeoutMs: 5000,
      maxBuffer: 400_000,
      logOutput: false,
    }));
  ...
  const inspection = inspectLegacyTailscaleRoute(stdout, mode, gatewayPort);
  if (inspection === "none") {
    return false;
  }
  if (inspection === "conflict") {
    throw new TailscaleRouteMigrationRequiredError();
  }

  const args = [mode, "--yes", "--https=443", "--set-path=/", "off"];
  await withSudoFallback(
    () => runExec(tailscaleBin, args, { timeoutMs: TAILSCALE_ROUTE_START_TIMEOUT_MS }),
    () =>
      runExec("sudo", ["-n", tailscaleBin, ...args], {
        timeoutMs: TAILSCALE_ROUTE_START_TIMEOUT_MS,
      }),
  );
  return true;
}
