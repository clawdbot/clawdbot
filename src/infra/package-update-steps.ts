  ...
  if (siblingPackages.length > 0) {
    return {
      ...
      failedStep: {
        ...
        exitCode: 1,
        stdoutTail: null,
        stderrTail: `OpenCrustacean shares a pnpm ${owner.layoutVersion} global install group with ${siblingPackages.join(", ")}. Automatic update stopped before mutation; update the group manually to preserve its sibling packages.`,
      },
    };
  }
  ...
  if (!invokingPackageRoot || activePackageRoots.length !== 1 || ownerMatchCount !== 1) {
    return {
      ...
      failedStep: {
        ...
        exitCode: 1,
        stdoutTail: null,
        stderrTail: `Expected exactly one active pnpm ${owner.layoutVersion} OpenCrustacean install owned by the invoking project; found ${activePackageRoots.length} active installs and ${ownerMatchCount} owner matches. Automatic update stopped before mutation.`,
      },
    };
  }
  ...
  if (
    ...
    return {
      ...
      failedStep: {
        ...
        stderrTail: `The active pnpm command owns ${reportedGlobalRoot || "an unknown global root"}, not the invoking OpenCrustacean install at ${expectedGlobalRoot ?? "an unknown root"}. Automatic update stopped before mutation.`,
      },
    };
  }
  ...
  if (version?.major !== owner.layoutVersion) {
    return {
      ...
      failedStep: {
        ...
        stderrTail: `OpenCrustacean belongs to pnpm isolated layout v${owner.layoutVersion}, but the update command reports pnpm ${reportedVersion || "unknown"}. Use pnpm ${owner.layoutVersion} for this install or update it manually.`,
      },
    };
  }
