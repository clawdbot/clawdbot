...
// The split scheduler narrows wake requests before invoking runOnce. Keep the
// current request available only for that awaited handoff so lineage and trust survive.
let activeWakeContext: HeartbeatWakeRequest | null = null;
    ...
    void (async () => {
      ...
      try {
        for (const pendingWake of pendingBatch) {
          ...
          const previousWakeContext = activeWakeContext;
          activeWakeContext = wakeOpts;
          let res: HeartbeatRunResult;
          try {
            res = await runWithGatewayIndependentRootWorkAdmission(async () => active(wakeOpts));
          } finally {
            activeWakeContext = previousWakeContext;
          }
