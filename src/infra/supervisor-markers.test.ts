...
describe("detectRespawnSupervisor", () => {
  it("detects launchd from OpenCrustacean's explicit marker or current gateway launchd job", () => {
    expect(
  ...
  it("detects Linux OpenCrustacean gateway service markers only for opt-in callers", () => {
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ OPENCLAW_WINDOWS_TASK_NAME: "OpenCrustacean Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
      detectRespawnSupervisor(
        {
