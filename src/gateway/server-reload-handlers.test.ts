...
import type { fanInChannelIngressLifecycles } from "../plugin-sdk/channel-ingress-runtime.js";
...
describe("gateway restart deferral preflight", () => {
  ...
  it("admits newly allowed Discord messages during active work without restarting", async () => {
    ...
    setActivePluginRegistry(
      createTestRegistry([{ pluginId: "discord", plugin: discordPlugin, source: "synthetic" }]),
    );
    ...
    const startChannel = vi.fn(async () => new Map());
    const stopChannel = vi.fn(async () => {});
    const { applyHotReload } = createGatewayReloadHandlers({ startChannel, stopChannel });
    ...
    try {
      ...
      expect(hoisted.activeEmbeddedRunCount.value).toBe(1);
      expect(startChannel).not.toHaveBeenCalled();
      expect(stopChannel).not.toHaveBeenCalled();
    } finally {
      hoisted.activeEmbeddedRunCount.value = 0;
