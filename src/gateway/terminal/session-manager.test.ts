...
describe("TerminalSessionManager agent ownership", () => {
  ...
  it("closes all sessions for one agent owner with viewer cleanup symmetry", async () => {
    const emit = vi.fn();
    ...
    const first = await manager.open(baseRequest({ owner: runOwner }));
    const second = await manager.open(baseRequest({ owner: runOwner }));
    const persistent = await manager.open(baseRequest({ owner: persistentOwner }));
    const connection = await manager.open(
      baseRequest({ owner: { kind: "conn", connId: "connection-owner" } }),
    );
