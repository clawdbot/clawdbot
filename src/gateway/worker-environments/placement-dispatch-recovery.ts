import { WORKER_LAUNCH_V3_PROTOCOL_FEATURE } from "../../../packages/gateway-protocol/src/schema/worker-admission.js";
import {
...
function supportsWorkerLaunchV3(environment: ReturnType<WorkerEnvironmentService["get"]>): boolean {
  // A persisted bundle hash can still match a pre-v3 worker when current bundle preparation fails.
  // Require the admitted receipt so restart recovery never revives an incompatible launch contract.
  return (
    environment?.bootstrapReceipt?.protocolFeatures.includes(WORKER_LAUNCH_V3_PROTOCOL_FEATURE) ===
    true
  );
}
...
function sameActiveEnvironment(
  ...
  return Boolean(
    ...
    supportsWorkerLaunchV3(environment) &&
