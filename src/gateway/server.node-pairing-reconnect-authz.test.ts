// Paired node reconnect tests cover capability approval, reapproval, and
// metadata refresh behavior across authenticated Gateway handshakes.
...
const { cleanup: cleanupNodePairingTestState, setup: setupNodePairingTestState } =
  createNodePairingTestState("openclaw-node-pair-reconnect-authz-");

async function findPairedNode(nodeId: string) {
...
async function expectRePairingRequest(params: {
  ...
  try {
    ...
    await vi.waitFor(async () => {
      const list = await rpcReq<{ nodes?: NodeDiagnostics[] }>(connectedControlWs, "node.list", {});
...
describe("gateway paired node reconnect authorization", () => {
  beforeAll(async () => {
