...
export const createTalkClient: GatewayRequestHandler = async ({
  ...
  withPluginRegistryResourceOperationAsync(async () => {
    ...
    try {
      ...
      const mode =
        normalizeOptionalLowercaseString(params.mode) ?? realtimeConfig.mode ?? "realtime";
      if (mode !== "realtime") {
      ...
      if (resolution.provider.createBrowserSession && transport !== "gateway-relay") {
        ...
        const providerResources = requirePluginRegistryResourceScope().fork();
        const providerResourceLease = createPluginRegistryResourceLease(providerResources);
        let gatewayControlOwner: ReturnType<typeof createTalkClientGatewayControlOwner> | undefined;
        try {
          gatewayControlOwner = ownsProvider
            ? createTalkClientGatewayControlOwner({
                runWithProviderResources: providerResourceLease.run,
        ...
        let session:
          | Awaited<ReturnType<typeof resolution.provider.createBrowserSession>>
          | undefined;
        let delivered = false;
        try {
          assertCommitAllowed();
          session = await providerResourceLease.run(() =>
            resolution.provider.createBrowserSession!(browserSessionRequest),
          );
          ...
          if (gatewayControlOwner) {
            ...
            await gatewayControlOwner.adoptProvider(
              ...
              async () => {
                providerResourceLease.release();
                await providerResources.waitForDisposals();
              },
            );
          }
          assertCommitAllowed();
            ...
            if (!delivered) {
              try {
                if (gatewayControlOwner) {
                  ...
                  const createdSession = session;
                  await providerResourceLease.run(() =>
                    cancelInternalRealtimeVoiceBrowserSession({
                      provider: resolution.provider,
                      request: browserSessionRequest,
                      session: createdSession,
                ...
                context.logGateway.warn(
                  `talk browser session cleanup failed: ${formatForLog(error)}`,
                );
              }
            }
          } finally {
