...
async function auditGatewayRuntime(
  ...
  if (isBunRuntime(execPath)) {
    issues.push({
      ...
      message: "Gateway service uses Bun; OpenCrustacean runtime state requires node:sqlite.",
      detail: execPath,
      level: "recommended",
    });
    return;
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  ...
  issues.push({
    ...
    message: `Gateway service was installed by OpenCrustacean ${serviceVersion}; current CLI is ${VERSION}.`,
