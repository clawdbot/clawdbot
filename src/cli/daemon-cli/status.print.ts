...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean; deep?: boolean }) {
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      defaultRuntime.error(
        warnText(
          `Warning: this OpenCrustacean command is version ${status.cli.version}, but the running Gateway is version ${gatewayVersion}.`,
        ),
      );
      defaultRuntime.error(
        warnText(
          "Check `openclaw --version`, `which openclaw`, and `openclaw gateway status --deep`; if this mismatch is unexpected, update PATH so `openclaw` points to the version you want, or reinstall the Gateway service from that same OpenCrustacean install.",
        ),
      );
    }
    spacer();
  ...
  if (status.connections?.established.length) {
    ...
    defaultRuntime.log(
      warnText(
        "If logs show protocol mismatch after rollback, stop stale OpenCrustacean client processes listed here and re-run gateway status.",
      ),
    );
    spacer();
  }
  ...
  if (service.staleUpdateLaunchdJobs?.length) {
    defaultRuntime.error(errorText("Stale OpenCrustacean updater launchd job(s) detected."));
