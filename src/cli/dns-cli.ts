  ...
  dns
    ...
    .action(async (opts) => {
      ...
      if (!cfg.discovery?.wideArea?.domain?.trim()) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: set discovery.wideArea.domain in the active OpenCrustacean config ($OPENCLAW_CONFIG_PATH, default ~/.openclaw/openclaw.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
