...
describe("update-cli", () => {
  ...
  it("records ordered update phases across service stop, restart, and verified health", async () => {
    mockOwnedGitService();
    mockGitUpdateAfterMutation(
      makeOkUpdateResult({ before: { version: "0.9.0" }, after: { version: "1.0.0" } }),
    );
    mockCurrentProcessFreshDoctor();
    serviceLoaded.mockResolvedValue(true);
    vi.mocked(runDaemonRestart).mockResolvedValue(true);
  ...
  it("continues a package update when target schemas are compatible", async () => {
    ...
    const checks = databasePreflightMocks.preflightOpenClawDatabaseSchemas;
    expect(checks).toHaveBeenCalled();
    expect(checks.mock.calls.every(([input]) => input.supportedVersions.agent === 11)).toBe(true);
    const installOrder =
      vi.mocked(runCommandWithTimeout).mock.invocationCallOrder[
        commandCalls().findIndex(
          ([argv]) => argv[0] === "npm" && argv[1] === "i" && argv[2] === "-g",
        )
      ];
    expect(checks.mock.invocationCallOrder.at(-1)).toBeLessThan(installOrder!);
  ...
  it("refuses a package target that changes after the service stops", async () => {
    ...
    expect(serviceStop).toHaveBeenCalledOnce();
    const checks = databasePreflightMocks.preflightOpenClawDatabaseSchemas.mock;
    expect(
      checks.calls.some(
        ([input], index) =>
          checks.invocationCallOrder[index]! > serviceStop.mock.invocationCallOrder[0]! &&
          input.env.OPENCLAW_STATE_DIR === profileStateDir(),
      ),
    ).toBe(true);
    expect(packageInstallCommandCall()).toBeUndefined();
    expect(freshRestartCalls()).toEqual([
  ...
  it.each<{
    ...
    async ({ platform, env, supervisor, options = {}, ancestor = false, git = false }) => {
      ...
      expect(phases.mock.calls.map(([params]) => params.phase)).toEqual(
        git
          ? ["inspect", "inspect", "inspect", "prepare"]
          : ["inspect", "inspect", "inspect", "inspect", "prepare"],
      );
  ...
  it("reports a Git service refusal at the final ancestry recheck without an unsafe recovery verdict", async () => {
    ...
    let ancestryReads = 0;
    mockGetSelfAndAncestorPidsSync.mockImplementation(() => {
      ancestryReads += 1;
  ...
  it("keeps Windows Scheduled Task autostart disabled after unverified lifecycle failure", async () => {
    ...
    mockPackageReplacementFailure("update invariant broke", () =>
      fs.unlink(path.join(root, "dist", "index.js")),
    );
    ...
    try {
      ...
      await expect(fs.access(path.join(root, "dist", "index.js"))).rejects.toMatchObject({
        code: "ENOENT",
      });
      expect(serviceRestart).not.toHaveBeenCalled();
    } finally {
  ...
  it.each(
    ...
    async ({ signal, phase }) => {
      ...
      if (phase === "schema preflight") {
        ...
        const schemas = await import("./update-cli/schema-preflight.js");
        const check = schemas.checkTargetDatabaseSchemasForContexts;
        let observed = false;
        vi.spyOn(schemas, "checkTargetDatabaseSchemasForContexts").mockImplementation(
          async (...args) => {
            if (!observed && suspendScheduledTaskAutoStartForUpdate.mock.calls.length > 0) {
              observed = true;
              await waitForSignal();
            }
            return check(...args);
  ...
  it("keeps an owned service stopped after an in-place package-to-Git install failure", async () => {
