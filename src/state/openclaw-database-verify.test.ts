...
describe("OpenCrustacean database integrity verifier", () => {
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const targets: OpenClawDatabaseVerifyTarget[] = [
        { kind: "agent", label: "OpenCrustacean agent database worker-1", path: agent.path },
      ];
  ...
  it("detects corruption off-thread, quarantines it, and latches later opens", async () => {
    ...
    const targets: OpenClawDatabaseVerifyTarget[] = [
      { kind: "agent", label: "OpenCrustacean agent database worker-1", path: agentPath },
    ];
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const targets: OpenClawDatabaseVerifyTarget[] = [
        { kind: "state", label: "OpenCrustacean state database", path: state.path },
      ];
      const results = await verifyOpenClawDatabases(targets);
