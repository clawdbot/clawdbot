...
function splitBoardSchema(sql: string): { board: string; withoutBoard: string } {
  ...
  if (start === -1 || end === -1) {
    throw new Error(
      "OpenCrustacean agent board schema markers are missing from the canonical schema.",
    );
  }
  return {
...
function canonicalBoardWidgetsCreateSql(): string {
  ...
  if (start === -1 || end === -1) {
    throw new Error("OpenCrustacean agent board widget schema markers are missing.");
  }
...
function legacyBoardWidgetsCreateSql(): string {
  ...
  if (legacy === canonical) {
    throw new Error("OpenCrustacean agent board widget legacy schema derivation failed.");
  }
  return legacy;
}
...
export function ensureOpenClawAgentBoardSchemaInTransaction(db: DatabaseSync): void {
  ...
  if (typeof row?.sql !== "string") {
    throw new Error("OpenCrustacean agent board widget schema is missing after ensure.");
  }
  ...
  if (normalizedSchema !== normalizeBoardWidgetsCreateSql(legacyBoardWidgetsCreateSql())) {
    throw new Error(
      "OpenCrustacean agent board widget schema has an unsupported content-kind constraint.",
    );
  }
  ...
  if (existingMigrationTable) {
    throw new Error(
      `OpenCrustacean agent board migration table already exists: ${BOARD_WIDGETS_MIGRATION_TABLE}`,
    );
  }
