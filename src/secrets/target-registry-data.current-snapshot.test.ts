...
describe("getSecretTargetRegistry metadata reuse", () => {
  ...
  it.each(["configure", "manual apply", "legacy saved apply", "plugin setup"] as const)(
    "keeps plugin IDs, array indices, and record keys distinct from discovery through %s",
    async (flow) => {
      ...
      const arrayTarget = plan.targets.find(
        (target) => target.pathSegments?.[2] === "array-plugin",
      )!;
      ...
      expect(
        resolveValidatedPlanTarget({ ...legacyArrayTarget, pathSegments: undefined }),
      ).toBeNull();
      expect(
      ...
      if (flow === "plugin setup") {
        ...
        expect(() =>
          buildPluginSecretRefSetupPlan({
            ...
            providerSecrets: [],
            configTargetSecrets: [{ path: quotedArrayPath, secretId: "credentials/forged" }],
          }),
        ).toThrow(/Unknown or unsupported/);
        ...
        flow === "legacy saved apply"
          ...
          : flow === "manual apply"
            ? {
                ...plan,
                generatedBy: "manual" as const,
                targets: plan.targets.map(({ type, path: targetPath, ref }) => ({
                  type,
                  path: targetPath,
                  ref,
                })),
            ...
            : plan;
      expect(isSecretsApplyPlan(applyPlan)).toBe(true);
      ...
      for (const scenario of [
        {
          name: "explicit array creates an array",
          pluginId: "array-plugin",
          target: arrayTarget,
          initialConfig: {},
          expectedAccounts: [{ token: arrayTarget.ref }],
        },
        {
          name: "explicit array rejects a numeric-key record",
          pluginId: "array-plugin",
          target: arrayTarget,
          initialConfig: { accounts: { "0": { token: "untouched" } } },
          reject: true,
        },
        {
          name: "legacy dotted array index creates an array",
          pluginId: "array-plugin",
          target: legacyArrayTarget,
          initialConfig: {},
          expectedAccounts: [{ token: arrayTarget.ref }],
        },
        {
          name: "legacy dotted array index rejects a numeric-key record",
          pluginId: "array-plugin",
          target: legacyArrayTarget,
          initialConfig: { accounts: { "0": { token: "untouched" } } },
          reject: true,
        ...
        {
          name: "quoted numeric key creates a record",
          pluginId: "record-plugin",
          target: recordTarget,
          initialConfig: {},
          expectedAccounts: { "0": { token: recordTarget.ref } },
        },
        {
          name: "quoted numeric key rejects an array",
          pluginId: "record-plugin",
          target: recordTarget,
          initialConfig: { accounts: [{ token: "untouched" }] },
          reject: true,
        },
        {
          name: "wildcard array capture creates an array",
          pluginId: "wildcard-array-plugin",
          target: wildcardArrayTarget,
          initialConfig: {},
          expectedAccounts: [{ token: wildcardArrayTarget.ref }],
        },
        {
          name: "wildcard array capture rejects a numeric-key record",
          pluginId: "wildcard-array-plugin",
          target: wildcardArrayTarget,
          initialConfig: { accounts: { "0": { token: "untouched" } } },
          reject: true,
        ...
        {
          name: "legacy dotted numeric record key remains a record",
          pluginId: "record-plugin",
          target: legacyRecordTarget,
          initialConfig: {},
          expectedAccounts: { "0": { token: recordTarget.ref } },
        },
        {
          name: "legacy dotted numeric record key rejects an array",
          pluginId: "record-plugin",
          target: legacyRecordTarget,
          initialConfig: { accounts: [{ token: "untouched" }] },
          reject: true,
        ...
        const scenarioConfig = {
          plugins: {
            load: { paths: plugins.map((plugin) => plugin.rootDir) },
            entries: {
              [scenario.pluginId]: { enabled: true, config: scenario.initialConfig },
            },
          },
        };
        const originalConfig = JSON.stringify(scenarioConfig);
        fs.writeFileSync(configPath, originalConfig);
        const scenarioPlan = { ...plan, targets: [scenario.target] };

        if (scenario.reject) {
          await expect(
            testing.projectConfigForTest({ plan: scenarioPlan, env }),
            scenario.name,
          ).rejects.toThrow(/Invalid path shape/);
          expect(fs.readFileSync(configPath, "utf8"), scenario.name).toBe(originalConfig);
          continue;
        }

        const scenarioProjection = await testing.projectConfigForTest({
          plan: scenarioPlan,
          env,
        });
        expect(
          scenarioProjection.plugins?.entries?.[scenario.pluginId]?.config?.accounts,
          scenario.name,
        ).toEqual(scenario.expectedAccounts);
      }
    },
  );
