...
export async function buildGatewayInstallPlan(params: {
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${OPENCLAW_WRAPPER_ENV_KEY} because it points to the Windows task script; using the OpenCrustacean gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
