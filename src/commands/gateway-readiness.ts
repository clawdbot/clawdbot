...
export async function ensureGatewayReadyForOperation(
  ...
  const prompt = shouldInstall
    ? `Gateway is not installed. Install and start it now so OpenCrustacean can ${options.operation}?`
    : `Gateway is not running. Start it now so OpenCrustacean can ${options.operation}?`;
