...
const refreshRunningGatewayAfterLogin = async (agentId: string): Promise<ModelAuthRefreshOutcome> =>
...
async function finalizeProviderLogin(
  ...
  const modelAccess =
    result.profiles.length > 0
      ...
      : "failed";
  const authRefresh = await (params.refreshAuthState ?? refreshRunningGatewayAfterLogin)(
    params.agentId,
  );
