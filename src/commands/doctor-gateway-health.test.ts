  ...
  it("lists every degraded SecretRef owner reported by Gateway status", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "  Retry: openclaw secrets reload",
        "- cold plugin-capability:alpha%3Abeta:gamma (plugins.entries.alpha:beta.config.capabilityToken): secret reference was not found",
        "  Retry: openclaw secrets reload",
        "- cold plugin-provider:alpha:beta%3Agamma (plugins.entries.alpha.config.providerToken): secret provider failed",
        "  Retry: openclaw secrets reload",
        "- cold plugin-route:webhooks:routes.a.secret (plugins.entries.webhooks.config.routes.a.secret): secret reference was not found",
        "  Retry: openclaw secrets reload",
      ].join("\n"),
