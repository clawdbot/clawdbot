...
function closeCanonicalRepairTestDatabases(): void {
  closeOpenClawAgentDatabasesForTest();
  closeOpenClawStateDatabaseForTest();
}

async function withCanonicalRepairStateDir<T>(
  prefix: string,
  fn: (ctx: { stateDir: string; tempRoot: string }) => Promise<T>,
): Promise<T> {
  return withStateDirEnv(prefix, async (ctx) => {
    try {
      return await fn(ctx);
    } finally {
      closeCanonicalRepairTestDatabases();
    }
  });
}

afterEach(closeCanonicalRepairTestDatabases);
...
describe("doctor canonical session-key retention repair", () => {
  it("copies only a cross-store winner and archives its stale same-store duplicate", async () => {
    await withCanonicalRepairStateDir(
      "openclaw-doctor-canonical-cross-store-",
      async ({ stateDir }) => {
        ...
        expect(
          ...
          previous_session_id: "destination-only-previous",
