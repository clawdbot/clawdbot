...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.openclaw/bin/ensure-whatsapp.sh` is not maintained by current OpenCrustacean and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
