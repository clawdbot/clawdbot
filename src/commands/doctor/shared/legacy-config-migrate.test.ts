...
describe("legacy memory search config migrate", () => {
  ...
  it("migrates authored agents.entries legacy memory slots and preserves keyed roster shape", () => {
    const raw = {
      agents: {
        entries: {
          main: {
            default: true,
            plugins: { slots: { memory: "entry-memory" } },
          },
        },
      },
    };
    ...
    expect(findLegacyConfigIssues(raw)).toEqual([
      expect.objectContaining({
        path: "agents.entries",
        message: expect.stringContaining("agents.entries.*.plugins.slots.memory is legacy"),
      }),
    ]);
    const res = migrateLegacyConfigForTest(raw);

    expect(res.config?.agents?.entries?.main?.plugins?.slots).toEqual({
      "memory.recall": "entry-memory",
    });
    expect(Object.prototype.propertyIsEnumerable.call(res.config?.agents ?? {}, "list")).toBe(
      false,
    );
    expect(res.changes).toEqual([
      "Moved agents.entries.main.plugins.slots.memory → memory.recall.",
    ]);
    expect(migrateLegacyConfigForTest(res.config)).toEqual({ config: null, changes: [] });
  });

  it("migrates agents.list compatibility input without converting roster shape", () => {
    const raw = {
      agents: {
        list: [
          {
            id: "main",
            plugins: { slots: { memory: "list-memory" } },
          },
        ],
      },
    };
    ...
    expect(findLegacyConfigIssues(raw)).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          path: "agents.list",
          message: expect.stringContaining("agents.list[].plugins.slots.memory is legacy"),
        }),
      ]),
    );
    const res = migrateLegacyConfigForTest(raw);
    ...
    expect(res.config?.agents?.entries?.main?.plugins?.slots).toEqual({
      "memory.recall": "list-memory",
    });
    expect(res.changes).toEqual(["Moved agents.list.0.plugins.slots.memory → memory.recall."]);
    expect(migrateLegacyConfigForTest(res.config)).toEqual({ config: null, changes: [] });
  });
