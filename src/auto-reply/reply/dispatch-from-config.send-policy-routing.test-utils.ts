...
describe("sendPolicy deny — suppress delivery, not processing (#53328)", () => {
  ...
  it.each([
    {
      name: "suppresses marked runtime failure notices for room events under always policy",
    ...
    {
      name: "suppresses marked runtime failure notices for room events under once policy",
      text: "⚠️ You've reached your Codex subscription usage limit.",
      command: {},
      policy: "once",
      queuedFinal: false,
  ...
  ] as const)("$name", async ({ text, command, policy, queuedFinal }) => {
    setNoAbort();
