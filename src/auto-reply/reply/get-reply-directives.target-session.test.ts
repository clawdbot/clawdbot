...
async function resolveHelloWithModelDefaults(params: {
  ...
  const result = await resolveReplyDirectives({
    ...
    preparedDefaultModel: { provider: "openai", model: "gpt-4o-mini" },
    preparedInitialModel: {
      provider: params.provider ?? "openai",
      model: params.model ?? "gpt-4o-mini",
    },
    preparedPrimaryModel: { provider: "openai", model: "gpt-4o-mini" },
    provider: params.provider ?? "openai",
    model: params.model ?? "gpt-4o-mini",
...
describe("resolveReplyDirectives", () => {
  ...
  it("prefers the target session entry from sessionStore for directive state", async () => {
    ...
    const result = await resolveReplyDirectives({
      ...
      aliasIndex: { byAlias: new Map(), byKey: new Map() },
      preparedDefaultModel: { provider: "openai", model: "gpt-4o-mini" },
      preparedInitialModel: { provider: "openai", model: "gpt-4o-mini" },
      preparedPrimaryModel: { provider: "openai", model: "gpt-4o-mini" },
      provider: "openai",
      model: "gpt-4o-mini",
      hasResolvedHeartbeatModelOverride: false,
