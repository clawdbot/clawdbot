...
import {
  ...
  isFollowupRunAwaitingAdmission,
...
export async function dropAbortedFollowups(
  ...
  // A drain reservation can still await admission. Admission attempts and started
  // injections retain custody until their outcome.
  const canDrop = (run: FollowupRun) =>
    run.steerPending?.phase !== "injecting" &&
    isFollowupRunAborted(run) &&
    (!queue.inFlight.has(run) || isFollowupRunAwaitingAdmission(run)) &&
    !queue.activeSummarySources.has(run);
  const pending = queue.items.filter(canDrop);
  // Reserved callbacks already own their cleanup, even if they have not admitted yet.
  const pendingCleanup = pending.filter((run) => !queue.inFlight.has(run));
  const summaries = [
    ...queue.summarySources,
