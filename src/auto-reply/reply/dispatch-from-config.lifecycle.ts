...
export function createDispatchReplyOperationCoordinator(params: {
  ...
  const queuedLifecycleSignal = params.replyOptions?.turnAdoptionLifecycle?.abortSignal;
  const queuedSourceSignal = params.replyOptions?.abortSignal;
  const queuedAbortSignal =
    queuedLifecycleSignal && queuedSourceSignal && queuedLifecycleSignal !== queuedSourceSignal
      ? AbortSignal.any([queuedLifecycleSignal, queuedSourceSignal])
      : (queuedLifecycleSignal ?? queuedSourceSignal);
  const getQueuedFollowupAbortSignal = () =>
    queuedLifecycleSignal
      ? queuedAbortSignal
      : (dispatchReplyOperation?.abortSignal ?? queuedSourceSignal);
