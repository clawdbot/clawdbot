...
describe("runMemoryFlushIfNeeded", () => {
  ...
  it.each(["within cleanup", "past cleanup"] as const)(
    "keeps accepted accounting ahead of preflight when memory unwind finishes %s",
    async (unwind) => {
      const storePath = path.join(rootDir, "sessions.json");
      const sessionKey = "main";
      const entry = createFlushSessionEntry({
        totalTokens: 100_000,
        lifecycleRevision: "memory-cleanup-generation",
      });
      const sessionStore = { [sessionKey]: entry };
      await writeTestSessionStore(storePath, sessionKey, entry);
      registerMemoryFlushPlanResolverForTest(() =>
        createModifiedMemoryFlushPlan({ reserveTokensFloor: 10, softThresholdTokens: 0 }),
      );
      incrementCompactionCountMock.mockImplementation(incrementCompactionCount);
      const compactionCommitted = createDeferred();
      const releaseExecution = createDeferred();
      const memoryUnwound = createDeferred();
      clearAgentRunContextMock.mockImplementationOnce(() => memoryUnwound.resolve());
      runEmbeddedAgentMock.mockImplementationOnce(async (params: EmbeddedAgentParams) => {
        await commitMemoryCompaction({ sessionKey, storePath, run: params });
        compactionCommitted.resolve();
        await releaseExecution.promise;
        return { payloads: [], meta: {} };
      });
      const preflightCounts: Array<number | undefined> = [];
      compactEmbeddedAgentSessionMock.mockImplementationOnce(async () => {
        preflightCounts.push(loadMainSessionEntry(storePath).compactionCount);
        return { ok: true, compacted: true, result: { tokensAfter: 42 } };
      });
      const followupRun = createTestFollowupRun({ timeoutMs: 1_000, workspaceDir: rootDir });
      const queue = createTestIngressQueue(rootDir);
      await queue.enqueue("memory-owner", { text: "continue after maintenance" });
      const drain = createChannelIngressDrain({
        ...
        dispatchClaimedEvent: (_event, lifecycle) =>
          withIngressProcessingScope(lifecycle.abortSignal, async () => {
            const memory = await runDefaultMemoryFlush(entry, {
              followupRun,
              sessionStore,
              sessionKey,
              storePath,
            ...
            if (!memory.sessionEntry) {
              throw new Error("Memory failure did not return its persisted session");
            }
            await runDefaultPreflight(memory.sessionEntry, {
              followupRun,
              sessionStore,
              sessionKey,
              storePath,
              modelContextTokens: 100,
              promptForEstimate: "A pending user message that needs compaction. ".repeat(20),
              abortSignal: lifecycle.abortSignal,
            });
      ...
      try {
        ...
        await compactionCommitted.promise;
        await vi.advanceTimersByTimeAsync(1_001);

        expect.soft(preflightCounts).toEqual([]);
        expect.soft(await queue.listClaims()).toHaveLength(1);
        expect.soft(loadMainSessionEntry(storePath).compactionCount).toBe(1);

        if (unwind === "past cleanup") {
          ...
          expect(await queue.listPending()).toMatchObject([
            {
              id: "memory-owner",
              attempts: 1,
              lastError: expect.stringContaining("Memory flush timed out"),
            },
          ]);
          expect(preflightCounts).toEqual([]);
        }

        releaseExecution.resolve();
        await memoryUnwound.promise;
        await drain.waitForIdle();
        expect(runEmbeddedAgentMock).toHaveBeenCalledOnce();
        if (unwind === "within cleanup") {
          expect(preflightCounts).toEqual([2]);
          expect(loadMainSessionEntry(storePath)).toMatchObject({
            sessionId: "session-rotated",
            compactionCount: 3,
          ...
          expect((await queue.enqueue("memory-owner", { text: "redelivery" })).kind).toBe(
            "completed",
          );
        } else {
          expect(preflightCounts).toEqual([]);
