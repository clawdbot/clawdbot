...
import { awaitIngressProcessing } from "../../channels/message/ingress-processing-handoff.js";
...
export async function runReplyAgent(
  ...
  const sendDirectCompactionNotice = shouldNotifyUserAboutCompaction(cfg)
    ? async (phase: CompactionNoticePhase, text?: string) => {
        ...
        await awaitIngressProcessing(async (abortSignal) => {
          try {
            await onBlockReply(noticePayload, { abortSignal });
          } catch (err) {
