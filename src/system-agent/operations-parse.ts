...
export function parseSystemAgentOperation(input: string): SystemAgentOperation {
  ...
  switch (lower) {
    ...
    case "quit":
    case "exit":
      return { kind: "none", message: "OpenCrustacean retracts into shell. Bye." };
    default:
      break;
  }
...
export function describeSystemAgentPersistentOperation(operation: SystemAgentOperation): string {
  switch (operation.kind) {
    ...
    case "doctor-fix":
      return "exit OpenCrustacean and run openclaw doctor --fix";
    case "plugin-install":
...
function formatSetupPlanDescription(
  ...
  return `bootstrap OpenCrustacean setup for workspace ${workspace}`;
}
