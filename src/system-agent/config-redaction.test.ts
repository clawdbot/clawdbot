...
describe("isSystemAgentSensitiveConfigPathEmbedding", () => {
  ...
  // Codex review P2 on #128904: the schema collector keys channel metadata under the canonical
  // id, so a plugin declaring a case-preserving channel stores `acmechat` while the operator's
  // authored `channels.AcmeChat` was tested raw. The configured owner then read as unknown, which
  // is not a cosmetic miss -- it redacts the whole channel block out of model-visible `config-get`
  // and marks its ordinary paths sensitive.
  function installCasedChannelOwner() {
    type ManifestPlugin = PluginManifestRegistry["plugins"][number];
    const plugin = {
      id: "zzproof-cased",
      origin: "workspace",
        ...
        AcmeChat: {
          schema: {
            ...
            properties: {
              ...
              glorpValue: { type: "string" },
              webhookUrl: { type: "string" },
            },
            additionalProperties: false,
          },
          uiHints: {
            glorpValue: { sensitive: true },
            webhookUrl: { tags: ["url-secret"] },
          },
        },
      },
    } as unknown as ManifestPlugin;
    const effective = {
      channels: { AcmeChat: { accountLabel: "acme", glorpValue: "sauce" } },
      plugins: { entries: { "zzproof-cased": { enabled: true } } },
    } as unknown as OpenClawConfig;

    pluginMetadata?.restore();
    clearRuntimeConfigSnapshot();
    setRuntimeConfigSnapshot(effective, structuredClone(effective));
    pluginMetadata = installSystemAgentPluginMetadataTestSnapshot(effective, {
      manifestPlugins: [plugin],
    });
  }

  it("recognises a configured owner declared under a case-preserving channel id", () => {
    installCasedChannelOwner();

    // The declared spelling is the one runtime reads, so its block survives intact...
    expect(
      redactSystemAgentConfig({
        channels: { AcmeChat: { accountLabel: "acme" } },
      } as unknown as OpenClawConfig),
    ).toEqual({ channels: { AcmeChat: { accountLabel: "acme" } } });
    expect(redactSystemAgentConfigPath("channels.AcmeChat.accountLabel")).toBe(
      "channels.AcmeChat.accountLabel",
    );
  });

  // Admitting the owner removed the unknown-owner fallback that had been hiding this channel's
  // fields wholesale. Hints are stored canonically (`channels.acmechat.*`), so a lookup with the
  // authored segment finds nothing and a field the plugin marked sensitive reaches model-visible
  // operation text in plaintext. Both spellings must stay covered: the declared one through the
  // canonical hint lookup, the inert one through the unknown-owner fallback it still falls into.
  // The `sensitive: true` lookup and the url-secret lookup are separate reads of the same hints,
  // and canonicalising only the first left a credential embedded in a URL visible under the
  // declared spelling.
  it("keeps a url-secret-tagged cased channel field out of operation text", () => {
    installCasedChannelOwner();

    expect(
      isSystemAgentSensitiveConfigValue(
        "channels.AcmeChat.webhookUrl",
        "https://gateway.invalid/hook?token=synthetic",
      ),
    ).toBe(true);
  });

  // Two plugins spelling one canonical id differently: only one supplies the schema. Admission is
  // scoped to that owner, so the non-owner's spelling stays an unknown owner and is redacted whole
  // rather than being read against hints that do not describe it. The field carries no hint from
  // either side, so the two spellings can only differ by whether the spelling was admitted at all.
  it("admits only the spelling belonging to the channel's schema owner", () => {
    type ManifestPlugin = PluginManifestRegistry["plugins"][number];
    const makePlugin = (id: string, declared: string) =>
      ({
        id,
        origin: "workspace",
        channels: [declared],
        channelConfigs: {
          [declared]: {
            schema: {
              type: "object",
              properties: { plainValue: { type: "string" } },
              additionalProperties: false,
            },
          },
        },
    ...
    const effective = {
      channels: { acmechat: { plainValue: "a" }, AcmeChat: { plainValue: "b" } },
      plugins: { entries: { "zzproof-owner": { enabled: true } } },
    } as unknown as OpenClawConfig;
    ...
    pluginMetadata = installSystemAgentPluginMetadataTestSnapshot(effective, {
      manifestPlugins: [
        makePlugin("zzproof-owner", "acmechat"),
        makePlugin("zzproof-other", "AcmeChat"),
      ],
    });

    // The schema owner's own spelling is admitted and its unmarked field reads as ordinary.
    expect(isSystemAgentSensitiveConfigValue("channels.acmechat.plainValue", "a")).toBe(false);
    // The non-owner's spelling is not admitted, so it falls back to unknown-owner redaction.
    expect(isSystemAgentSensitiveConfigValue("channels.AcmeChat.plainValue", "b")).toBe(true);
  });

  it.each(["AcmeChat", "ACMECHAT"])(
    "keeps a declared-sensitive channel field out of operation text under %s",
    (authoredChannelId) => {
      installCasedChannelOwner();

      expect(
        isSystemAgentSensitiveConfigValue(`channels.${authoredChannelId}.glorpValue`, "new-sauce"),
      ).toBe(true);
    },
  );
