...
import {
  ...
  normalizePreparedProviderModelRef,
...
async function prepareSetupProviderAuthChoice(
  params: SetupInferencePlanInput,
  choice: ProviderAuthChoiceMetadata,
  handles: ReturnType<typeof acquirePluginProvidersCore>[],
) {
  ...
  return await withPluginLifecycleLease({ signal: params.signal }, async () => {
    ...
    const handle = (params.deps.acquirePluginProviders ?? acquirePluginProvidersCore)({
...
export async function buildTestPlan(
  ...
  let pendingAuth: Promise<unknown> | undefined;
  try {
    ...
    if (providerAutoChoiceId) {
      ...
      const providerChoice = await prepareSetupProviderAuthChoice(params, choice, providerHandles);
      ...
      case "provider-auth": {
        ...
        if (interactive && authChoice && managedWizardChoice) {
          ...
          const prepared = await prepareAuthChoiceLoadedPluginProvider({
            authChoice,
            projectStarterModelRef: normalizePreparedProviderModelRef,
            config: cfg,
            prompter: params.prompter,
            runtime: params.runtime,
            agentDir: params.agentDir,
          ...
          return buildPreparedProviderTestPlan({
            ...
            targetModelRef: prepared.projectedModelRef,
        ...
        const providerChoice = await prepareSetupProviderAuthChoice(
          params,
          choice,
          providerHandles,
        );
        ...
        try {
          if (interactive) {
            ...
            throwIfSetupInferenceCancelled(params);
            const auth = runProviderPluginAuthMethodUnpersisted({
              config: enableResult.config,
              runtime: params.runtime,
              ...(params.signal ? { signal: params.signal } : {}),
              isRemote: params.isRemoteProviderAuth,
            ...
            pendingAuth = auth;
            result = await waitForProviderAuth(auth, params.signal);
            throwIfSetupInferenceCancelled(params);
        ...
        return buildPreparedProviderTestPlan({
          ...
          ...(interactive && choice.appGuidedDiscovery === true
            ? {}
            : {
                targetModelRef: normalizePreparedProviderModelRef(modelRef, resolved.provider),
    ...
    // Cancellation may finish before provider I/O; resources follow actual completion.
    if (pendingAuth) {
      void pendingAuth.then(release, release);
    } else {
      release();
    }
  }
