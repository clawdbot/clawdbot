...
describe("manifest model catalog planner", () => {
  ...
  it.each([
    { manifestStatus: "deprecated", remoteStatus: "available" },
    { manifestStatus: "deprecated", remoteStatus: "preview" },
    { manifestStatus: "disabled", remoteStatus: "available" },
    { manifestStatus: "disabled", remoteStatus: "preview" },
  ] as const)(
    "prevents a remote $remoteStatus row from reviving a manifest $manifestStatus model",
    ({ manifestStatus, remoteStatus }) => {
      const plan = planManifestModelCatalogRows({
        registry: {
          plugins: [
            {
  ...
  it.each([
    { manifestStatus: "available", remoteStatus: "deprecated" },
    { manifestStatus: "preview", remoteStatus: "disabled" },
    { manifestStatus: "preview", remoteStatus: "available" },
    { manifestStatus: "available", remoteStatus: "preview" },
  ] as const)(
    "applies a newer remote $remoteStatus lifecycle to a manifest $manifestStatus model",
    ({ manifestStatus, remoteStatus }) => {
      const plan = planManifestModelCatalogRows({
        registry: {
          plugins: [
                    ...
                    models: [
                      {
                        id: "changing",
                        status: manifestStatus,
                        statusReason: "Bundled status",
                        replaces: ["nvidia/bundled-older"],
                        replacedBy: "nvidia/bundled-replacement",
                      },
                    ],
                  },
                },
  ...
  it.each(["deprecated", "disabled"] as const)(
    "does not import remote lifecycle metadata into a manifest %s model",
    (status) => {
      const plan = planManifestModelCatalogRows({
        registry: {
          plugins: [
            {
              ...
              modelCatalog: {
                providers: {
                  nvidia: { models: [{ id: "retired", status }] },
                },
              },
            },
          ],
      ...
      expect(plan.rows[0]).toMatchObject({ id: "retired", status });
      expect(plan.rows[0]).not.toHaveProperty("statusReason");
      expect(plan.rows[0]).not.toHaveProperty("replaces");
      expect(plan.rows[0]).not.toHaveProperty("replacedBy");
    },
  );
