...
function classifyCloneFailure(params: {
  ...
  if (
    ...
    return new ProjectCloneError(
      ...
      params.tokenConfigured
        ? "GitHub rejected the configured Control UI credential. Update gateway.controlUi.github.token and retry."
        : "GitHub authentication is required. Configure gateway.controlUi.github.token or the Gateway GH_TOKEN/GITHUB_TOKEN environment fallback to clone private repositories.",
    );
  }
  if (/repository not found|not found/u.test(detail)) {
    return params.tokenConfigured
      ...
      : new ProjectCloneError(
          "auth_required",
          "The repository was not found or is private. Check the URL, or configure gateway.controlUi.github.token (or the Gateway environment fallback) for private repositories.",
        );
  }
  return new ProjectCloneError(
    "clone_failed",
