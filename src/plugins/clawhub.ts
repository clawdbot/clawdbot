...
function validateClawHubPluginPackage(params: {
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this OpenCrustacean runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires OpenCrustacean >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
