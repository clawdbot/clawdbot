...
describe("plugin status reports", () => {
  ...
  it.each([
    { mode: "snapshot", load: () => buildPluginSnapshotReport({ config: {} }) },
    { mode: "runtime", load: () => buildPluginDiagnosticsReport({ config: {} }) },
    ...
    setSinglePluginLoadResult(
      createPluginRecord({
        ...
        dependencyStatus: {
          ...
          missingOptional: [],
          dependencies: [
            { name: "missing-runtime", spec: "1.0.0", installed: false, optional: false },
          ],
    ...
    const report = load();
    const plugin = report.plugins.find((entry) => entry.id === "missing-dependency-plugin");
