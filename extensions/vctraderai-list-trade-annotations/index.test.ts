...
import plugin, { runListTradeAnnotations, LIST_TRADE_ANNOTATIONS_TOOL_NAME } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-list-trade-annotations", () => {
  ...
  it("registers the list_trade_annotations tool with the plugin api", () => {
    const captured = createCapturedPluginRegistration({ id: "vctraderai-list-trade-annotations" });
  ...
  it("calls the workspace-scoped read with the owner bearer", async () => {
    ...
    await runListTradeAnnotations(
      {
        trade_source: "trade_source-x",
        source_run_id: "source_run_id-x",
        trade_key: "trade_key-x",
        limit: 1,
      },
      { fetchImpl },
    );
    expect(new URL(capturedUrl).pathname).toBe(`/api/v1/workspaces/${WORKSPACE_ID}/journal`);
