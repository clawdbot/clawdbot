...
// Delivery-boundary proof: a `||` inside code is a literal, not a spoiler attempt,
// so the body that reaches the Matrix client must not collapse to "[Spoiler]".
// These exercise the real sendMessageMatrix -> chunking -> markdownToMatrixBody
// path with a mocked Matrix client, covering the changed delivery boundary the
// pure-function tests in format.test.ts do not reach.
describe("sendMessageMatrix keeps the body when || appears inside code", () => {
  beforeEach(() => {
    resetMatrixSendRuntimeMocks();
  });

  it("delivers the inline-code body to the Matrix client without collapsing it", async () => {
    const { client, sendMessage } = makeClient();
    await sendMessageMatrix("room:!room:example", "Ran `rg foo || true` and found nothing.", {
      client,
      cfg: {} as never,
    });

    expect(sendMessage).toHaveBeenCalledOnce();
    const content = sentContent(sendMessage) as { body?: string };
    expect(content.body).toContain("rg foo");
    expect(content.body).toContain("|| true");
    expect(content.body).not.toBe("[Spoiler]");
  });

  it("delivers the fenced-code-block body to the Matrix client without collapsing it", async () => {
    const { client, sendMessage } = makeClient();
    await sendMessageMatrix("room:!room:example", "Result:\n\n```\nrg foo || true\n```\n", {
      client,
      cfg: {} as never,
    });
  ...
  it("preserves inline-code text and still redacts a real spoiler in the delivered body", async () => {
    const { client, sendMessage } = makeClient();
    await sendMessageMatrix(
      "room:!room:example",
      "Ran `rg foo || true` and ||secret|| found nothing.",
      { client, cfg: {} as never },
    );
