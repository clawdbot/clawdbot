...
describe("monitorMSTeamsProvider lifecycle", () => {
  ...
  it.each([false, true])(
    "preserves stable group conversation allowlists through provider startup without widening DMs (name matching: %s)",
  ...
  it.each([false, true])(
    "preserves group conversation fallback through provider startup without widening DMs (name matching: %s)",
    async (dangerouslyAllowNameMatching) => {
      isDangerousNameMatchingEnabled.mockReturnValue(dangerouslyAllowNameMatching);
      const abort = new AbortController();
      const cfg = createConfig(0);
  ...
  it("preserves Graph-resolved sender identities in the group conversation fallback", async () => {
