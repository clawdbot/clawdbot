...
describe("msteams monitor handler authz", () => {
  ...
  ])("does not authorize $label by a group conversation allowlist", async (testCase) => {
    runtimeApiMockState.dispatchReplyWithBufferedBlockDispatcher.mockClear();
    const { conversationStore, deps } = createDeps({
      channels: {
        msteams: {
          ...
          dangerouslyAllowNameMatching:
            "dangerouslyAllowNameMatching" in testCase && testCase.dangerouslyAllowNameMatching,
        },
      },
    } as OpenClawConfig);
    ...
    await handler(
      createMessageActivity({
        ...
        from: {
          ...
          name: "senderName" in testCase ? testCase.senderName : "Unlisted Member",
        },
        conversation: {
          id: testCase.conversationId,
          conversationType: testCase.conversationType,
  ...
  it.each([
  ...
  ])("$label over the loopback Bot Framework activity endpoint", async (testCase) => {
    runtimeApiMockState.dispatchReplyWithBufferedBlockDispatcher.mockClear();
    const { conversationStore, deps } = createDeps({
      channels: {
        msteams: {
    ...
    const activity = createMessageActivity({
      ...
      from: {
        ...
        name: "senderName" in testCase ? testCase.senderName : "Loopback Member",
      },
      conversation: {
        id: testCase.conversationId,
        conversationType: testCase.conversationType,
