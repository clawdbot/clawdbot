...
import { runRiskGatePreview } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-risk-gate-preview egress allowlist", () => {
  ...
  it("every captured url on the happy path (POST body) matches the workspace-scoped allowlist", async () => {
    ...
    await runRiskGatePreview(
      { account_id: "acct-1", instrument: "EUR_USD", direction: "buy" },
      { fetchImpl },
    );
  ...
  it("enforces the allowlist even for a body-carrying POST before a socket opens", async () => {
    ...
    await expect(
      bffFetch("/internal/admin/secrets", { method: "POST", body: { account_id: "acct-1" } }),
