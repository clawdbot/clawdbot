...
import plugin, { runGetLiveAccountState, GET_LIVE_ACCOUNT_STATE_TOOL_NAME } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-get-live-account-state", () => {
  ...
  it("registers the get_live_account_state tool with the plugin api", () => {
    const captured = createCapturedPluginRegistration({ id: "vctraderai-get-live-account-state" });
  ...
  it("calls the workspace-scoped read with the owner bearer", async () => {
    ...
    await runGetLiveAccountState({ account_id: "account_id-x" }, { fetchImpl });
    expect(new URL(capturedUrl).pathname).toBe(
      `/api/v1/workspaces/${WORKSPACE_ID}/live/account-state`,
    );
  ...
  it("surfaces a structured error on bff 4xx", async () => {
    ...
    await expect(
      runGetLiveAccountState({ account_id: "account_id-x" }, { fetchImpl }),
