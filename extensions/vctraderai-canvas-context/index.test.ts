...
import plugin, { runCanvasContext, CANVAS_CONTEXT_TOOL_NAME } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-canvas-context", () => {
  ...
  it("registers the canvas_context tool with the plugin api", () => {
    const captured = createCapturedPluginRegistration({ id: "vctraderai-canvas-context" });
  ...
  it("calls the workspace-scoped read with the owner bearer", async () => {
    ...
    await runCanvasContext(
      { account_id: "account_id-x", symbol: "EUR_USD", timeframe: "H1" },
      { fetchImpl },
    );
    expect(new URL(capturedUrl).pathname).toBe(`/api/v1/workspaces/${WORKSPACE_ID}/cockpit/canvas`);
