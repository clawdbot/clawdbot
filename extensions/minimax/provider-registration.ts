...
async function resolveApiCatalog(ctx: ProviderCatalogContext) {
  ...
  if (!auth.apiKey) {
    return {
      provider: buildMinimaxProvider(ctx.env),
      outcomes: [{ provider: API_PROVIDER_ID, status: "ready" as const }],
    };
  }
...
async function resolvePortalCatalog(ctx: ProviderCatalogContext) {
  ...
  if (!apiKey) {
    return {
      provider: buildMinimaxPortalProvider(ctx.env),
      outcomes: [{ provider: PORTAL_PROVIDER_ID, status: "ready" as const }],
    };
  }
  ...
  const discoveryAuth = apiKeyAuth.discoveryApiKey
    ? apiKeyAuth
    : usesPortalBearerAuth && profileAuth.discoveryApiKey
      ? profileAuth
      : apiKeyAuth.apiKey
        ? apiKeyAuth
        : explicitApiKey
