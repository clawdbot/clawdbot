...
describe("Codex /codex plugins subcommand", () => {
  ...
  it.each(["openai-curated", "openai-curated-remote"])(
    "prefers the exact %s marketplace when local and remote curated sources differ",
    async (requestedMarketplace) => {
      const io = inMemoryIO();
      const remotePluginId = "plugins~Plugin_github_opaque";
      const localMarketplacePath = "/managed/openai-curated/marketplace.json";
      const runtime = {
        ...pluginRuntime({ pluginName: "github", remotePluginId }),
        list: vi.fn(async (params: v2.PluginListParams) => {
          const remote = Boolean(params.marketplaceKinds);
          const marketplace = remote ? "openai-curated-remote" : "openai-curated";
          return {
            marketplaces: [
              {
                name: marketplace,
                ...(remote ? {} : { path: localMarketplacePath }),
                plugins: [pluginSummary("github", marketplace, remote ? { remotePluginId } : {})],
              },
            ],
            marketplaceLoadErrors: [],
            featuredPluginIds: [],
      ...
      const result = await handleCodexPluginsSubcommand(
        fakeCtx,
        ["install", `github@${requestedMarketplace}`],
        io,
        runtime,
      );

      expect(runtime.install).toHaveBeenCalledWith(
        requestedMarketplace === "openai-curated"
          ? { marketplacePath: localMarketplacePath, pluginName: "github" }
          : { remoteMarketplaceName: requestedMarketplace, pluginName: remotePluginId },
      );
      expect(io.current()).toHaveProperty("github@openai-curated");
      expect(result.text).toContain("installed and authorized");
    },
