...
describe("Codex app-server dynamic tool build", () => {
  ...
  it("falls back to messageProvider when messageChannel is absent for the question prompt (#139809)", async () => {
    // A Telegram direct message often carries messageProvider=telegram with no
    // explicit messageChannel. The question prompt must still resolve a
    // deliverable channel so the secrets tool can publish its protected link.
    const workspaceDir = path.join(tempDir, "question-prompt-provider-workspace");
    const params = createParams(
      path.join(tempDir, "question-prompt-provider-session.jsonl"),
      workspaceDir,
    );
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    params.messageProvider = "telegram";
    // Intentionally no params.messageChannel — this is the reported bug shape.
    const onToolResult = vi.fn();
    params.onToolResult = onToolResult;
    let capturedQuestionPrompt: OpenClawCodingToolsOptionsForTest["questionPrompt"];
    setOpenClawCodingToolsFactoryForTests((options) => {
