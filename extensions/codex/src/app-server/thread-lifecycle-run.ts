...
import {
  ...
  withReclaimedCodexSessionLease,
...
export async function startOrResumeThread(
  ...
  // session id. Reclaim and acquire the lease in one atomic transition so a
  // stale same-identity lifecycle cannot take the recovered binding first.
  return await withReclaimedCodexSessionLease({
