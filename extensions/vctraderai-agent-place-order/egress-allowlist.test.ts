...
import { runAgentPlaceOrder } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-agent-place-order egress allowlist", () => {
  ...
  it("accepts the plugin's own workspace-scoped live path", () => {
    expect(`/api/v1/workspaces/${WORKSPACE_ID}/live/orders/agent-place`).toMatch(
      VCTRADERAI_BFF_ALLOWLIST_PATH_PATTERN,
    );
  });
  ...
  it("rejects an absolute external URL with a valid-looking prefix", async () => {
    ...
    await expect(
      bffFetch(
        `https://evil.example.com/api/v1/workspaces/${WORKSPACE_ID}/live/orders/agent-place`,
      ),
    ).rejects.toBeInstanceOf(BffEgressViolation);
    expect(fetchImpl).not.toHaveBeenCalled();
  });
