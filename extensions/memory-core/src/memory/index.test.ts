...
describe("memory index", () => {
  ...
  it("recovers the configured primary when the fallback index identity is valid", async () => {
    ...
    const callsBeforeRecovery = providerFixture.providerCalls.length;
    await expect(manager.search("alpha")).resolves.not.toStrictEqual([]);
    expect(
      providerFixture.providerCalls.slice(callsBeforeRecovery).map((call) => call.provider),
    ).toContain("openai");
    await vi.waitFor(() => {
  ...
  it("returns fallback results before a stalled primary recovery finishes", async () => {
    ...
    const queryCallsBeforeRecovery = providerFixture.providerQueryCalls;
    ...
    providerFixture.providerProbeGate = new Promise<void>((resolve) => {
      releaseProbe = resolve;
    });
    providerFixture.providerQueryTimeoutMs = 1_000;
    try {
      const results = await manager.search("alpha");
      expect(results).not.toEqual([]);
      await vi.waitFor(() => {
        expect(providerFixture.providerQueryCalls).toBe(queryCallsBeforeRecovery + 1);
      });
      expect(fields.provider?.id).toBe("fallback-provider");
      ...
      releaseProbe();
      providerFixture.providerProbeGate = null;
      providerFixture.providerQueryTimeoutMs = null;
    }
  });
  ...
  it("waits for a background primary recovery before closing the manager", async () => {
    ...
    try {
      await expect(manager.search("alpha")).resolves.not.toStrictEqual([]);
      await vi.waitFor(() => expect(providerFixture.providerQueryCalls).toBeGreaterThan(0));
