...
import {
  ...
  inspectEmbeddingProviderStartupPrerequisites,
...
describe("inspectEmbeddingProviderStartupPrerequisites", () => {
  beforeEach(() => {
    clearMemoryEmbeddingProviders();
  });
  ...
  it("returns indeterminate without invoking create when inspection is unsupported", async () => {
    const create = vi.fn(async () => ({ provider: null }));
    registerMemoryEmbeddingProvider({ id: "openai", create });

    await expect(
      inspectEmbeddingProviderStartupPrerequisites(createOptions("openai")),
    ).resolves.toEqual({
  ...
  it("returns a provider-owned blocker without invoking create", async () => {
    const create = vi.fn(async () => ({ provider: null }));
    registerMemoryEmbeddingProvider({
      id: "local",
        ...
        issues: [
          {
            ...
            message: `Missing setup for ${options.model}`,
          },
        ],
      }),
      create,
    ...
    await expect(
      inspectEmbeddingProviderStartupPrerequisites(createOptions("local")),
    ).resolves.toEqual({
      status: "blocked",
      issues: [
        {
          provider: "local",
          code: "managed-server-config-missing",
          message: "Missing setup for default.gguf",
        },
      ],
    });
    expect(create).not.toHaveBeenCalled();
  ...
  it("accepts a ready fallback and preserves fallback-owned options", async () => {
    const primaryCreate = vi.fn(async () => ({ provider: null }));
    const fallbackCreate = vi.fn(async () => ({ provider: null }));
    const fallbackInspect = vi.fn<
      NonNullable<MemoryEmbeddingProviderAdapter["inspectStartupPrerequisites"]>
    >(() => ({
      status: "ready",
    }));
    registerMemoryEmbeddingProvider({
      ...
      inspectStartupPrerequisites: () => ({
        status: "blocked",
        issues: [{ code: "missing", message: "missing" }],
      }),
    ...
    registerMemoryEmbeddingProvider({
      ...
      inspectStartupPrerequisites: fallbackInspect,
    ...
    expect(fallbackInspect).toHaveBeenCalledWith(
      expect.objectContaining({
        provider: "ollama",
  ...
  it("returns indeterminate when a declared inspection throws", async () => {
    const create = vi.fn(async () => ({ provider: null }));
    registerMemoryEmbeddingProvider({
      id: "openai",
      inspectStartupPrerequisites: () => {
    ...
    await expect(
      ...
      status: "indeterminate",
      reason: 'Embedding provider "openai" startup inspection failed: inspection failed',
    });
    expect(create).not.toHaveBeenCalled();
  });
});
