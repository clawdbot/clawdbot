...
// The proof permits up to 219 seconds of sequential request/quiet waits. Keep
// startup and teardown headroom outside that proof budget.
const TWO_WAVE_TIMEOUT_MS = 300_000;
...
describe("two-wave requester-settle observed proof", () => {
  ...
  it(
    "proves barrier-observed two-wave requester-settle with quiet gate",
    async () => {
        ...
        const wave1SettleWakeReq = await pollDebugRequests(
          ...
          (reqs) =>
            reqs
              ...
              .find((r) => r.allInputText.includes(SETTLE_WAKE_NEEDLE)),
          30_000,
        );
