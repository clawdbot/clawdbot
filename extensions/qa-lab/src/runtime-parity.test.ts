...
describe("runtime parity", () => {
  ...
  it("ignores Codex mirror records and shared logs without an OpenClaw selection", async () => {
    const tempRoot = await seedRuntimeParityTranscript({
      ...
      sessionKey: "agent:qa:forced-codex-embedded-runtime",
      messages: [
  ...
  it("fails a forced-Codex mock cell that selects the OpenClaw fallback", async () => {
    const tempRoot = await seedRuntimeParityTranscript({
      ...
      sessionKey: "agent:qa:forced-codex-openclaw-selection",
  ...
  it("fails a forced-Codex mock cell containing any OpenClaw Responses record", async () => {
    const tempRoot = await seedRuntimeParityTranscript({
      sessionId: "forced-codex-openai-egress",
      sessionKey: "agent:qa:forced-codex-openai-egress",
      messages: [
        { role: "user", content: "runtime isolation check" },
        {
          role: "assistant",
