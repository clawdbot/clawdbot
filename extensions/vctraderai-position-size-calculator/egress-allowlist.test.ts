...
import { runPositionSizeCalculator } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-position-size-calculator egress allowlist", () => {
  ...
  it("enforces the allowlist even for a body-carrying POST before a socket opens", async () => {
    ...
    await expect(
      bffFetch("/internal/admin/secrets", { method: "POST", body: { symbol: "EUR_USD" } }),
