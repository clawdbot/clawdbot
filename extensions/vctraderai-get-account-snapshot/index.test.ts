...
import plugin, { runGetAccountSnapshot, GET_ACCOUNT_SNAPSHOT_TOOL_NAME } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-get-account-snapshot", () => {
  ...
  it("registers the get_account_snapshot tool with the plugin api", () => {
    const captured = createCapturedPluginRegistration({ id: "vctraderai-get-account-snapshot" });
  ...
  it("calls the workspace-scoped read with the owner bearer", async () => {
    ...
    await runGetAccountSnapshot(
      { mt5_account_id: "mt5_account_id-x", history_limit: 1 },
      { fetchImpl },
    );
    expect(new URL(capturedUrl).pathname).toBe(
      `/api/v1/workspaces/${WORKSPACE_ID}/openclaw/live/accounts/mt5_account_id-x/snapshot`,
    );
