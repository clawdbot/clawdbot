...
import { runGetRunMetrics } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-get-run-metrics egress allowlist", () => {
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runGetRunMetrics({ run_id: "run_id-x", schema: "schema-x", limit: 1 }, { fetchImpl });
