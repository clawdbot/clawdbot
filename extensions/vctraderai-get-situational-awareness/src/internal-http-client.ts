// VC Trader AI BFF HTTP client for the get_situational_awareness read tool.
//
// Uses the server-to-server OPENCLAW_GATEWAY_TOKEN and restricts egress to the
// single OpenClaw internal BFF route this plugin is allowed to call
// (GET /api/v1/openclaw/situational-awareness). Any non-allowlisted path is
// rejected before a socket opens, so a buggy or malicious tool body cannot
...
const ALLOWLIST_PATH_PATTERN = /^\/api\/v1\/openclaw\/situational-awareness(\?.*)?$/;
