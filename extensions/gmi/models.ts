...
export const buildGmiModelDefinition = buildManifestModelDefinition({
  providerId: "gmi",
  catalog: GMI_MANIFEST_CATALOG,
  decorate: (model) => ({ ...model, api: "openai-completions" }),
});
export const GMI_MODEL_CATALOG: ModelDefinitionConfig[] =
  GMI_MANIFEST_CATALOG.models.map(buildGmiModelDefinition);
