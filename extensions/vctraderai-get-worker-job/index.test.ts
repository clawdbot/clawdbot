...
import plugin, { runGetWorkerJob, GET_WORKER_JOB_TOOL_NAME } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-get-worker-job", () => {
  ...
  it("registers the get_worker_job tool with the plugin api", () => {
    const captured = createCapturedPluginRegistration({ id: "vctraderai-get-worker-job" });
  ...
  it("calls the workspace-scoped read with the owner bearer", async () => {
    ...
    await runGetWorkerJob({ job_id: "job_id-x" }, { fetchImpl });
    expect(new URL(capturedUrl).pathname).toBe(
      `/api/v1/workspaces/${WORKSPACE_ID}/openclaw/research/worker-jobs/job_id-x`,
    );
  ...
  it("surfaces a structured error on bff 4xx", async () => {
    ...
    await expect(runGetWorkerJob({ job_id: "job_id-x" }, { fetchImpl })).rejects.toMatchObject({
