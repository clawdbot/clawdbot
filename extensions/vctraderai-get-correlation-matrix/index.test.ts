...
import plugin, { runGetCorrelationMatrix, GET_CORRELATION_MATRIX_TOOL_NAME } from "./index.js";

const WORKSPACE_ID = "11111111-2222-3333-4444-555555555555";

describe("vctraderai-get-correlation-matrix", () => {
  ...
  it("registers the get_correlation_matrix tool with the plugin api", () => {
    const captured = createCapturedPluginRegistration({ id: "vctraderai-get-correlation-matrix" });
  ...
  it("calls the workspace-scoped read with the owner bearer", async () => {
    ...
    await runGetCorrelationMatrix(
      { method: "method-x", min_overlap: 2, window_days: 1 },
      { fetchImpl },
    );
    expect(new URL(capturedUrl).pathname).toBe(
      `/api/v1/workspaces/${WORKSPACE_ID}/openclaw/analytics/correlation-matrix`,
    );
