...
beforeAll(async () => {
  const mockedProviderHttp = await import("openclaw/plugin-sdk/provider-http");
  const actualProviderHttp = await vi.importActual<
    typeof import("openclaw/plugin-sdk/provider-http")
  >("openclaw/plugin-sdk/provider-http");
  vi.doMock("openclaw/plugin-sdk/provider-http", () => ({
    ...mockedProviderHttp,
    readProviderBinaryResponse: actualProviderHttp.readProviderBinaryResponse,
  }));
...
describe("openai video generation provider", () => {
  ...
  it.each([
    { label: "queued text-to-video", status: "queued", referenceKind: undefined },
    { label: "queued image-to-video", status: "queued", referenceKind: "image" },
    { label: "queued video edit", status: "queued", referenceKind: "video" },
    { label: "already-completed text-to-video", status: "completed", referenceKind: undefined },
  ] as const)("releases the $label submission before polling or downloading", async (testCase) => {
    ...
    if (testCase.status === "queued") {
      pollProviderOperationJsonMock.mockImplementationOnce(async () => {
        expect(submissionRelease).toHaveBeenCalledOnce();
        return { id: "vid_released", model: "sora-2", status: "completed" };
      });
    }
    fetchWithTimeoutMock.mockImplementationOnce(async () => {
      expect(submissionRelease).toHaveBeenCalledOnce();
      return streamedVideoResponse("rendered-video");
    });

    const result = await buildOpenAIVideoGenerationProvider().generateVideo({
      provider: "openai",
      model: "sora-2",
      prompt: "Release the submission before the next request",
      cfg: {},
      ...(testCase.referenceKind === "image"
        ? { inputImages: [{ buffer: Buffer.from("image"), mimeType: "image/png" }] }
        : {}),
      ...(testCase.referenceKind === "video"
        ? { inputVideos: [{ buffer: Buffer.from("video"), mimeType: "video/mp4" }] }
        : {}),
    });

    expect(result.videos[0]?.buffer?.toString()).toBe("rendered-video");
    expect(submissionRelease).toHaveBeenCalledOnce();
  });
  ...
  it.each([
    ...
    { label: "image", contentType: "image/png", body: "image-bytes" },
    { label: "audio", contentType: "audio/mp4", body: "audio-bytes" },
    {
      label: "a second content type hidden in video parameters",
      contentType: "video/mp4; codecs=avc1, application/json",
      body: "video-bytes",
    },
    {
      label: "a missing video type before a conflicting type",
      contentType: "; video/mp4, application/json",
      body: "video-bytes",
    },
    { label: "a present empty video content type", contentType: "", body: "video-bytes" },
  ...
  it.each([
    { contentType: "video/webm", expectedMimeType: "video/webm" },
  ...
  ])("accepts a nonempty generated video with content type $contentType", async (testCase) => {
    postMultipartRequestMock.mockResolvedValueOnce({
      response: streamedJsonResponse({ id: "vid_valid", status: "completed" }),
      release: vi.fn(async () => {}),
    });
    fetchWithTimeoutMock.mockResolvedValueOnce(
      new Response(
        Buffer.from("rendered-video"),
        testCase.contentType ? { headers: { "content-type": testCase.contentType } } : undefined,
      ),
    );
    ...
    const result = await buildOpenAIVideoGenerationProvider().generateVideo({
      provider: "openai",
      model: "sora-2",
      prompt: "Keep valid generated video content types",
      cfg: {},
    });

    expect(result.videos[0]).toMatchObject({
      buffer: Buffer.from("rendered-video"),
