...
import { runModifyPositionProtection } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-modify-position-protection egress allowlist", () => {
  ...
  it("accepts the plugin's own workspace-scoped live path", () => {
    expect(`/api/v1/workspaces/${WORKSPACE_ID}/live/positions/pos-9/agent-protection`).toMatch(
      VCTRADERAI_BFF_ALLOWLIST_PATH_PATTERN,
    );
  });
  ...
  it("every captured url on the happy path matches the workspace-scoped allowlist", async () => {
    ...
    await runModifyPositionProtection(
      { account_id: "acct-1", position_id: "pos-9", sl: 1.1 },
      { fetchImpl },
    );
