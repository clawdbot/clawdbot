...
describe("Telegram model callback loopback", () => {
  ...
  it.each(["edit", "replacement", "undeliverable"] as const)(
    "sends, authorizes, persists, and confirms an opaque callback (%s)",
    async (notification) => {
      const stateDir = await mkdtemp(join(tmpdir(), "openclaw-telegram-model-loopback-"));
      const requests: TelegramApiRequest[] = [];
      const runtimeErrors: string[] = [];
      let failEdit = notification !== "edit";
      let failReceiptSend = notification === "undeliverable";
      ...
      const handleApiRequest = async (request: IncomingMessage, response: ServerResponse) => {
        ...
        if (method === "sendMessage") {
          if (failReceiptSend && sentMessage) {
            failReceiptSend = false;
            response.destroy();
            return;
          }
      ...
      try {
        ...
        const router = createTelegramCallbackRouter({
          params: {
            accountId: "default",
            bot,
            runtime: { error: (errorMessage: string) => runtimeErrors.push(errorMessage) },
            telegramDeps,
        ...
        expect(requests.map(({ method }) => method)).toEqual([
          ...
          "answerCallbackQuery",
          "editMessageText",
          ...(notification === "edit" ? [] : ["editMessageReplyMarkup", "sendMessage"]),
        ]);
        expect(runtimeErrors).toHaveLength(
          notification === "edit" ? 0 : notification === "replacement" ? 1 : 2,
        );
        for (const error of runtimeErrors) {
          expect(error).toContain("Model selection applied;");
          expect(error).toContain("HttpError");
        }
