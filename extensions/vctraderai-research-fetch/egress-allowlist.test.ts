...
import { runResearchFetchPage } from "./index.js";
import {
  BffEgressViolation,
  createBffFetch,
...
describe("vctraderai-research-fetch egress allowlist", () => {
  ...
  it("every captured url on the happy path (POST body) matches the workspace-scoped allowlist", async () => {
    ...
    await runResearchFetchPage({ url: "https://example.com" }, { fetchImpl });
  ...
  it("enforces the allowlist even for a body-carrying POST before a socket opens", async () => {
    ...
    await expect(
      bffFetch("/internal/admin/secrets", { method: "POST", body: { url: "https://x" } }),
