...
describe("stage-live-auth-profiles", () => {
  ...
  it("materializes the live OpenAI environment key into an otherwise empty isolated store", () => {
    const sourceStateDir = createStateDir("openclaw-live-auth-empty-source-");
    const targetStateDir = createStateDir("openclaw-live-auth-env-target-");

    stageLiveAuthProfiles(sourceStateDir, targetStateDir, {
      OPENAI_API_KEY: "prepared-openai-test-key",
    });

    const targetAgentDir = path.join(targetStateDir, "agents", "main", "agent");
    expect(inspectPersistedAuthProfileStoreRaw(targetAgentDir)).toEqual({
      status: "readable",
      raw: {
        version: 1,
        profiles: {
    ...
    expect(inspectPersistedAuthProfileStateRaw(targetAgentDir)).toEqual({
      status: "readable",
      raw: { version: 1, order: { openai: ["openai:live"] } },
    });
  });

  it("puts the live OpenAI environment key before staged profiles", () => {
    const sourceStateDir = createStateDir("openclaw-live-auth-source-");
    const targetStateDir = createStateDir("openclaw-live-auth-target-");
    createAuthSource(sourceStateDir);
    ...
    const targetAgentDir = path.join(targetStateDir, "agents", "main", "agent");
    const store = inspectPersistedAuthProfileStoreRaw(targetAgentDir);
    const state = inspectPersistedAuthProfileStateRaw(targetAgentDir);
    expect(store.status === "readable" ? store.raw : null).toMatchObject({
      profiles: {
        "openai:test": expect.any(Object),
        "openai:live": {
          type: "api_key",
          provider: "openai",
          key: "prepared-openai-test-key",
    ...
    expect(state.status === "readable" ? state.raw : null).toMatchObject({
      order: { openai: ["openai:live", "openai:test"] },
    });
  });
});
