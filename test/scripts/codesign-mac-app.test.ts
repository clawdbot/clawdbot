...
describe("codesign-mac-app temp file hygiene", () => {
  ...
  macIt("cleans the actual entitlement directory when signing fails", () => {
    const tempRoot = tempDirs.make("openclaw-codesign-fail-");
    const app = path.join(tempRoot, "Fake.app");
    const binDir = path.join(tempRoot, "bin");
    const capture = path.join(app, "Contents", "signing-temp");
    const signer = path.join(binDir, "codesign");
    mkdirSync(path.join(app, "Contents", "MacOS"), { recursive: true });
    mkdirSync(binDir);
    writeFileSync(
      signer,
...
const entitlements = args[args.indexOf('--entitlements') + 1];
const existed = fs.statSync(directory).isDirectory() && fs.statSync(entitlements).isFile();
fs.writeFileSync(${JSON.stringify(capture)}, [directory, entitlements, String(existed)].join('\\0'));
process.exit(7);
`,
    );
    ...
    // Darwin mktemp -t can ignore TMPDIR; observe the paths used by the signer.
    const [directory, entitlements, existed] = readFileSync(capture, "utf8").split("\0");
    const actualDirectory = expectDefined(directory, "allocated signing directory");
    const actualEntitlements = expectDefined(entitlements, "allocated entitlements file");
    try {
      expect(existed).toBe("true");
      expect(path.dirname(actualEntitlements)).toBe(actualDirectory);
      expect(existsSync(actualEntitlements)).toBe(false);
      expect(existsSync(actualDirectory)).toBe(false);
    } finally {
      // Clean a regression's task-created leak, never an unexpected system path.
      if (path.basename(actualDirectory).startsWith("openclaw-entitlements.")) {
        cleanupTempDirs([actualDirectory]);
      }
    }
  });
  ...
  macIt("does not retry non-timestamp signing failures", () => {
