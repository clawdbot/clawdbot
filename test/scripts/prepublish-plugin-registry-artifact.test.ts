...
import {
  ...
  validateNpmPackageTarballIdentity,
...
describe("prepublish plugin registry artifact", () => {
  ...
  it("validates caller-owned tarball identity before registry serving", () => {
    const paths = fixture();
    ...
    expect(
      validateNpmPackageTarballIdentity({
        ...
        tarball: paths.tarballPath,
      }),
    ).toMatchObject({ name: PACKAGE_NAME, version: VERSION });
    expect(() =>
      validateNpmPackageTarballIdentity({
        expectedName: "@openclaw/slack",
        expectedVersion: VERSION,
        tarball: paths.tarballPath,
      }),
