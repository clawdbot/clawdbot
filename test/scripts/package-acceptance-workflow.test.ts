...
describe("package artifact reuse", () => {
  ...
  it("keeps release publish creation compatible with gh api and prerelease notes", () => {
    ...
    expect(maintainerSkill).toContain("its exact `full_release_validation_run_attempt`");
  ...
  it("keeps beta release verification and ClawHub publish repair hooks wired", () => {
    ...
    expect(
      workflowStep(
        ...
        "Upload continuation identity",
      ).with?.name,
    ).toContain("clawhub-continuation-identity-");
    expect(
      workflowStep(
        workflowJob(".github/workflows/plugin-clawhub-new.yml", "resolve_bootstrap_plan"),
        "Upload continuation identity",
      ).with?.name,
    ).toContain("clawhub-continuation-identity-");
