...
`,
  ...
  it("keeps MCP permission ownership strict when frozen omissions are disabled", () => {
    const result = spawnSync(
      "bash",
      [
        "-c",
        'set -euo pipefail; source "$1"; openclaw_resolve_frozen_mcp_channel_permission_mode "$2"; printf "%s\\n" "$OPENCLAW_FROZEN_TARGET_MCP_CHANNEL_PERMISSION_MODE"',
        "test",
        stageScriptPath,
        process.cwd(),
      ],
    ...
    expect(result.status, result.stderr).toBe(0);
    expect(result.stdout.trim()).toBe("owner-required");
  });

  it.each([
    {
      name: "owner-required permissions",
      bridge:
        "const CLAUDE_PERMISSION_REPLY_RE = /yes/;\nif (payload.senderIsOwner === true && this.pendingClaudePermissions.has(requestId)) {}\n",
      shared: "export type SessionMessagePayload = {\n  senderIsOwner?: boolean;\n};\n",
      expected: "owner-required",
    },
    {
      name: "legacy any-user permissions",
      bridge:
        "const CLAUDE_PERMISSION_REPLY_RE = /yes/;\nif (this.pendingClaudePermissions.has(requestId)) {}\n",
      shared: "export type SessionMessagePayload = {};\n",
      expected: "legacy-any-user",
    },
    {
      name: "partial ownership propagation",
      bridge:
        "const CLAUDE_PERMISSION_REPLY_RE = /yes/;\nif (payload.senderIsOwner === true && this.pendingClaudePermissions.has(requestId)) {}\n",
      shared: "export type SessionMessagePayload = {};\n",
      error: "unable to resolve frozen MCP permission ownership contract",
    },
    {
      name: "missing permission machinery",
      bridge: "export const channelBridge = true;\n",
      shared: "export type SessionMessagePayload = {};\n",
      error: "unable to resolve frozen MCP permission ownership contract",
    },
  ])("classifies $name from the selected source", ({ bridge, shared, expected, error }) => {
    const root = tempDirs.make("openclaw-frozen-target-mcp-permission-");
    const bridgePath = path.join(root, "src/mcp/channel-bridge.ts");
    const sharedPath = path.join(root, "src/mcp/channel-shared.ts");
