...
describe("write-plugin-sdk-entry-dts", () => {
  ...
  it("publishes fresh canonical partitions with stable bytes and public nominal identity", () => {
    ...
    const beforeSourceChange = treeHashes(path.join(root, "dist"));
    expectOutputs(root, qa, Object.keys(beforeSourceChange));
    expectStagingClean(root);
    ...
    // Both selections can leave shared chunks outside flat SDK publication ownership.
    const history = { ...beforeSourceChange, ...before };
    for (const file of Object.keys(history).filter((entry) => !entry.startsWith("plugin-sdk/"))) {
      expect(first[file]).toBe(history[file]);
