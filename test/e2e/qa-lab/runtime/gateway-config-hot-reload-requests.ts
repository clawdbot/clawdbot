...
export async function proveHotReloadRequests({
  ...
  for (const value of ["max-age=60", "max-age=120; includeSubDomains", false]) {
    ...
    assert.equal((await http("/healthz")).headers.get("strict-transport-security"), value || null);
    assert.equal(
  ...
  for (const enabled of [false, true, false]) {
    ...
    if (enabled) {
      assert.equal(titles.titles.proof, "Read project notes");
      assert(fixture.toolTitleRequests > before);
    } else {
