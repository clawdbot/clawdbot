...
function createSelectedPluginPackageFixture(linkedDependencies = false) {
  ...
  if (linkedDependencies) {
    const builtDependency = path.join(sourceDir, "dist/extensions/demo/node_modules/host-native");
...
describe("package-openclaw-for-docker", () => {
  it.each([false, true])(
    ...
    async (linkedDependencies) => {
      const { sourceDir, outputDir, files } =
        createSelectedPluginPackageFixture(linkedDependencies);
  ...
  it("packs the staged AI runtime with pnpm from an isolated workspace", async () => {
    const { sourceDir, outputDir, files, packageJson } = createSelectedPluginPackageFixture(true);
