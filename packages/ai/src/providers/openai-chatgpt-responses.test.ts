...
describe("streamOpenAICodexResponses transport", () => {
  ...
  it("consumes CRLF-delimited ChatGPT Responses through the provider SSE stream", async () => {
    const terminal = {
      type: "response.completed",
      response: {
        id: "resp_crlf",
...
describe("parseSSEForTest", () => {
  ...
  it.each([
    ...
    {
      label: "multiline LF",
      chunks: [`${multilineDataLines.join("\n")}\n\n`],
    },
    {
    ...
    {
      label: "multiline lone CR",
      chunks: [`${multilineDataLines.join("\r")}\r\r`],
    },
    {
