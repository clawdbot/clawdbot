...
export function createResponsesTerminalController(params: {
  ...
  getOutputItemContentIndex: (item: ResponseOutputItem) => number | undefined;
  setOutputItemContentIndex: (item: ResponseOutputItem, contentIndex: number) => void;
  ...
  const recoverTerminalOutput = (items: ResponseOutputItem[], includeToolCalls: boolean) => {
    ...
    for (const item of items.toReversed()) {
      if (item.type === "reasoning") {
        ...
        hasCompletedLaterOutput ||= params.getOutputItemContentIndex(item) !== undefined;
        continue;
      }
      ...
      if (
        params.getOutputItemContentIndex(item) !== undefined ||
    ...
    for (const [terminalIndex, item] of items.entries()) {
      if (item.type === "message") {
        const contentIndex = params.getOutputItemContentIndex(item);
        ...
        if (appendedIndex !== undefined) {
          params.setOutputItemContentIndex(item, appendedIndex);
        }
      } else {
        ...
        if (item.type === "compaction" && output.providerReplay?.id !== item.id) {
          ...
          for (const laterItem of items.slice(terminalIndex + 1)) {
            const laterContentIndex = params.getOutputItemContentIndex(laterItem);
          ...
          if (params.getOutputItemContentIndex(item) !== undefined) {
            continue;
          }
          params.setOutputItemContentIndex(item, appendToolCall(item));
        }
      }
    }
  };
