...
describe("memory search staleness", () => {
  ...
  it("reports the latest automatic sync failure", () => {
    expect(
      resolveMemorySearchStaleness(
        { provider: "openai", lastSyncError: "embedding request timed out" },
        "main",
      ),
    ).toEqual({
      stale: true,
      ...
      action:
        "Run: openclaw memory status --index --agent main. Rebuilding may call the configured embedding provider (openai) and can incur provider cost.",
    });
  });
