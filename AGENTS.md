...
- Purely additive SQLite surface (new tables or bare nullable columns; downgraded builds keep working, just without the new feature): do not bump the schema version. Additive columns must use only a SQLite `STRICT` datatype with no default or constraints so older writers can omit them. Declare the surface in the canonical schema file plus a one-time idempotent lazy ensure on first feature use; fold it into the migration path at the next natural bump. Bumps are reserved for changes older readers cannot tolerate.
