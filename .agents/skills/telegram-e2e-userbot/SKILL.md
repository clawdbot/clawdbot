...
The checkout must contain its declared
`qa/convex-credential-broker/node_modules/.bin/convex` executable. Prepare that
package once with `bun install --frozen-lockfile` in its directory.
