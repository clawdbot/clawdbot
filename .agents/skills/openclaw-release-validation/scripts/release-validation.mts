...
export function renderRunComment(state: RunState): string {
  ...
  const lines = [
    ...
    "",
    "**Final feedback:**",
    finalFeedback ? redactPublicText(finalFeedback) : "No additional feedback.",
    "",
    "**Setup and cleanup findings:**",
    ...(findings.length > 0
      ? findings.map((finding) => `- ${redactPublicText(finding)}`)
      : ["- None recorded."]),
    ...(cleanup
      ? [
          `- Fixture stopped: ${cleanup.fixtureStopped ? "yes" : "no"}`,
          `- Source restoration: ${cleanup.sourceRestoration}${cleanup.sourceRestorationNote ? ` — ${redactPublicText(cleanup.sourceRestorationNote)}` : ""}`,
          ...(cleanup.retainedArtifacts.length > 0
            ? cleanup.retainedArtifacts.map(
                (artifact) => `- Retained recovery artifact: ${redactPublicText(artifact)}`,
              )
            : []),
        ]
      : []),
