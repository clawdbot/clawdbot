...
2. Check `git status -sb`. Refresh with `git fetch origin main`; record the full `origin/main` SHA and inspect the current source, package scripts, scenario inventory, workflows, and scoped guides at that SHA. Never pull, rebase, or switch a checkout that another agent is using.
3. Make one requested, fresh `codex/` worktree per implementation task. Keep reviewer workers read-only. Refresh `origin/main` periodically and recreate or revalidate candidates against the new head before claiming current behavior.
