...
- While recording Chromium, keep screenshots viewport-only: no locator screenshots, browser-side clips, or `fullPage: true`. Crop PNGs outside the browser when needed. These captures can corrupt screencast frames despite correct PNGs and passing assertions; see [the version-scoped limitation](https://docs.openclaw.ai/reference/test#screenshots-during-chromium-recordings). Inspect finalized video content, not just dimensions.
